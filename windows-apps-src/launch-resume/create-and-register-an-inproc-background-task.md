...
豊富な**OnBackgroundActivated**の例では、[ホスト アプリと同じプロセスで実行するアプリ サービスを変換](convert-app-service-in-process.md)をご覧ください。
