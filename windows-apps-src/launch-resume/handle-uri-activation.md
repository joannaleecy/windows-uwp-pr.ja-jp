...
> [!NOTE]
> UWP [アプリ] で特定の Uri とファイル拡張子が、使用する組み込みのアプリおよびオペレーティング システムによって予約されています。 予約されている URI またはファイル拡張子にアプリを登録しようとしても無視されます。 予約または禁止されいるため、UWP アプリを登録できない URI スキームの一覧 (アルファベット順) については、「[予約済みの URI スキーム名とファイルの種類](reserved-uri-scheme-names.md)」をご覧ください。
...
> [!NOTE]
> プロトコルの契約が起動したときは、その [戻る] ボタンは、ユーザー戻るアプリの起動画面をし、アプリの前のコンテンツを確認します。
...
> [!NOTE]
> アプリの新しい名前 URI を作成する場合は、 [RFC 4395](http://go.microsoft.com/fwlink/p/?LinkID=266550)のガイダンスに従ってください。 これにより確実に名前が URI スキームの標準に準拠するようになります。
