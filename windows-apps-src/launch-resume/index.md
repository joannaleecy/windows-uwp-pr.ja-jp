...
| [別のプロセスで実行するバックグラウンド タスクの作成と登録](create-and-register-a-background-task.md)                               | バックグラウンド タスク クラスを作り、アプリがフォアグラウンドにない場合に別のプロセスで実行されるように登録します。                                                                                                 |
| [単一プロセスで実行するバックグラウンド タスクの作成と登録](create-and-register-a-singleprocess-background-task.md)                               | フォアグラウンド アプリと同じプロセスで実行されるバックグラウンド タスク クラスを作成します。                                                                                                 |
| [マルチプロセスのバックグラウンド タスクを単一プロセスのバックグラウンド タスクへ変換](convert-multiple-process-background-task.md)                               | アプリがバックグラウンドにあるときに別のプロセスで実行されるように設計されたバックグラウンド タスクを、フォアグラウンド アプリと同じプロセスで実行される単一プロセスのバックグラウンド タスクに変換する方法を説明します。
...
| [ホスト アプリと同じプロセスで実行するようにアプリ サービスを変換する](convert-app-service-single-process.md)                                | 別のバックグラウンド プロセスで実行されるアプリ サービスのコードを、アプリ サービスのホスト アプリと同じプロセス内で実行されるコードに変換する方法を説明します。                                                                                  |
