        ...
        private async Task LinqSelectColorDepthInfrared()
        {
            ...
            var selectedGroupObjects = frameSourceGroups.Select(group =>
               new
               {
                   ...
                   infraredSourceInfo = group.SourceInfos.FirstOrDefault((sourceInfo) =>
                   {
                       return sourceInfo.MediaStreamType == MediaStreamType.VideoPreview
                       && sourceInfo.SourceKind == MediaFrameSourceKind.Infrared;
                   }),
                   depthSourceInfo = group.SourceInfos.FirstOrDefault((sourceInfo) =>
                   {
                       return sourceInfo.MediaStreamType == MediaStreamType.VideoPreview
                       && sourceInfo.SourceKind == MediaFrameSourceKind.Depth;
                   })

               }).Where(t => t.colorSourceInfo != null && t.infraredSourceInfo != null && t.depthSourceInfo != null)
               .FirstOrDefault();

            MediaFrameSourceGroup selectedGroup = selectedGroupObjects?.sourceGroup;
            MediaFrameSourceInfo colorSourceInfo = selectedGroupObjects?.colorSourceInfo;
            MediaFrameSourceInfo infraredSourceInfo = selectedGroupObjects?.infraredSourceInfo;
            MediaFrameSourceInfo depthSourceInfo = selectedGroupObjects?.depthSourceInfo;
            //</SnippetColorInfraredDepth>
        }
