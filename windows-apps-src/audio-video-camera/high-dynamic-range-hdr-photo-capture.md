...
Windows 10 バージョン 1607 以降では、**AdvancedPhotoCapture** により、ローライトの設定でキャプチャされた写真の品質を高める組み込みアルゴリズムを使って、写真をキャプチャすることが可能です。

> [!NOTE] 
> **AdvancedPhotoCapture** を使ってビデオの録画と写真のキャプチャを同時に行うことはサポートされていません。

> [!NOTE] 
> [**FlashControl.Enabled**](https://msdn.microsoft.com/library/windows/apps/Windows.Media.Devices.FlashControl.Enabled) プロパティを true に設定すると、**AdvancedPhotoCapture** 設定が上書きされ、通常の写真がフラッシュを使ってキャプチャされるようになります。 [**Auto**](https://msdn.microsoft.com/library/windows/apps/Windows.Media.Devices.FlashControl.Auto) を true に設定すると、**AdvancedPhotoCapture** が構成どおりに使用され、フラッシュは使われません。
...
## ローライトの写真のキャプチャ
[**AdvancedPhotoCapture**](https://msdn.microsoft.com/library/windows/apps/Windows.Media.Capture.AdvancedPhotoCapture) クラスのローライト機能を使うと、システムは現在のシーンを評価し、必要に応じて、ローライトの状況に合わせてアルゴリズムを適用します。 システムでアルゴリズムが必要ないと判断された場合は、通常のキャプチャが実行されます。
