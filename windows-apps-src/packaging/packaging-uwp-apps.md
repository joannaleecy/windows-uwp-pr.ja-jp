...
- **アプリ パッケージ (.appx または .msix)**  
    デバイスにサイドローディングできる形式でアプリが含まれているファイル。 Visual Studio によって作成されたすべての 1 つのアプリ パッケージ ファイルは、**いない**デベロッパー センターに提出するためのものし、サイドローディングおよびテスト目的にのみ使用する必要があります。 デベロッパー センターにアプリを提出する場合、アプリ パッケージ アップロード ファイルを使います。  
