...
| **ユーザー設定のデバイス** | **LowLevelDevices**機能には、アプリの他の要件が満たされた場合にユーザー設定のデバイスにアクセスすることができます。 この機能は、GPIO、I2C、SPI、および PWM デバイスへのアクセス許可**ローレベル**のデバイスの機能と混同しないでください。<br /><br /> [デバイスのインターフェイス](https://docs.microsoft.com/windows-hardware/drivers/install/device-interface-classes)を公開するユーザー設定のドライバーを開発するには、このデバイスのハンドルを開くをしたい場合は、する必要があります。<ul><li>アプリケーション マニフェストで**lowLevelDevices**機能を有効にします。 <table><thead><tr><th>XML</th></tr></thead><tbody><tr><td><pre><code>&lt;Capabilities&gt;&lt;iot:Capability Name="lowLevelDevices"/&gt;&lt;/Capabilities&gt;</code></pre></td></tr></tbody></table></li><li>[埋め込みモード](https://docs.microsoft.com/windows/iot-core/develop-your-app/EmbeddedMode)を有効にします。</li><li>[制限付き](https://docs.microsoft.com/windows-hardware/drivers/install/devpkey-deviceinterface-restricted) [INF](https://msdn.microsoft.com/library/windows/desktop/hh404264(v=vs.85).aspx)またはドライバー [WdfDeviceAssignInterfaceProperty()](https://docs.microsoft.com/windows-hardware/drivers/ddi/content/wdfdevice/nf-wdfdevice-wdfdeviceassigninterfaceproperty)を使用して名前を付けて、デバイスのインターフェイスをマークします。</ul>  <br /><br />お使いのデバイスにハンドルを開く[**Windows.Devices.Custom.CustomDevice**](https://docs.microsoft.com/uwp/api/Windows.Devices.Custom.CustomDevice)を使用できます。 詳細については、[内部デバイス用の UWP デバイスのアプリ](https://docs.microsoft.com/windows-hardware/drivers/devapps/uwp-device-apps-for-specialized-devices)を参照してください。
...
> [!NOTE]
> 申請でデベロッパー センターの開発サンドボックスを使う場合 (たとえば、Xbox Live と統合されるゲームがこれに該当します)、**申請オプション** ページで情報を提供するのではなく事前に承認をリクエストする必要があります。 このためには、[Windows 開発者向けサポート ページ](https://developer.microsoft.com/windows/support)にアクセスしてください。 開発のサポート トピック**ダッシュ ボードの問題**問題の種類の**アプリの送信**、およびサブカテゴリ**その他**] を選びます。 [機能の使用方法と、その製品の必要がある理由を説明します。 必要情報がすべて記載されていない場合、要求が拒否されます。 また別途、追加情報の提供を求められることがあります。 このプロセスには通常 5 営業日以上かかるため、十分前もってリクエストを送信してください。
>
> 承認を要求するには、この方法を使用することも (送信中にこの情報を提供します)] ではなくかどうかを使っている開発サンド ボックスを開始する前に、制限付き機能を使用する承認されていることを確認する場合、送信します。
...
| **完全信頼のアクセス許可レベル** | **RunFullTrust**が制限されている機能は、アプリをユーザーのコンピューター上の完全信頼のアクセス許可レベルで実行できます。 [FullTrustProcessLauncher](https://docs.microsoft.com/uwp/api/windows.applicationmodel.fulltrustprocesslauncher) API を使用するには、この機能が必要です。<br /><br />この機能は、いずれかのデスクトップ アプリケーションを appx として配信されるに必要なもパッケージ化する ([デスクトップ ブリッジ](https://developer.microsoft.com/windows/bridges/desktop)) と同様、デスクトップ アプリ コンバーター (DAC) または Visual を使用して、これらのアプリをパッケージ化するときに自動的に、マニフェストで表示してされますStudio します。 |
| **昇格** | **AllowElevation**が制限されている機能には、アプリを起動またはアプリの有効期間中に自動昇格が必要な既存のデスクトップの機能を保持するには、マイクロソフトのパートナーや企業によって作成されたことができます。<br/><br/>Microsoft Store に提出するアプリでこの機能を宣言することはお勧めしません。 ほとんどの開発者の方は、この機能の使用が承認されません。 展開済みで、企業向け Microsoft ストア経由でプライベート ストアで基幹業務アプリケーションのみ承認されます。  |
| **Windows デバイス資格情報をチーム** | **TeamEditionDeviceCredentials**が制限されている機能は、アプリを Windows 10、1703 以降のバージョンを実行している Surface ハブ デバイスでデバイス アカウントの資格情報を要求する Api にアクセスできます。<br/><br/>Microsoft Store に提出するアプリでこの機能を宣言することはお勧めしません。 ほとんどの開発者の方は、この機能の使用が承認されません。 |
| **Windows チーム アプリケーションの表示** | **TeamEditionView**が制限されている機能は、Windows 10、1703 以降のバージョンを実行している Surface ハブ デバイスでのアプリケーションのビューをホストする Api にアクセスするアプリできます。<br/><br/>Microsoft Store に提出するアプリでこの機能を宣言することはお勧めしません。 ほとんどの開発者の方は、この機能の使用が承認されません。 |
