...
このパッケージのレイアウトは、x64、1 つのフラット アプリ バンドルのファイルを作成アーキテクチャ パッケージと、"Media"資産パッケージ。 
...
**PrebuiltPackage**またはビルドするアプリのバンドル ファイルで参照されているパッケージのレイアウトで定義されていないパッケージを追加する要素を使用します。 この場合、別の DLC 省略可能なパッケージに含まれるここでプライマリ アプリ バンドルのファイルがそれを参照して、関連する一連の一部にすることがあるようにします。
