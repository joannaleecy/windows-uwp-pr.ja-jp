...
<span data-ttu-id="ba017-112">アプリのサイドローディングは、この機能を構成する重要な要素です。</span><span class="sxs-lookup"><span data-stu-id="ba017-112">Side-loading of the app is a critical component of this feature.</span></span>
...
**<span data-ttu-id="ba017-129">デスクトップ コンポーネント</span><span class="sxs-lookup"><span data-stu-id="ba017-129">Desktop component</span></span>**
...
><span data-ttu-id="ba017-145">**注:** サイドローディングを設定で有効にできる&gt;更新とセキュリティ&gt;開発者向けです。</span><span class="sxs-lookup"><span data-stu-id="ba017-145">**Note**Side-loading can be turned on in Settings-&gt; Update & security -&gt; For developers.</span></span>
...
<span data-ttu-id="ba017-164">コントラクトを定義する流れは次のようになります。</span><span class="sxs-lookup"><span data-stu-id="ba017-164">The sequence to define the contract is introduced as following:</span></span>
...
-   <span data-ttu-id="ba017-231">Windows ランタイムの非同期操作</span><span class="sxs-lookup"><span data-stu-id="ba017-231">Windows Runtime Async operations</span></span>
...
**<span data-ttu-id="ba017-248">サーバーにおける Windows ランタイムの非同期処理</span><span class="sxs-lookup"><span data-stu-id="ba017-248">Windows Runtime async in the server</span></span>**
...
<span data-ttu-id="ba017-261">この非同期メソッドのクライアントは、他の Windows ランタイム非同期操作と同じようにこの操作を待つことができます。</span><span class="sxs-lookup"><span data-stu-id="ba017-261">Clients of this async method can await this operation like any other Windows Runtime aysnc operation.</span></span>
...
<span data-ttu-id="ba017-274">次の手順については、サーバー コンポーネントは**MyWinRTComponent**と呼ばれるものとします。</span><span class="sxs-lookup"><span data-stu-id="ba017-274">For the steps below, we assume the server component is called**MyWinRTComponent**.</span></span>
...
**<span data-ttu-id="ba017-291">[構成プロパティ] > [全般] > [ターゲット名]</span><span class="sxs-lookup"><span data-stu-id="ba017-291">Comfiguration Properties > General > Target Name :</span></span>**
...
*<span data-ttu-id="ba017-309">icacls . </span><span class="sxs-lookup"><span data-stu-id="ba017-309">icacls .</span></span> <span data-ttu-id="ba017-310">/T /grant \*S-1-15-2-1:RX</span><span class="sxs-lookup"><span data-stu-id="ba017-310">/T /grant \*S-1-15-2-1:RX</span></span>*
...
<span data-ttu-id="ba017-339">サーバーを変更するときは、以前に実行されていたインスタンスがいずれも実行されていないことを確認することが必要です。</span><span class="sxs-lookup"><span data-stu-id="ba017-339">When you make changes to the server, it is necessary to make sure any previously running instances are no longer running.</span></span> <span data-ttu-id="ba017-340">COM は、最終的にはプロセスを清掃しますが、タイマーによる処理には時間がかかり、反復的な開発では効率的ではありません。</span><span class="sxs-lookup"><span data-stu-id="ba017-340">COM will eventually scavenge the process, but the rundown timer takes longer than is efficient for iterative development.</span></span> <span data-ttu-id="ba017-341">そのため、前に実行されているインスタンスを強制終了することは、開発中に通常の手順です。</span><span class="sxs-lookup"><span data-stu-id="ba017-341">Thus, killing a previously running instance is a normal step during development.</span></span> <span data-ttu-id="ba017-342">これには、どの dllhost のインスタンスがサーバーをホストしているかを開発者が追跡する必要があります。</span><span class="sxs-lookup"><span data-stu-id="ba017-342">This requires that the developer keep track of which dllhost instance is hosting the server.</span></span>

<span data-ttu-id="ba017-343">サーバー プロセスを見つけて強制終了するには、タスク マネージャーやその他のサード パーティ アプリを使うことができます。</span><span class="sxs-lookup"><span data-stu-id="ba017-343">The server process can be found and killed using Task Manager or other third party apps.</span></span> <span data-ttu-id="ba017-344">**TaskList.exe**コマンド ライン ツールも含まれていると、柔軟な構文では、たとえば。</span><span class="sxs-lookup"><span data-stu-id="ba017-344">The command line tool**TaskList.exe**is also included and has flexible syntax, for example:</span></span>

  
 | **<span data-ttu-id="ba017-345">コマンド</span><span class="sxs-lookup"><span data-stu-id="ba017-345">Command</span></span>** | **<span data-ttu-id="ba017-346">アクション</span><span class="sxs-lookup"><span data-stu-id="ba017-346">Action</span></span>** |
 | ------------| ---------- |
 | <span data-ttu-id="ba017-347">tasklist</span><span class="sxs-lookup"><span data-stu-id="ba017-347">tasklist</span></span> | <span data-ttu-id="ba017-348">実行中のすべてのプロセスをほぼ作成時刻の順序で一覧表示します。最後に作成されたプロセスは下の方に表示されます。</span><span class="sxs-lookup"><span data-stu-id="ba017-348">Lists all the running processes in approximate order of creation time, with the most recently created processes near the bottom.</span></span> |
 | <span data-ttu-id="ba017-349">tasklist /FI "IMAGENAME eq dllhost.exe" /M</span><span class="sxs-lookup"><span data-stu-id="ba017-349">tasklist /FI "IMAGENAME eq dllhost.exe" /M</span></span> | <span data-ttu-id="ba017-350">すべての dllhost.exe インスタンスの情報を一覧表示します。</span><span class="sxs-lookup"><span data-stu-id="ba017-350">Lists info on all the dllhost.exe instances.</span></span> <span data-ttu-id="ba017-351">/M スイッチは、読み込み済みのモジュールを一覧表示します。</span><span class="sxs-lookup"><span data-stu-id="ba017-351">The /M switch lists the modules that they have loaded.</span></span> |
 | <span data-ttu-id="ba017-352">tasklist /FI "PID eq 12564" /M</span><span class="sxs-lookup"><span data-stu-id="ba017-352">tasklist /FI "PID eq 12564" /M</span></span> | <span data-ttu-id="ba017-353">PID がわかっている場合に dllhost.exe を照会するには、このオプションを使用できます。</span><span class="sxs-lookup"><span data-stu-id="ba017-353">You can use this option to query the dllhost.exe if you know its PID.</span></span> |

<span data-ttu-id="ba017-354">ブローカー サーバーのモジュールの一覧が*clrhost.dll*を一覧表示する必要があります読み込まれたモジュールの一覧にします。</span><span class="sxs-lookup"><span data-stu-id="ba017-354">The module list for a broker server should list*clrhost.dll*in its list of loaded modules.</span></span>

## <a name="resources"></a><span data-ttu-id="ba017-355">リソース</span><span class="sxs-lookup"><span data-stu-id="ba017-355">Resources</span></span>

-   [<span data-ttu-id="ba017-356">Windows 10 および VS 2015 用 WinRT コンポーネント ブローカー プロジェクト テンプレート</span><span class="sxs-lookup"><span data-stu-id="ba017-356">Brokered WinRT Component Project Templates for Windows 10 and VS 2015</span></span>](https://visualstudiogallery.msdn.microsoft.com/10be07b3-67ef-4e02-9243-01b78cd27935)

-   [<span data-ttu-id="ba017-357">NorthwindRT WinRT コンポーネント ブローカー サンプル</span><span class="sxs-lookup"><span data-stu-id="ba017-357">NorthwindRT Brokered WinRT Component Sample</span></span>](http://go.microsoft.com/fwlink/p/?LinkID=397349)

-   [<span data-ttu-id="ba017-358">信頼性の高い Microsoft Store アプリの提供</span><span class="sxs-lookup"><span data-stu-id="ba017-358">Delivering reliable and trustworthy Microsoft Store apps</span></span>](http://go.microsoft.com/fwlink/p/?LinkID=393644)

-   [<span data-ttu-id="ba017-359">アプリ コントラクトと拡張機能 (Microsoft Store アプリ)</span><span class="sxs-lookup"><span data-stu-id="ba017-359">App contracts and extensions (Windows Store apps)</span></span>](https://msdn.microsoft.com/library/windows/apps/hh464906.aspx)

-   [<span data-ttu-id="ba017-360">Windows 10 でアプリをサイドロードする方法</span><span class="sxs-lookup"><span data-stu-id="ba017-360">How to sideload apps on Windows 10</span></span>](https://msdn.microsoft.com/windows/uwp/get-started/enable-your-device-for-development#GroupPolicy)

-   [<span data-ttu-id="ba017-361">企業への UWP アプリの展開</span><span class="sxs-lookup"><span data-stu-id="ba017-361">Deploying UWP apps to businesses</span></span>](http://go.microsoft.com/fwlink/p/?LinkID=264770)
