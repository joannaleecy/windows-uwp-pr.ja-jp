...
[MIDL 3.0](/uwp/midl-3/)で読み取り専用プロパティを宣言するとき、期待どおり、`cppwinrt.exe`されるツールの実装の機能を生成する`const`-修飾 (const 関数は、定数として*この*ポインターを扱う)。

可能であれば、定数を使用してを確実にお勧めしますが、`cppwinrt.exe`のどの実装について関数が考え const、ありする可能性がある理由ツール自体は行われません。 この例のように、const 任意の実装関数のすることができます。
...
削除することができます`const` **ToString**の修飾子すべき実装ではいくつかオブジェクトの状態を変更する必要があることを決定します。 ただし、メンバーの各両方関数 const または非定数のいずれか。 つまり、しないオーバー ロードの実装の機能に`const`します。

Const 場所に配置他の別の実装の関数を除くが使えるように画像は、Windows ランタイム関数のプロジェクションにします。 このコードを検討してください。
...
投影の関数は、資格を得ることの実装を選択する方法に関係なく const。 バック グラウンドで、プロジェクションは、アプリケーション バイナリ インターフェイス (ABI)、COM インターフェイス ポインターを使用して、呼び出しにどの金額を呼び出します。 投影された**ToString**と対話する唯一の状態は、COM インターフェイス ポインターです。その確実に不要になったため、関数は const そのポインターを変更します。 これにより、 **IStringable**を参照する保証を通じてを呼び出している**IStringable**の参照について何も変わりませんことと、これにより、const しても**ToString**を呼び出すことができます。

理解これらの例の`const`は、C++ の実装の詳細/WinRT プロジェクションおよび実装します。これらは、しやすくするコードに対する予防措置を構成します。 このようなものはありません`const`COM も (メンバー関数) 用の Windows ランタイム ABI [します。
...
Windows ランタイム オブジェクトを使用する際は、以下を参照できるので否定的な影響をアプリケーションに以上バイナリのコードを生成するために必要なことで、コーディング パターンを避ける必要があります。
...
Windows ランタイムの世界でコンパイラことができないの値をキャッシュする`c()`または間接参照から呼び出されるメソッドでは各インターフェイス ('. ')。 介入する場合を除き、複数の仮想呼び出しと参照カウントのオーバーヘッドで結果します。 上記のパターンでは、として厳密に必要な 2 回の多くのコードを簡単に作成可能性があります。 代わりに、以下を参照することができる場合は必ず、パターンのことを希望します。 少量のコードを生成し、実行時のパフォーマンスも大幅に向上させることができます。
