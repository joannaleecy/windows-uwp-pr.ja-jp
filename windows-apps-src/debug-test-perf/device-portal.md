...
3. ブラウザーでデバイス ポータルのページに移動します。 次の表は、ポートと各デバイス ファミリで使われるプロトコルを示します。
...
このページを表示して、サイドローディングされたアプリによって保存されたファイルを操作できます。 投稿するファイル エクスプ ローラーとその使用方法について詳しくは、[アプリのエクスプ ローラーを使って](https://blogs.windows.com/buildingapps/2016/06/08/using-the-app-file-explorer-to-see-your-app-data/)ブログをご覧ください。 
...
ETW ログ] ページでは、リアルタイム イベント Windows トレーシング (ETW)、デバイスの情報を管理します。
...
- **登録済みプロバイダー**: イベント プロバイダーとトレース レベルを選択します。 トレース レベルでは、これらの値のいずれかです。
  1. 異常終了または終了
  2. 重大なエラー
  3. 警告
  4. エラーではない警告
...
- **フィルター/イベント**:**イベント**] セクションの表形式で選択したプロバイダーからの ETW イベントの一覧を示します。 テーブルは、リアルタイムで更新されます。 **フィルターを使って**メニューを使用して、イベントが表示されるカスタム フィルターを設定します。 すべての ETW イベントを表から削除する**クリア**] ボタンをクリックします。 これによってプロバイダーが無効になることはありません。 現在収集されている ETW イベントをローカルの CSV ファイルにエクスポートする**ファイルを保存する**をクリックすることができます。
...
- **IP 構成**: ホストの各アドレスについてのデバイスのネットワーク ポートを示しています。
...
> [!IMPORTANT]
> この保護は、スタンドアロン クライアント (など、コマンド ライン ユーティリティなど) から REST Api の使用法を防ぎます。 これは 3 つの方法で解決できます。 
