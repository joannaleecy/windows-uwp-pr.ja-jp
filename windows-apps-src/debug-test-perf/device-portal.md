...
- **インストール済みアプリ**: ドロップダウン メニューを使用して削除したり、デバイスにインストールされているアプリを起動します。 **追加**] をクリックして、新しいアプリをインストールします。 ローカルからのパッケージ化されたアプリを展開する UX のインストールが開始、ネットワークまたは web をホストし、ネットワーク共有からルーズ ファイルを登録します。
...
2.  Device Portal のアプリ マネージャー] セクションでは、**追加**] をクリックし、**ローカル記憶域からアプリ パッケージのインストール**を選択します。
...
このページを表示して、サイドローディングされたアプリによって保存されたファイルを操作できます。 投稿する、エクスプ ローラーとその使用方法について詳しくは、[アプリのエクスプ ローラーを使って](https://blogs.windows.com/buildingapps/2016/06/08/using-the-app-file-explorer-to-see-your-app-data/)ブログをご覧ください。 
...
- **フィルターを使って/イベント**: [**イベント**] セクションの表形式で選択したプロバイダーからの ETW イベントの一覧を示します。 テーブルはリアルタイムで更新されます。 **フィルターを使って**メニューを使用して、イベントが表示されるカスタム フィルターを設定します。 すべての ETW イベントを表から削除する**クリア**] ボタンをクリックします。 これによってプロバイダーが無効になることはありません。 現在収集されている ETW イベントをローカルの CSV ファイルにエクスポートする**ファイルを保存する**をクリックすることができます。
...
トレースを停止するには、**[Stop]** (停止) をクリックします。 このページで、目的のトレース ファイルに保持される (します。ETL) のダウンロードが完了します。
...
> [!IMPORTANT]
> この保護は、スタンドアロン クライアント (など、コマンド ライン ユーティリティなど) から REST Api を使用できないようにします。 これは 3 つの方法で解決できます。 
