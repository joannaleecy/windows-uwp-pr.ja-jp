...
Windows Device Portal は、PC 上の web ブラウザーから接続できるデバイスで web サーバーです。 デバイスの web ブラウザーである場合、またローカルにそのデバイスのブラウザーで接続できます。
...
- **インストール済みアプリ**: ドロップダウン メニューを使用して削除したり、デバイスにインストールされているアプリを起動します。 **追加**] をクリックして新しいアプリをインストールします。 ローカルからのパッケージ化されたアプリを展開する UX のインストールが開始、ネットワークまたは web をホストし、ネットワーク共有からルーズ ファイルを登録します。
...
2.  Device Portal のアプリ マネージャー] セクションでは、**追加**] をクリックし、**ローカル記憶域からのアプリ パッケージのインストール**を選択します。
...
2.  場合は、**アプリの実行**に移動して閉じます。 アプリの実行中にアンインストールするしようとすると、アプリを再インストールしようとしたときに問題が発生されます。 
3.  ドロップダウン リストからアプリを選択し、[**削除**] をクリックします。

### <a name="running-processes"></a>実行中のプロセス

このページでは、ホスト デバイスで実行中のプロセスに関する詳細を表示します。 これには、アプリとシステムの両方のプロセスが含まれます。 (デスクトップ、IoT、および HoloLens) 一部のプラットフォームでは、プロセスを終了できます。
...
- **メモリ**: 利用可能なコミット、ページ、および非ページは、使用中の合計
- **I/O**: 読み取りと書き込みのデータ量
- **ネットワーク**: 受信し、データの送信
...
- **フィルター/イベント**:**イベント**] セクションの表の形式で選択したプロバイダーからの ETW イベントの一覧を示します。 テーブルは、リアルタイムで更新されます。 **フィルターを使って**メニューを使用して、イベントが表示されるカスタム フィルターを設定します。 すべての ETW イベントを表から削除する**クリア**] ボタンをクリックします。 これによってプロバイダーが無効になることはありません。 現在収集されている ETW イベントをローカルの CSV ファイルにエクスポートするのには**ファイルを保存する**をクリックすることができます。
...
トレースを停止するには、**[Stop]** (停止) をクリックします。 このページで、目的のトレース ファイルに保持される (します。ETL) のダウンロードが完了するとします。
...
- **利用可能なネットワーク**: デバイスに利用可能な WiFi ネットワークを示しています。 ネットワークをクリックまたはタップすると、そのネットワークに接続し、必要に応じてパスキーを提供できます。 まだ、device Portal では、エンタープライズ認証をサポートしていません。 既知のデバイスに WiFi プロファイルのいずれかに接続しようとするのに**プロファイル**ドロップダウン リストを使用することもできます。
...
> [!IMPORTANT]
> この保護では、スタンドアロン クライアント (コマンド ライン ユーティリティ) などから REST Api を使用できないようにします。 これは 3 つの方法で解決できます。 
