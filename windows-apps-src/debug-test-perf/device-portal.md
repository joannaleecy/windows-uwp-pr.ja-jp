...
このページを表示して、サイドローディングされたアプリによって保存されたファイルを操作できます。 投稿については、エクスプ ローラーとその使用方法について詳しくは、[アプリのエクスプ ローラーを使って](https://blogs.windows.com/buildingapps/2016/06/08/using-the-app-file-explorer-to-see-your-app-data/)ブログをご覧ください。 
...
- **フィルター/イベント**: [**イベント**] セクションの表の形式で選択したプロバイダーからの ETW イベントの一覧を示します。 テーブルは、リアルタイムで更新されます。 イベントが表示されるカスタム フィルターを設定するには、**フィルターを使って**メニューを使用します。 すべての ETW イベントを表から削除する**クリア**] ボタンをクリックします。 これによってプロバイダーが無効になることはありません。 現在収集されている ETW イベントをローカルの CSV ファイルにエクスポートする**ファイルを保存する**をクリックすることができます。
...
- **IP 構成**: デバイスのネットワーク ポート、ホストの各に関するのアドレス情報が表示されます。
