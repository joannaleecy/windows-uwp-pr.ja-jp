...
> **重要な**タップ ツールを初めて起動すると、ことは Windows ファイアウォールのプロンプトが表示されます。 3 つのチェック ボックスをすべてオンにし、ツールがファイアウォールを通過できるようにする必要があります。そうしないと、ツールが動作せず、通知も表示されません。
...
> **注:** 電話に使われるため SD カードは FAT32 ファイル システムでフォーマットされて、最大のサイズは 32 GB です。
...
Windows 10 の新しいフライトへの更新後など、特定の状況で、エミュレーターに関連付けられている仮想ネットワーク スイッチは、ユーザー インターフェイスを使って削除できない状態に取得できます。
...
**注:** このコマンドは、エミュレーターに関連付けられているだけでなく、すべてのネットワーク デバイスを削除します。 使用しているコンピューターを再度起動すると、すべてのハードウェア ネットワーク デバイスが自動的に検出されます。
 
...
> **注:** XDECleanup.exe すべてエミュレーターを削除する特定の HYPER-V Vm、VM のチェックポイントを削除やも保存された状態。
