...
**注:** これらを c# または Visual Basic から使うには、相互運用性の境界を越えるために、windows 10 に付属している UWP コンポーネントのほとんどは C++ で実装されています。 通常の場合と同様に、アプリのパフォーマンスに UWP コンポーネントが影響を与えているかどうかを調査した後で、コードの変更に注力してください。
