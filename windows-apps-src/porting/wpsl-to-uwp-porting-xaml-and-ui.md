...
命令型コードの修正がパラメーターの型の変更のみになることもあります。 それ以外の場合、.NET Api の Windows ランタイム 8.x アプリではなく UWP Api を使用する必要があります。 特定するには、どの Api がサポートされて、 [.net for Windows の 8.x アプリの概要](https://msdn.microsoft.com/library/windows/apps/xaml/br230302.aspx)と、 [Windows ランタイム リファレンス](https://msdn.microsoft.com/library/windows/apps/br211377)と組み合わせて移植このガイドの残りの部分を使用します。
...
| Panorama | Windows Phone Silverlight パノラマ コントロールは、ハブ コントロールの[Windows ランタイム 8.x アプリでハブ コントロールするためのガイドライン](https://msdn.microsoft.com/library/windows/apps/dn449149)とガイドラインにマップします。 <br/> Panorama コントロールは、最後のセクションから最初のセクションに折り返され、背景画像はセクションを基準とした視差効果で移動します。 [Hub](https://msdn.microsoft.com/library/windows/apps/dn251843) セクションは折り返されず、視差効果は使われません。 |
...
淡色テーマと濃色テーマのビットマップ形式に違いがある場合に適切であるより一般的なアプローチは、2 つの画像アセット (一方が淡色テーマ用の濃色の前景、他方が濃色テーマ用の淡色の前景) を使うことです。 この一連のビットマップのリソース ファイルの名前を付ける方法の詳細については、[言語、およびその他の区切り記号のリソースの調整](../app-resources/tailor-resources-lang-scale-contrast.md)を参照してください。 イメージ ファイル セットに正しい名前を付けた後、次のようにしてルート名を使って抽象内で参照できます。
