...
Arm 版 Windows 10 では、Microsoft Store から ARM32 と ARM64 [UWP アプリ](../get-started/universal-application-platform-guide.md)のすべての x86 を実行します。 ARM32 と ARM64 のアプリではエミュレーションが任意にネイティブで実行中、x86 アプリのエミュレーション下で実行します。 UWP 開発者の場合、デバイスの最適なユーザー エクスペリエンスを提供するため、必ずアプリの ARM パッケージを提出してください。 詳しくは、「[アプリ パッケージのアーキテクチャ](../packaging/device-architecture.md)」をご覧ください。
...
UWP アプリに加えて、arm 版 Windows 10 も実行できる x86 Win32 アプリ (Adobe Reader) などの変更せずに適切なパフォーマンスとシームレスなユーザー エクスペリエンスと同様に任意の PC。 ARM プロセッサで実行されているこれらの x86 Win32 アプリが ARM 用に再コンパイルする必要はなく、気付くしないでください。 64 ビット x64 Win32 アプリはサポートされていませんが、ほぼすべてのアプリに x86 バージョンがあるため、ユーザーの観点からは 32 ビット x86 インストーラーを選ぶだけで ARM PC の Windows で実行できます。
...
|[ARM で ARM アプリのトラブルシューティング](apps-on-arm-troubleshooting-arm32.md)|ARM32 アプリと ARM64 アプリ ARM とその解決方法で実行されているときの一般的な問題です。 |
