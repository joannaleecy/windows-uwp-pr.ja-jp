...
> [!WARNING]
> 任意の文字列識別子は、ここで指定した名前を選択した言語のプログラミングで有効な通常はする XAML でも、依存関係プロパティを設定することができます。 XAML で設定するには、選ぶプロパティが有効な XAML 名である必要があります。 詳しくは、「[XAML の概要](xaml-overview.md)」をご覧ください。
