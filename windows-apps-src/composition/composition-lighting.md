...
- 音楽再生シーンのようなイマーシブのシナリオを実現するシーン内の他のオブジェクトのライトの独立したを変換します。
- 一緒に移動できるように、ライトを使ってオブジェクトをペアリングする機能の Fluent[表示](/design/style/reveal.md)ハイライトなどのシナリオを実現する、シーンの残りの部分とは無関係です。
...
ポイント ライトのオフセットをアニメーションを追加することで、光る効果は簡単に実現されます。
...
> [!NOTE]
> 複数の照明同じのビジュアルにヒットしたとき、または光の色の値が 1.0 を超過する十分な大きさに取得されるたびには、ライトのカラー チャネルのクランプがあるため、光の色が変更できます。
...
- 作成し、ライトを配置します。 ライトを作成し、指定された座標空間に配置します。
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect)は、素材の作成によく使用されます。 SceneLightingEffect は、イメージの反射プロパティを有効にするや、法線マップを使って、奥行き感を提供するようより複雑なものを実現するために必要なときに使用する効果です。 SceneLightingEffect 反射および拡散金額などの光源のプロパティを使用して、UI をカスタマイズする機能を提供します。 個別にブレンドし、コンテンツを含むさまざまな照明反応できる効果パイプラインの残りの部分で照明効果をカスタマイズすることができます。
