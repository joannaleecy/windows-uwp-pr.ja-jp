...
この例では、対象となる Uielement とブラシにマルチカラー スポット ライトを適用するカスタム XamlLight の定義を示します。
...
簡易にするには、XAML UIElement またはブラシにこの light を適用することができます。 この例では、さまざまな潜在的な用法を検出を使用します。

> [!Important]
> [C + +/WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt)] の 2 つのアイテムを削除する`local:OrangeSpotLight.IsTarget="True"`次の変更履歴とコメントからします。 接続されているプロパティは、分離コードで既に設定されています。
