...
* **サービス**です。 これはお客様のビジネス環境のコンテキストで安全に実行されるアプリケーションであり、選択した任意の開発プラットフォームを使用して実装できます。 サービスは、Azure AD アクセス トークンの作成に、必要なシナリオでは、Microsoft Store コレクションの残りの部分の Uri を呼び出す API および購入 API の担当します。
...
1.  まだ完了していないしたら場合で[Azure Active Directory とアプリケーションの統合](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications)を登録の指示に従って、 **Web アプリ/API**と Azure AD アプリケーション。
    > [!NOTE]
...
4.  後で必要なキーを作成 (これはすべてと呼ばれる*クライアント シークレット*)。 左側のウィンドウで**設定**し、**キー**をクリックします。 このページでは、[キーを作成](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications#to-add-application-credentials-or-permissions-to-access-web-apis)する手順を完了します。 後で使うには、このキーをコピーします。
