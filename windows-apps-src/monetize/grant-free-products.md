...
ms.prod: windows
ms.technology: uwp
keywords: "Windows 10, UWP, Windows ストア購入 API, 製品の付与"
...
* 付与する無料製品のユーザーの ID を表す Windows ストア ID キー。
...
|----------------|--------|---------------------|----------|
| availabilityId | string | Windows ストア カタログから付与される製品の可用性 ID。         | 必須      |
| b2bKey         | string | 付与する製品のユーザーの ID を表す [Windows ストア ID キー](view-and-grant-products-from-a-service.md#step-4)。    | 必須      |
...
| quantity       | int    | 購入する数量。 現時点では、サポートされている唯一の値は 1 です。 指定されていない場合、既定値は 1 です。   | 必須ではない       |
...
| productId               | string         | Windows ストア カタログでの行項目を表す[製品](in-app-purchases-and-trials.md#products-skus-and-availabilities)の[ストア ID](in-app-purchases-and-trials.md#store-ids) です。 製品のストア ID の例は、9NBLGGH42CFD です。   | 必須      |
...
| skuId                   | string         | Windows ストア カタログ内の行項目の [SKU](in-app-purchases-and-trials.md#products-skus-and-availabilities) の[ストア ID](in-app-purchases-and-trials.md#store-ids) です。 SKU のストア ID の例は、0010 です。                                                                   | 必須      |
