...
お客様の観点からは、アプリを 1 つを使用します。 区別できるように、2 つのモードをお勧めしますリテール モードでは、アプリが、その表示される単語「市販」目立つようにタイトル バーなどの適切な場所です。
...
* **メリットを提示**します。 ショーケース アプリ岩理由を市販デモ エクスペリエンスを使用します。 これは、最初に、お客様には、最適な部分を表示するため、アプリが表示されます可能性が高いです。
...
* **オフライン エクスペリエンス Memorable**します。 アプリは、製品版の場所に、デバイスの約 50% がオフラインに優れたオフライン エクスペリエンスを示す必要があります。 この要件は、お客様がオフラインでアプリを操作する場合でも、意味のある肯定的なエクスペリエンスを保証することを目的としています。
...
* **クリーンアップ処理を使用して、一貫したエクスペリエンスを提供**します。 市販デモ デバイスは、使用開始にあたってすべてのお客様に同じエクスペリエンスを提供する必要があります。 アプリは、[クリーンアップ処理](#clean-up-process)を使用する必要があります、使用後、同じ既定の状態に戻ります。 次のお客様にどのような最後のお客様がたいしないでください。 これには、スコアボード、達成度、ロック解除が含まれます。

* **年齢に応じた適切なコンテンツ**です。 アプリのすべてのコンテンツがある必要があります、ティーン区分します。 詳細については、[アプリが iarc 評価取得](https://www.globalratings.com/for-developers.aspx)と[ESRB 評価に関するページ](https://www.esrb.org/ratings/ratings_guide.aspx)を参照してください。
...
クリーンアップ、買い物では、デバイスとのやり取りが停止したら、2 分が開始されます。 市販デモを再生し、Windows は、連絡先、写真、および他のアプリのサンプル データがすべてのリセットを開始します。 デバイスに応じてが完全にすべての情報を正常にリセットする 1 ~ 5 分間かかります。 これにより、小売店でのすべてのユーザーがデバイスに手順について説明し、デバイスを操作するとき、同じエクスペリエンスがあることができます。
...
カスタムのクリーンアップ ロジックが必要なシナリオには、データをキャッシュにダウンロードしたりしない*LocalState*データを削除しようとしている、広範なセットアップの実行が含まれています。
