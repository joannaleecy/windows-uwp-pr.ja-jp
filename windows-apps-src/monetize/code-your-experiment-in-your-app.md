...
> [!NOTE]
> 実験では、Microsoft Store Services SDK Api のいくつかは、デベロッパー センターからデータを取得する[の非同期パターン](../threading-async/asynchronous-programming-universal-windows-platform-apps.md)を使います。 つまり、これらのメソッドの実行の一部可能性があります場所後、メソッドが呼び出されると、操作が完了するまで、アプリの UI を応答性の高い維持できるようにします。 非同期パターンでは、この記事のコード例で示されていると、Api を呼び出すときに、**非同期**キーワードと**await**演算子を使用するアプリが必要です。 慣例により、非同期メソッドを**非同期**で終了します。
...
必要な特定のコードは、アプリによって異なりますが、次の例は、基本的なプロセスを示しています。 完全なコード例を参照してください。[を作成および実行を使用して最初の実験 B テスト/](create-and-run-your-first-experiment-with-a-b-testing.md)します。
...
2. [プロジェクト ID](run-app-experiments-with-a-b-testing.md#terms)を取得する実験に割り当てられている文字列変数を宣言します。
    > [!NOTE]
    > プロジェクトを取得するときの ID [、デベロッパー センター ダッシュ ボードでプロジェクトを作成](create-a-project-and-define-remote-variables-in-the-dev-center-dashboard.md)します。 次に示すプロジェクト ID は、目的にのみたとえばです。
...
A[コンバージョン イベント](run-app-experiments-with-a-b-testing.md#terms)をログに記録するコードを次に、追加 B は、デベロッパー センターでサービスをテストします。 コードでは、ユーザーが実験の目標に達したときは、コンバージョン イベントを記録する必要があります。 必要な特定のコードは、アプリによって異なりますが、一般的な手順を以下に示します。 完全なコード例を参照してください。[を作成および実行を使用して最初の実験 B テスト/](create-and-run-your-first-experiment-with-a-b-testing.md)します。
