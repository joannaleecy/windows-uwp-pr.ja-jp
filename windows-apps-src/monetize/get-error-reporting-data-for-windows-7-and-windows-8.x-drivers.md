...
description: "特定の日付範囲などのオプション フィルターを使って Windows 7 や Windows 8.x のドライバーに関する集計エラー報告データを取得するには、Windows ストア分析 API に含まれる以下のメソッドを使用します。 このメソッドは、IHV のみを対象としています。"
...
特定の日付範囲などのオプション フィルターを使って Windows 7/Windows 8.x のドライバー エラーに関する集計報告データを取得するには、Windows ストア分析 API に含まれる以下のメソッドを使用します。 また、[Windows 7 や Windows 8.x のドライバー エラーに関する詳細を取得する](get-details-for-a-windows-7-or-windows-8.x-driver-error.md)メソッドを利用すれば、追加のエラー情報を取得することもできます。
