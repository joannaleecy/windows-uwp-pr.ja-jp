...
description: "特定の日付範囲などのオプション フィルターを使って集計ハードウェア エラー報告データを取得するには、Windows ストア分析 API に含まれる以下のメソッドを使用します。 このメソッドは、OEM のみを対象としています。"
...
特定の日付範囲などのオプション フィルターを使って OEM ハードウェア エラーの集計報告データを取得するには、Windows ストア分析 API に含まれる以下のメソッドを使用します。 また、[OEM ハードウェア エラーの詳細を取得する](get-details-for-an-oem-hardware-error.md)メソッドを利用すれば、追加のエラー情報を取得することもできます。
