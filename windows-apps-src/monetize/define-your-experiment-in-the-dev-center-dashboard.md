...
したら[プロジェクトを作成し、パートナー センターでリモート変数定義](create-a-project-and-define-remote-variables-in-the-dev-center-dashboard.md)と[実験用のアプリのコード](code-your-experiment-in-your-app.md)プロジェクトで実験を作成する準備が整いました。 実験を作成するときは、ゴールとユーザーが受け取るバリエーションを定義します。
...
8.   [プロジェクトID](run-app-experiments-with-a-b-testing.md#terms) の値をメモしておきます。 ときにする[実験用のアプリをコード](code-your-experiment-in-your-app.md)、バリエーションの 1 つのデータを受信し、パートナー センターに表示し、変換のイベントを報告できるように、コードでこの ID を参照する必要があります。
...
> [!NOTE]
> パートナー センターでは、24 時間の期間には、各ユーザー ビューの最初の変換イベントのみを報告します。 ユーザーが 24 時間以内にアプリで複数のコンバージョン イベントをトリガーした場合は、最初のコンバージョン イベントのみ報告されます。 これは、コンバージョンを行ったユーザーの数を最大化することを目標としていた場合に、単一のユーザーによって、サンプルのユーザー グループの試験的機能の結果が歪曲されないようにすることを目的としています。
...
1. **リモート変数とバリエーション** セクションで、2 つの既定のバリエーションを確認する必要があります**バリエーション (コントロール)** と**バリエーションの 1 つの B**します。複数のバリエーションを実行する場合に、クリックして**バリエーションの 1 つの追加**します。 必要に応じて、各バリエーションの名前を変更することができます。
...
試験的機能のパラメーターに問題がなく、試験的機能をアクティブ化する準備ができたら、**[アクティブ化]** をクリックして、アプリから試験的機能データの収集を開始することができます。 実験がアクティブの場合、アプリはバリエーションの 1 つの変数を取得し、パートナー センターに表示し、変換のイベントを報告できます。 詳細については、次を参照してください。[実行し、パートナー センターで実験を管理](manage-your-experiment.md)します。
...
準備ができたら、実験をパートナー センターと、実験のコードで、アプリで定義した後[実行し、パートナー センターで実験を管理](manage-your-experiment.md)します。
