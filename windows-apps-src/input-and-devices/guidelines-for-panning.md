...
> </div>
> 既定では、UWP アプリでは、アクティブ ペンは、テキストの選択ではなく、スクロール/パン (タッチ、タッチパッド、パッシブ ペンなどと同様に) をするようになりました。  
...
**パン対応領域** パン対応領域の動作は、JavaScript を使った Windows ストア アプリの開発者に対して、設計時にカスケード スタイル シート (CSS) を通じて公開されます。
