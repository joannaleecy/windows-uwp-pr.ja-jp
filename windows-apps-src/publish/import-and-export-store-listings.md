...
- **[フィールド]** 列には、ストア登録情報のすべての項目に関連付けられている名前が含まれます。 これらは、一部の名前が若干異なりますがパートナー センターで、ストア登録情報を作成するときに提供できるものと同じ項目に対応しています。 同じ種類の項目で複数の情報を入力できるものについては、指定できる最大数以下で、複数の行が含まれます。 たとえば、**[アプリの機能]** には **Feature1**、**Feature2**、～最大で **Feature20** までが含まれます (アプリの機能は最大で 20 個まで指定できるため)。
- **[ID** ] 列には、パートナー センターは、各フィールドに関連付けられている番号が含まれています。 
...
行全体ではなく、特定の言語について任意のフィールドの情報を削除する場合は、フィールドによって動作が異なります。 **[種類]** が **[テキスト]** のフィールドは、任意のフィールドの情報を削除すると、単純にその言語の登録情報から該当するエントリーが削除されます。  ただし、スクリーン ショットやロゴなど、画像のフィールドに情報を削除する、任意の効果以前のイメージがパートナー センターで直接編集して削除する場合を除き、引き続き使用されます。 トレーラーのフィールドの情報を削除する、実際には、パートナー センターからそのトレーラーを削除する、されるため、これを行う前に必要なファイルのコピーがあることを確認します。
...
既にパートナー センターに追加していない新しい資産を使用する場合は、[単一の .csv ファイルではなく、フォルダーとして登録情報をインポートすることによって実行できます。 .csv ファイルを含むフォルダを作成する必要があります。 次に、その同じフォルダーのルート フォルダーまたはサブフォルダーに画像を追加します。 フィールドには、ルート フォルダー名を含む完全なパスを入力する必要があります。
...
> [!IMPORTANT]
> エクスポートした登録情報にトレーラーが含まれている場合、.csv ファイルから、トレーラーやそのサムネイル画像への URL は完全に削除削除されたファイルからパートナー センターとなることができなくにアクセスすることがあります (ano でも使われます。この登録情報の削除されていない)。 
...
None の .csv ファイルからの情報は保存されますパートナー センターでのすべてのファイルでエラーが解決されるまで、エラーなしのフィールドであっても。 エラーがない .csv ファイルをインポートすると、用意した登録情報がパートナー センターで、保存され、その申請に使用されます。

別の更新された .csv ファイルをインポートするか、パートナー センターで直接変更することで、登録情報の更新を行うを続行できます。

## <a name="add-ons"></a>アドオン

アドオンのインポートおよびエクスポートのストア登録情報を使用して、上記で説明した同じプロセスことを除き、[アドオンのストア登録情報](create-add-on-store-listings.md)に関連する 3 つのフィールドにのみ表示されます**説明**、**タイトル**、および**StoreLogo300x300** (。呼ばれるを**アイコン**にパートナー センターでストア登録情報ページ)。 **[タイトル]** フィールドは必須で、その他の 2 フィールドは省略可能です。
