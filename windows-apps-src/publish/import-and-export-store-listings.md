...
ms.locfileid: "7719457"
---
# <a name="import-and-export-store-listings"></a>ストア登録情報のインポートとエクスポート

を[パートナー センターで直接ストア登録情報の情報を入力する](create-app-store-listings.md)のではなくには、追加や情報を更新しても、登録情報を .csv ファイルにエクスポートを情報と資産を入力し、更新したファイルをインポートするオプションがあります。 この方法を使うと、登録情報を最初から作成することも、既に作成している登録情報を更新することもできます。

このオプションは、複数の言語で製品の Store 登録情報を作成または更新する場合に特に便利です。複数のフィールドに同じ情報をコピーして貼り付け、特定の言語に適用される変更を簡単に行うことができます。 ただし、以下のメソッドを使用して、作成したり、以前の OS バージョンをサポートする、以前に公開されたアプリの[プラットフォーム固有のストア登録情報](create-platform-specific-store-listings.md)を更新することはできません。 
...
行全体ではなく、特定の言語について任意のフィールドの情報を削除する場合は、フィールドによって動作が異なります。 **[種類]** が **[テキスト]** のフィールドは、任意のフィールドの情報を削除すると、単純にその言語の登録情報から該当するエントリーが削除されます。  ただし、スクリーン ショットやロゴなど、画像のフィールドで情報を削除する、任意の効果以前のイメージがパートナー センターで直接編集して削除する場合を除き、引き続き使用されます。 トレーラーのフィールドの情報を削除する、実際には、パートナー センターからそのトレーラーを削除する、されるため、これを行う前に必要なファイルのコピーがあることを確認します。
...
アドオンのインポートおよびエクスポートのストア登録情報を使用して、上記と同じプロセスことを除き、[アドオンのストア登録情報](create-add-on-store-listings.md)に関連する 3 つのフィールドにのみ表示されます**説明**、**タイトル**、および**StoreLogo300x300** (。呼ばれるを**アイコン**にパートナー センターでストア登録情報ページ)。 **[タイトル]** フィールドは必須で、その他の 2 フィールドは省略可能です。
