...
ms.locfileid: "7964175"
---
# <a name="import-and-export-store-listings"></a>ストア登録情報のインポートとエクスポート

[パートナー センターで直接ストア登録情報の情報を入力する](create-app-store-listings.md)、代わりを追加しても、登録情報を .csv ファイルにエクスポートを情報と資産を入力し、更新したファイルをインポートして情報を更新またはためのオプションがあります。 この方法を使うと、登録情報を最初から作成することも、既に作成している登録情報を更新することもできます。
...
注意を常に作成するか (インポート/エクスポートの方法を使用した) 場合でも、パートナー センターで直接登録情報を更新してください。 パートナー センターで直接更新は、単純な変更を行うだけですが、いずれかのメソッドを使用するには、いつでも簡単に可能性があります。
...
- **[種類**] 列は、**テキスト**や**相対パス (またはパートナー センターでのファイルへの URL)** など、そのフィールドを提供する情報の種類に関する一般的なガイダンスを提供します。 
...
行全体ではなく、特定の言語について任意のフィールドの情報を削除する場合は、フィールドによって動作が異なります。 **[種類]** が **[テキスト]** のフィールドは、任意のフィールドの情報を削除すると、単純にその言語の登録情報から該当するエントリーが削除されます。  ただし、スクリーン ショットやロゴなど、画像のフィールドに情報を削除する、任意の効果前の画像がパートナー センターで直接編集して削除する場合を除き、引き続き使用されます。 トレーラーのフィールドの情報を削除する、実際には、パートナー センターからそのトレーラーを削除する、されるため、これを行う前に必要なファイルのコピーがあることを確認します。
...
パートナー センターに追加していない以前の新しいアセットが使用する場合は、単一の .csv ファイルではなく、フォルダーとして登録情報をインポートすることによってこれを実行できます。 .csv ファイルを含むフォルダを作成する必要があります。 次に、その同じフォルダーのルート フォルダーまたはサブフォルダーに画像を追加します。 フィールドには、ルート フォルダー名を含む完全なパスを入力する必要があります。
...
アドオンの場合のインポートとエクスポートのストア登録情報を使用して、上記と同じプロセスことを除き、[アドオンのストア登録情報](create-add-on-store-listings.md)に関連する 3 つのフィールドにのみ表示されます**説明**、**タイトル**、および**StoreLogo300x300** (。呼ばれるを**アイコン**にパートナー センターでストア登録情報ページ)。 **[タイトル]** フィールドは必須で、その他の 2 フィールドは省略可能です。
