...
ストアは、アプリのアイコンと、アプリのパッケージに含まれている他の画像も使わ可能性があります。 アプリを申請する前に、[Windows アプリ認定キット](../debug-test-perf/windows-app-certification-kit.md)を実行して、必須の画像が不足していないかどうかを確認してください。 ガイダンスと推奨事項については、これらの画像は、[アプリのアイコンとロゴ](../design/style/app-icons-and-logos.md)を参照してください。
...
ストア登録情報を[複数の言語](supported-languages.md)で用意する場合は、言語ごとに **[ストア登録情報]** ページがあります。 言語ごとに別個に画像をアップロードして (同じ画像を使う場合でも)、使用する字幕を言語ごとに入力する必要があります。 (多くの言語のストア登録情報があれば、するが一番簡単に[、登録情報データをエクスポート](import-and-export-store-listings.md)してオフラインで作業してそれらを更新します。)
...
この画像は、Windows Phone 8.1 以前で適切に表示されるために必要です。 以前に公開されたアプリは、Windows Phone 8.1 以前をサポートしています。 この画像を用意しない場合、それらのユーザーには、アプリの一覧に空白のアイコンが表示されます。 (これにも適用されます Windows 10 ユーザーの場合は、アプリがある Windows Phone 8.1 以前をターゲットとするパッケージ。)

UWP パッケージには申請*のみ*が含まれている場合は、(するチェック ボックスをオンの**Windows 10 および Xbox でのユーザーに対して、パッケージの画像ではなく、アップロードしたロゴ画像を表示**するには、次で説明されていない場合、この画像を提供する必要はありません。セクション)。

### <a name="display-only-uploaded-logo-images-in-the-store"></a>アップロードされたストア ロゴ画像のみを表示

Windows 10 (Xbox を含む) のユーザーに、登録情報を表示するときに、アプリのパッケージのロゴ画像を使用してから、ストアを防ぐため、代わりにアップロードした画像のみを使用して、ストアのオプションがあります。 これにより、Windows 10 (Xbox を含む) ユーザーについては、Microsoft Store のさまざまな画面でのアプリの魅せ方をさらに細かく制御できます。 (以前に公開されたアプリは、以前の OS バージョンをサポートする場合それらのユーザー可能性がありますでも参照イメージのパッケージから)。

パッケージの画像ではなくロゴ画像をアップロード (ユーザー向けに Windows 10, Xbox を含む)、アップロードした画像のみを使用していないとパッケージから任意のイメージを使用して、チェック ボックスをオン**Store For Windows 10 と Xbox のユーザーを表示するには**.

このボックスをオンにすると、**ストアで表示する画像**と呼ばれる新しいセクションが表示されます。 ここでは、(ボックス、そのイメージがこのセクションに移動を指定するフィールドを確認する) の場合は、 **1:1 アプリ タイル アイコン (300 x 300 ピクセル)** のサイズを含む、3 つの画像をアップロードすることができます。 このオプションを使う場合は、300 x 300 ピクセル、150 x 150 ピクセル、71 x 71 ピクセルの 3 つの画像をすべて指定することをお勧めします。 ただし、必須のサイズは 300 x 300 だけです。
...
> [!TIP]
> [16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)画像は特に推奨をストアで[のビデオ トレーラー](#trailers)を含める場合です。これを含めない場合、トレーラーは、登録情報の上部に表示されません。


### <a name="trailers"></a>トレーラー

トレーラーは、製品がどのようなものかを知ってもらうために、製品の動作をユーザーに紹介する短いビデオです。 アプリのストア登録情報の (限りするには、 [16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)画像が含まれます) の上部に表示されます。 
...
> [!IMPORTANT]
> トレーラーを使用する場合は、ストア登録情報の最上部に表示する、トレーラーの順序で[16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)画像セクションも用意する必要があります。 この画像は、トレーラーの再生後に表示されます。
...
**Windows 8.1 以前**では、いくつかのプロモーション用のレイアウトで **414 x 180** ピクセルのサイズが使われることがあります。 アプリでは、Windows 8.1 またはそれ以前を実行する場合は、この sizen で画像を提供することをお勧めします。
