...
ストア登録情報を[複数の言語](supported-languages.md)で用意する場合は、言語ごとに **[ストア登録情報]** ページがあります。 言語ごとに別個に画像をアップロードして (同じ画像を使う場合でも)、使用する字幕を言語ごとに入力する必要があります。 (多くの言語でストア登録情報があれば、するが一番[、登録情報データをエクスポート](import-and-export-store-listings.md)し、オフラインの更新を容易になります。)
...
この画像は、Windows Phone 8.1 以前で適切に表示されるために必要です。 以前に公開されたアプリは、Windows Phone 8.1 以前をサポートしています。 この画像を用意しない場合は、それらのユーザーには、アプリの一覧に空白のアイコンが表示されます。 (これも適用されます Windows 10 のユーザーにアプリには Windows Phone 8.1 以前を対象とするパッケージにしかない場合。)
...
このボックスをオンにする場合は、**ストアで表示する画像**と呼ばれる新しいセクションが表示されます。 ここでは、(ボックスでは、そのイメージがこのセクションに移動を提供するフィールドを確認する) の場合は、 **1:1 アプリ タイル アイコン (300 x 300 ピクセル)** のサイズを含む、3 つの画像をアップロードすることができます。 このオプションを使う場合は、300 x 300 ピクセル、150 x 150 ピクセル、71 x 71 ピクセルの 3 つの画像をすべて指定することをお勧めします。 ただし、必須のサイズは 300 x 300 だけです。
...
> [!TIP]
> [16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)画像特にお勧めする、ストアでの[ビデオ トレーラー](#trailers)を含める場合は登録情報これを含めない場合、トレーラーは登録情報の上部に表示されません。
...
**Windows 10 および Xbox 画像**のセクションでは、 **16:9 スーパー ヒーロー アート (1920 x 1080 または 3840 x 2160 ピクセル)** イメージを使用して、すべての Windows 10 デバイスの種類が (Xbox を含む) で、Microsoft Store で、さまざまなレイアウトにします。 アプリがターゲットとしている OS バージョンやデバイスの種類が何であっても、この画像を提供することをお勧めします。
...
**Windows 8.1 以前**では、いくつかのプロモーション用のレイアウトで **414 x 180** ピクセルのサイズが使われることがあります。 アプリでは、Windows 8.1 またはそれ以前を実行する場合、この sizen で画像を提供することをお勧めします。
