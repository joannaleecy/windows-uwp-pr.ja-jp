...
ms.locfileid: "6256779"
---
# <a name="app-screenshots-images-and-trailers"></a>アプリのスクリーンショット、画像、トレーラー

デザインが優れた画像は、ストアで潜在顧客にアプリをアピールする効果的な方法の 1 つです。

[スクリーン ショット](#screenshots)、[ロゴ](#store-logos)、[トレーラー](#trailers)や、アプリのストア登録情報に含める追加のアート資産を提供することができます。 これらの中には必須の画像も、省略可能な画像もあります (ただし、省略可能な画像の中には、ストア登録情報をできるだけ魅力的なものにするため、含めた方がよい画像があります)。
...
UWP パッケージには申請*のみ*が含まれている場合は、(場合を除くするチェック ボックスをオンの**Windows 10 および Xbox でのユーザーに対して、パッケージの画像ではなく、アップロードしたロゴ画像を表示**するには、次のように、この画像を提供する必要はありません。セクション)。
...
(ユーザー向けに Windows 10, Xbox を含む)、アップロードした画像のみを使用していないとパッケージから任意のイメージを使用して、**と書かれているボックスをオンにストア For Windows 10 と Xbox のユーザーを表示するには、パッケージの画像ではなくロゴ画像をアップロード**.

このボックスをオンにする場合は、**ストアで表示する画像**と呼ばれる新しいセクションが表示されます。 ここでは、(ボックスでは、その画像がこのセクションに移動を提供するフィールドを確認する) 場合は、 **1:1 アプリ タイル アイコン (300 x 300 ピクセル)** のサイズを含む 3 つの画像をアップロードすることができます。 このオプションを使う場合は、300 x 300 ピクセル、150 x 150 ピクセル、71 x 71 ピクセルの 3 つの画像をすべて指定することをお勧めします。 ただし、必須のサイズは 300 x 300 だけです。
...
> [!TIP]
> [16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)画像は特に推奨をストアでの[ビデオ トレーラー](#trailers)を含める場合です。これを含めない場合、トレーラーは登録情報の上部に表示されません。
...
**Windows Phone 8.1 以前**では、**1000 x 800 ピクセル (5:4)** と **358 x 358 ピクセル (1:1)** の 2 つの画像サイズをプロモーション用のレイアウトで使用できます。 Windows Phone 8.1 以前で、アプリの実行する場合は、これらのサイズの両方の画像を提供することをお勧めします。  
