...
[スクリーン ショット](#screenshots)、[ロゴ](#store-logos)、[トレーラー](#trailers)、およびアプリのストア登録情報に含める追加のアート資産を提供することができます。 これらの中には必須の画像も、省略可能な画像もあります (ただし、省略可能な画像の中には、ストア登録情報をできるだけ魅力的なものにするため、含めた方がよい画像があります)。
...
ストアは、アプリのアイコンと、アプリのパッケージに含まれている他の画像を使用することも。 アプリを申請する前に、[Windows アプリ認定キット](../debug-test-perf/windows-app-certification-kit.md)を実行して、必須の画像が不足していないかどうかを確認してください。 ガイダンスと推奨事項については、これらの画像は、[アプリのアイコンとロゴ](../design/style/app-icons-and-logos.md)を参照してください。
...
ストア登録情報を[複数の言語](supported-languages.md)で用意する場合は、言語ごとに **[ストア登録情報]** ページがあります。 言語ごとに別個に画像をアップロードして (同じ画像を使う場合でも)、使用する字幕を言語ごとに入力する必要があります。 (、多くの言語でストア登録情報があれば、するが一番簡単に[、登録情報データをエクスポート](import-and-export-store-listings.md)してオフラインで作業してそれらを更新します。)
...
この画像は、Windows Phone 8.1 以前で適切に表示されるために必要です。 以前に公開されたアプリは、Windows Phone 8.1 以前をサポートしています。 この画像を用意しない場合、それらのユーザーには、アプリの一覧に空白のアイコンが表示されます。 (これも適用されます Windows 10 のユーザーにアプリには Windows Phone 8.1 以前を対象とするパッケージにしかない場合。)

UWP パッケージには申請*のみ*が含まれている場合は、(するチェック ボックスをオンの**Windows 10 および Xbox でのユーザーに対して、パッケージの画像ではなく、アップロードしたロゴ画像を表示**するには、次で説明されていない場合は、この画像を提供する必要はありません。セクション)。

### <a name="display-only-uploaded-logo-images-in-the-store"></a>アップロードされたストア内のロゴ画像のみを表示

Windows 10 (Xbox を含む) のユーザーに、登録情報を表示するときに、アプリのパッケージのロゴ画像を使用してから、ストアを防ぐため、代わりにアップロードした画像のみを使用して、ストアのオプションがあります。 これにより、Windows 10 (Xbox を含む) ユーザーについては、Microsoft Store のさまざまな画面でのアプリの魅せ方をさらに細かく制御できます。 (以前に公開されたアプリは、以前の OS バージョンをサポートする場合それらのユーザー可能性がありますでも参照イメージ、パッケージから)。

パッケージの画像ではなくロゴ画像をアップロード (ユーザー向けに Windows 10, Xbox を含む)、アップロードした画像のみを使用していないと、パッケージから任意のイメージを使用して、チェック ボックスをオン**Store For Windows 10 と Xbox のユーザーを表示するには**.

このボックスをオンにすると、**ストアで表示する画像**と呼ばれる新しいセクションが表示されます。 ここでは、(ボックス、そのイメージがこのセクションに移動を提供するフィールドを確認する) の場合は、 **1:1 アプリ タイル アイコン (300 x 300 ピクセル)** のサイズを含む、3 つの画像をアップロードすることができます。 このオプションを使う場合は、300 x 300 ピクセル、150 x 150 ピクセル、71 x 71 ピクセルの 3 つの画像をすべて指定することをお勧めします。 ただし、必須のサイズは 300 x 300 だけです。
...
> [!TIP]
> [16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)画像は特に推奨されますをストアでの[ビデオ トレーラー](#trailers)を含める場合は、登録情報これを含めない場合、トレーラーは、登録情報の上部に表示されません。


### <a name="trailers"></a>トレーラー

トレーラーは、製品がどのようなものかを知ってもらうために、製品の動作をユーザーに紹介する短いビデオです。 アプリのストア登録情報の (限り[16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)画像が含まれます) の上部に表示されます。 
...
> [!IMPORTANT]
> トレーラーを使用する場合、トレーラー ストア登録情報の上部に表示するために、 [16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)イメージ] セクションを提供する必要もあります。 この画像は、トレーラーの再生後に表示されます。
...
登録情報からトレーラーを削除するには、ファイル名の横にある **[X]** をクリックします。 のみ、現在ストア登録情報には、次の作業から削除するすべての製品の Store 登録情報 (すべての言語) から削除するかを選択できます。
...
**Windows 10 および Xbox 画像**のセクションでは、 **16:9 スーパー ヒーロー アート (1920 x 1080 または 3840 x 2160 ピクセル)** イメージを使用して、(Xbox を含む) すべての Windows 10 デバイスの種類の Microsoft Store で、さまざまなレイアウトにします。 アプリがターゲットとしている OS バージョンやデバイスの種類が何であっても、この画像を提供することをお勧めします。
...
以前から送信されたアプリは、以前の OS バージョンをサポートしている場合 (Windows 8.x や Windows Phone 8.x)、プロモーション用のレイアウトでアプリを取り上げる (ただし、アプリが取り上げられることが保証はありません) を考慮するためにはこれらの画像を提供する必要があります。 アプリがこれらの以前の OS バージョンをサポートしていない場合は、このセクションをスキップします。 (このセクションは、以前は「**オプションのプロモーション用の画像**」と呼ばれていました)。

**Windows Phone 8.1 以前**では、**1000 x 800 ピクセル (5:4)** と **358 x 358 ピクセル (1:1)** の 2 つの画像サイズをプロモーション用のレイアウトで使用できます。 Windows Phone 8.1 以前で、アプリの実行している場合は、これらのサイズの両方の画像を提供することをお勧めします。  
...
**Windows 8.1 以前**では、いくつかのプロモーション用のレイアウトで **414 x 180** ピクセルのサイズが使われることがあります。 アプリには、Windows 8.1 またはそれ以前が実行される、この sizen で画像を提供することをお勧めします。
