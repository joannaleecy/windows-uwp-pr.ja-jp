...
ストア登録情報を[複数の言語](supported-languages.md)で用意する場合は、言語ごとに **[ストア登録情報]** ページがあります。 言語ごとに別個に画像をアップロードして (同じ画像を使う場合でも)、使用する字幕を言語ごとに入力する必要があります。 (多くの言語のストア登録情報があれば、するが一番簡単に[登録情報データをエクスポートしてオフライン](import-and-export-store-listings.md)で更新する。)
...
UWP パッケージで、申請*のみ*含まれる場合 (するチェック ボックスをオンの**Windows 10 および Xbox でのユーザーに対して、パッケージの画像ではなく、アップロードしたロゴ画像を表示**するには、次で説明されていない場合は、この画像を提供する必要はありません。セクション)。
...
このボックスをオンにする場合は、**ストアで表示する画像**と呼ばれる新しいセクションが表示されます。 ここでは、(ボックス、そのイメージがこのセクションに移動を指定するフィールドを確認する) の場合は、 **1:1 アプリ タイル アイコン (300 x 300 ピクセル)** のサイズを含む、3 つの画像をアップロードすることができます。 このオプションを使う場合は、300 x 300 ピクセル、150 x 150 ピクセル、71 x 71 ピクセルの 3 つの画像をすべて指定することをお勧めします。 ただし、必須のサイズは 300 x 300 だけです。
...
> [!TIP]
> [16:9 スーパー ヒーロー アート](#windows-10-and-xbox-image-169-super-hero-art)画像は特に推奨をストアでの[ビデオ トレーラー](#trailers)を含める場合は、登録情報これを含めない場合、トレーラーは登録情報の上部に表示されません。
...
登録情報からトレーラーを削除するには、ファイル名の横にある **[X]** をクリックします。 のみ、現在ストア登録情報で作業しているから削除するすべての製品の Store 登録情報 (すべての言語) から削除するかを選択できます。
...
セクションでは、 **Windows 10 および Xbox 画像**、 **16:9 スーパー ヒーロー アート (1920 x 1080 または 3840 x 2160 ピクセル)** イメージはすべて Windows 10 デバイスの種類 (Xbox を含む) で、Microsoft Store で、さまざまなレイアウトに使用されます。 アプリがターゲットとしている OS バージョンやデバイスの種類が何であっても、この画像を提供することをお勧めします。
