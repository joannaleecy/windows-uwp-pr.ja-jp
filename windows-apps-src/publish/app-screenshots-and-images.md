...
### <a name="display-only-uploaded-logo-images-in-the-store"></a>アップロードされたストア内のロゴのイメージのみを表示

ストア (Xbox を含む)、Windows の 10 の顧客の一覧を表示するときに、アプリのパッケージのロゴのイメージを使用して、アップロードした画像のみを使用して、ストアがある代わりにすることができます。 これにより、Windows 10 (Xbox を含む) ユーザーについては、Microsoft Store のさまざまな画面でのアプリの魅せ方をさらに細かく制御できます。 (お使いの製品では、以前の OS バージョンをサポートする場合顧客可能性がありますも参照イメージから、パッケージでは、このオプションを使用する場合でも)。

10 の Windows と Xbox 上で顧客向けストア (上で顧客向け Windows 10、Xbox を含む)、アップロードする画像だけを使用しないと、パッケージからすべてのイメージを使用して、**ことを示すボックスをオンにを表示するには、自分のパッケージのイメージではなくロゴの画像をアップロード**.

このボックスをチェックすると、**イメージを表示するストア**と呼ばれる新しいセクションが表示されます。 ここでは、(ボックスでは、そのイメージは、このセクションに移動しますを提供するフィールドを確認する) 場合は、 **1:1 のアプリケーションを並べて表示するアイコン (300 x 300 ピクセル)** のサイズを含む 3 つの画像をアップロードできます。 このオプションを使う場合は、300 x 300 ピクセル、150 x 150 ピクセル、71 x 71 ピクセルの 3 つの画像をすべて指定することをお勧めします。 ただし、必須のサイズは 300 x 300 だけです。
