...
Windows 10 (Xbox を含む) のユーザーに、登録情報を表示するとき、アプリのパッケージのロゴ画像を使用してから、ストアを防ぐため、代わりにアップロードした画像のみを使用して、ストア オプションがあります。 これにより、Windows 10 (Xbox を含む) ユーザーについては、Microsoft Store のさまざまな画面でのアプリの魅せ方をさらに細かく制御できます。 (製品は、以前の OS バージョンをサポートする場合それらのユーザー可能性がありますも参照イメージ、パッケージからこのオプションを使用する場合でも)。

[パッケージから画像ではなくロゴ画像をアップロード (ユーザー向けに Windows 10, Xbox を含む)、アップロードした画像のみを使用して、パッケージから任意のイメージを使用して、いない**ことを示すチェック ボックスをオンにストア For Windows 10 と Xbox のユーザーを表示するには**.

このチェック ボックスをオンにすると、**ストアで表示する画像**と呼ばれる新しいセクションが表示されます。 ここでは、(ボックス、そのイメージがこのセクションに移動を指定するフィールドを確認する) の場合は、 **1:1 アプリ タイル アイコン (300 x 300 ピクセル)** のサイズを含む、3 つの画像をアップロードすることができます。 このオプションを使う場合は、300 x 300 ピクセル、150 x 150 ピクセル、71 x 71 ピクセルの 3 つの画像をすべて指定することをお勧めします。 ただし、必須のサイズは 300 x 300 だけです。
