...
**30 D**期間を選択したら、[**ユーザー**、**デバイス**の場合は、[**期間**] タブを表示したときに、円マーカーを表示可能性があります。 これらは大幅に増加を表す増減内の指定された値を知りたいと思います。 円を表示する日付が大幅に増加または減少前に、その曜日と比較してを検出して週の最後を表します。 詳細については、変更内容を表示するには、円をマウスでポイントします。  
