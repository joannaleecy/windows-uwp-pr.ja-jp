...
アプリの Web ページの URL を入力します。 この URL は、ストアのアプリの登録情報 Web ページではなく、独自の Web サイトのページを指すものにする必要があります。 このフィールドは任意ですが、使用することをお勧めします。
...
このセクションは、製品のカテゴリとして **[ゲーム]** を選択した場合にのみ表示されます。 ここでは、ゲームがサポートしている機能を指定できます。 このセクションで指定した情報は、製品のストアに表示されますが一覧表示します。
...
- **Windows 上で顧客向け 8.x およびそれ以前または Windows Phone 8.x 以前のバージョン。**
