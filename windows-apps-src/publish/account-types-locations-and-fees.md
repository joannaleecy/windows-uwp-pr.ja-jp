...
会社のアカウントの方が費用が少し高くなる主な原因は、会社を代表してアカウントをセットアップする権限がユーザーにあることを確かめるために追加の手続きが必要になることにあります。 あたり[ストア ポリシー](https://docs.microsoft.com/legal/windows/agreements/store-policies#1014-account-type)アプリケーションで特定の機能では、財務のアカウントにアクセスする情報を必要と (せずに、セキュリティで保護された専用のサード パーティ製の認証プロバイダーを使用)、主要な機能にアクセスするための認証会社コードでのみ発行できます。

## <a name="account-verification"></a>アカウントの検証

サインアップ プロセスが完了すると、アカウントに対して検証プロセスが開始されます。 個人アカウントの場合、別の会社が、発行者の表示名を既に使用していないかどうかを確認します。 会社のアカウントについては、Microsoft の確認パートナーの Dun & Bradstreet により、会社の代理でアカウントを作る権限をお持ちかどうかを確認します。 この処理には、数日から数週間かかることがあり、多くの場合は会社への電話連絡が行われます (そのため、登録フォームに入力するときにはすべて最新の連絡先を指定してください)。 まで、それが検証されているが、待っている間[、アプリケーションの名前を予約](create-your-app-by-reserving-a-name.md)することができます。 会社のアカウントからアプリケーションを送信することはできませんを構築し、アプリケーションをテストを続行して、提出書類の準備で作業します。
