...
Windows デベロッパー センター ダッシュ ボードの **[取得]** レポートでは、取得してと共に、人口統計データ、アプリをインストールしたユーザーを表示できますプラットフォームの詳細と表示については、どの Windows 10 (Xbox を含む) のユーザーがアプリの。登録情報。 最後の時間、または 70-2 時間の期間のリアルタイムの入手データの近く表示することもできます。 
...
ページの上部で、データを表示する期間を選択できます。 既定では **[30 日間]** が選択されていますが、3、6、12 か月間のデータや、指定した任意の期間のデータを表示することもできます。 すべてのオプションでリアルタイムに近いデータが表示されます (**アプリの累積的な**データを除く)。 **1 H**と**72 H**時間期間のみ**取得**グラフの**毎日アプリ**タブと適用**市場**グラフの**取得**] タブにされます。 
...
既定では、ほぼリアルタイム データが含まれている**アプリを毎日**のビューを表示します。 **[アプリの累計]** を選ぶと、アプリの有効期間内の取得数を確認することもできます。 この値は、アプリが初めて公開されたときからの全取得数の累計です。
...
**毎日のアプリ**ビューで**30 日間**の期間が選択されると、円形のマーカーを表示があります。 これらは、大幅に増加を表すまたは特定の値について理解しているたいと考えることが減少します。 円が表示される日付は、大幅に増加または減少それより前に、の週との比較を検出されました週の終わりを表します。 については、変更内容について詳しくは、円の上に置きます。  

> [!TIP]
> [インサイト レポート](insights-report.md)では、過去の 30 日間の大幅な変更に関連する他の情報を表示することができます。
