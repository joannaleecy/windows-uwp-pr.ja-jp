...
Windows デベロッパー センター ダッシュ ボードで**取得**レポートを取得して、アプリ、人口統計データと共にインストールされているユーザーを確認でき、プラットフォームの詳細と表示に関する情報で、アプリの Windows 10 (Xbox を含む) のユーザーが到着した方法登録情報。 最後の時間、または 70 ~ 2 時間の期間のリアルタイムの入手データの近く表示することもできます。 
...
ページの上部で、データを表示する期間を選択できます。 既定では **[30 日間]** が選択されていますが、3、6、12 か月間のデータや、指定した任意の期間のデータを表示することもできます。 すべてのオプションのリアルタイムに近いデータが表示されます (**アプリの累積的な**データを除く)。 **1 H**と**72**時間期間のみ**取得**グラフの**アプリを毎日**タブと適用**市場**グラフの**取得**] タブにされます。 
...
**総売り上げ**(2016 年 10 月に存在する) からアプリの使用もこのグラフで合計金額 (USD) でのアプリ販売から獲得を示すできます。 この金額が、払い戻し、払戻し、チャージ バックなどのアカウントしないことに注意してください。
