...
Windows デベロッパー センター ダッシュボードの **[取得]** レポートでは、アプリを取得してインストールしたユーザーと、人口統計データやプラットフォームの詳細を確認できます。 また、Windows 10 をお使いのお客様が、アプリの登録情報にたどり着いた経路についての情報も確認できます。
...
**[インストール]** グラフは、指定された期間で、Microsoft が検出できた、ユーザーがアプリを Windows 10 デバイスに正常にインストールした回数を示します。 合計数と、(指定された期間に応じて) 日次または週次でのインストール数を示すグラフが表示されます。 特定のパッケージ バージョンを基に、結果をフィルター処理することもできます。

インストールの合計数には、次のケースが含まれます。
-   **複数の Windows 10 デバイスにおけるインストール。** たとえば、同じユーザーがアプリを 2 台の Windows 10 PC と 1 台の Windows 10 スマートフォンにインストールした場合、3 回のインストールとしてカウントされます。
...
> [!IMPORTANT]
> **[顧客獲得モデル]** に表示されるのは、過去 90 日間の Windows 10 ユーザーについてのデータのみです。
