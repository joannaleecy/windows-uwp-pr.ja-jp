...
> [!IMPORTANT]
> 、アプリの追加の名前を予約して、ダッシュ ボードで最初に、アプリを作成するときに予約して 1 つではなく、アプリの公開されているバージョンでそれらのいずれかを使用することができます。 ただしなどの[id の詳細](view-app-identity-details.md)、**パッケージ ファミリ名 (PFN)** の製品の予約名は、it の一部で使用されます。 これらの値は、一部のユーザーに表示される場合がありすることはできません、変更されたため、必ずまずを予約する名前が、このような使用に適していること。
