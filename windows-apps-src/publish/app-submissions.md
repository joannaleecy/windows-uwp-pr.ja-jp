...
申請は、アプリが完成して公開する準備ができたときに開始できます。または、1 行のコードを記述する前でも情報を入力し始めることができます。 戻る準備ができたときに作業できるように、申請を更新プログラムが保存されます。
