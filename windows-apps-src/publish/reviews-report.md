...
Windows デベロッパー センター ダッシュ ボードで**レビュー**レポートでは、ストアでアプリを評価するときにユーザーが入力されたレビュー (コメント) を確認できます。

このデータは、ダッシュボードで表示することも、[レポートをダウンロード](download-analytic-reports.md) してオフラインで表示することもできます。 または、 [Microsoft Store 分析 REST API](../monetize/access-analytics-data-using-windows-store-services.md)、[アプリのレビューを取得する](../monetize/get-app-reviews.md)メソッドを使用してプログラムでこのデータを取得することができます。

顧客のレビュー[このページから直接](respond-to-customer-reviews.md)、プログラムで[、Microsoft Store レビュー API 経由で](../monetize/submit-responses-to-app-reviews.md)、または[デベロッパー センター アプリ](https://www.microsoft.com/store/apps/dev-center/9nblggh4r5ws)を使って応答することもできます。
...
- **レビュー コンテンツ**: 既定の設定は、**評価とレビュー コンテンツ**、レビュー コンテンツを唯一の評価が示されることを意味します。 **すべて**の書き込まれたレビューのテキストを含めないでください場合でも、すべての評価を表示するを選択できます。 **評価の内訳]** グラフで選択内容に関係なく、すべてのレビューは常に表示することに注意してください。
...
-   **[カテゴリ名]**: 既定のフィルターは **[すべて]** です。 特定の[レビュー インサイト カテゴリ](#review-insight-categories)を選択すると、そのカテゴリに関連付けられているレビューのみを表示できます。 
...
によって、次の確認を収集できるように、このレポートの上部にある**評価の内訳]** グラフが表示されます。 
- アプリの平均の星評価。
...
**レビュー コンテンツ**ページ フィルターで**評価とレビュー コンテンツ**を選択した場合でもこのグラフ常に含まれているすべてのレビュー、注意してください。

このグラフで[評価レポート](ratings-report.md)の詳細については、アプリの評価はと共にも確認できます。


< スパン id ="レビュー インサイトのカテゴリ/>

## <a name="insight-categories"></a>インサイト カテゴリ

**インサイト カテゴリ**グラフでグループ化判断されたカテゴリに従ってレビューはレビューを関連付けることができます。
...
このページのレビューは、日付や評価によって昇順または降順に並べ替えることができます。 **日付**や**評価**で並べ替えるオプションを表示するのには、 **[Sort by**リンクをクリックします。

アプリのレビューの特定の単語または語句を検索する検索ボックスを使用することもできます。 レビューが別の言語で記述された場合でも、ユーザーによって書き込まれた元のレビュー テキストのみを検索するに注意してください。 翻訳済みのレビューのテキストは検索されません。
...
前述のように、レビューを検索するには、ユーザーが残した元のテキストのみが検索されます (とテキストを翻訳なく)、**レビューの翻訳**チェック ボックスをオンにした場合も同様です。
