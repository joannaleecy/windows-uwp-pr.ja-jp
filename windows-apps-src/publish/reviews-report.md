...
[パートナー センター](https://partner.microsoft.com/dashboard)で**レビュー**レポートでは、ストアでアプリを評価するときにユーザーが入力されたレビュー (コメント) を確認できます。

オフラインで表示するパートナー センターでは、または[レポートをダウンロードするの](download-analytic-reports.md)には、このデータを表示できます。 または、 [Microsoft Store 分析 REST API](../monetize/access-analytics-data-using-windows-store-services.md)、[アプリのレビューを取得する](../monetize/get-app-reviews.md)メソッドを使用してこのデータをプログラムで取得できます。
...
- **レビュー コンテンツ**: 既定の設定は、**評価とレビュー コンテンツ**、レビュー コンテンツを持つ唯一の評価が示されることを意味します。 **すべて**のすべての評価を表示し、任意の書面によるレビューのテキストが含まれないを含めを選択できます。 **評価の内訳**グラフで、選択内容に関係なく、すべてのレビューは常に表示することに注意してください。
...
-   レビューアーの国/地域。
-   レビューが残された時点でのユーザーのデバイス上のアプリのパッケージ バージョン  (この情報はオンラインまたは送信された Windows8.1 以前のユーザーによって送信されたレビューについては利用できません) です。
-   レビューが残されたときにユーザーが使っていたデバイスの OS のバージョン。
-   レビューが残されたときにユーザーが使っていたデバイスの名前  (この情報はオンラインまたは送信された Windows8.1 以前のユーザーによって送信されたレビューについては利用できません) です。
...
このページのレビューは、日付や評価によって昇順または降順に並べ替えることができます。 **日付**や**評価**で並べ替えるオプションを表示、**並べ替え**リンクをクリックします。
...
前述のように、レビューを検索するには、お客様が残した元のテキストのみが検索されます (とテキストを翻訳なく)、**レビューの翻訳**チェック ボックスをオンにした場合。
