...
- テスターが書いたレビューは、[レビュー レポート](reviews-report.md)で確認できます。 ただし、申請が **[Public audience]** (パブリック対象ユーザー) に移動されても、これらのレビューはアプリの Store 登録情報に公開されません。  
- アプリを **[Private audience]** (プライベート対象ユーザー) から **[Public audience]** (パブリック対象ユーザー) に移動すると、Store 登録情報に表示される **[Release date]** (リリース日) はパブリック対象ユーザーに最初に公開された日になります (別の[リリース日の表示](set-app-pricing-and-availability.md#display-release-date)を指定した場合を除く)。
