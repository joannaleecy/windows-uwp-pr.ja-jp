...
3. セクションでは、**テンプレートを選択して**、**送信する**[通知の種類](#notification-template-types)を選択します。
...
2.  **通知**] ページで、**解析**] タブをクリックします。このタブでは、次のデータが表示されます。
    * トースト通知とアクション センターの通知のさまざまなユーザー操作の状態のグラフ ビュー。
    * トースト通知とアクションのクリック-を通じて-レートの世界地図ビュー中央に通知します。
