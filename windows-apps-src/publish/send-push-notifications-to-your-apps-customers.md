...
3. **テンプレートを選択**] セクションで、送信し、[ **OK**] をクリックする[通知の種類](#notification-template-types)を選択します。
4. 次のページで、ドロップダウン メニューを使って通知を生成する **1 つのアプリ**または**複数のアプリ**を選びます。 [マイクロソフト ストア サービス SDK を使用して通知を受信するように構成](../monetize/configure-your-app-to-receive-dev-center-notifications.md)されているアプリケーションを選択することができます。
...
2.  通知を既存のテーブルでは、**進行中**または**完了**を選択し、各通知の高レベルのパフォーマンスを確認するのには**配布率**および**アプリケーションの起動速度**の列を確認します。
...
すべての通知のためのユーザー アクティビティ データを分析します。

1.  Windows デベロッパー センター ダッシュボードで、**[利用率の引き上げ]** セクションを展開し、**[通知]** を選びます。
2.  [**通知**] ページで、[**分析**] タブをクリックします。このタブには、次のデータが表示されます。
    * Toasts とアクション センターの通知のさまざまなユーザー操作の状態のビューをグラフ化します。
    * Toasts とアクションのレートでの] をクリックします - 世界地図のビューは、通知を中央揃えします。
3. ページの上部で、データを表示する期間を選択できます。 既定では [30 日間] が選択されていますが、3、6、12 か月間のデータや、指定した任意の期間のデータを表示することもできます。 すべてのアプリケーションと市場でのデータをフィルター処理する**フィルター**を展開することもできます。
