...
以前の OS バージョンのストア登録情報の一部をカスタマイズするオプションがある場合は、さまざまなオペレーティング システムをターゲットとするパッケージが以前に公開されたアプリに、(Windows 8.x 以前や Windows Phone 8.x またはそれ以前)。 

Windows 10 (Xbox を含む) のユーザーは、既定の[Store 登録情報](create-app-store-listings.md)を常に表示されます。 1 つまたは複数の以前の OS バージョンをサポートするパッケージを使用したアプリを既に公開されている場合を除き、プラットフォーム固有のストア登録情報を作成するオプションは表示されません。 
...
1 つの OS バージョンでのみ表示される機能について説明したりするか、独立したデバイスの種類の特定の OS に固有のスクリーン ショットを提供する場合、プラットフォーム固有のストア登録情報が役に立ちます。
...
取り込まれます、適用可能なテキストと画像の既定のストアに入力した登録情報は、作業の開始点として、(Windows 10) の既定の Store 登録情報を使用することができます。そのことができますか保存する前に変更を加えます。 必要に応じて、完全に空のストア登録情報から始めることもできます。
