...
> [!TIP]
> こともできます[のインポートし、エクスポートのストアの番組表](import-and-export-store-listings.md)かどうか、.csv ファイルにオフライン、リスティング情報を入力する場合はなく、パートナー センターで直接ファイルの情報を指定して、アップロードします。 インポートおよびエクスポート オプションでは、一度に複数の更新を行うことができるため、多くの言語の登録情報がある場合に特に便利です。 

以前に発行アプリは、Windows をサポートしている場合 8.x および Windows Phone 8.x またはできます以前では、[プラットフォームに固有のストアの一覧を作成](create-platform-specific-store-listings.md)を顧客に表示します。 
...
このドロップダウン リスト ボックスでは、(アプリの 1 つ以上の名前を予約している) 場合、ストアの一覧で使用する名前を指定できます。

アップロードした場合は、パッケージ ストアの一覧を表示すると同じ言語で作業中、それらのパッケージで使用される名前が選択されます。 必要がある場合[、アプリの名前を変更](manage-app-names.md#rename-an-app-that-has-already-been-published)後、既に発行されていますが、新しい名前を使用するパッケージをアップロードした後、新しい送信を作成するときに別の予約された名前は、ここを選択できます。

言語のパッケージをアップロードしていない場合での作業し 1 つ以上の名前を予約した、元の名前を取得するには、その言語で、関連するパッケージがないため、予約済みのアプリ名のいずれかを選択する必要があります。

> [!NOTE]
> **製品名**する選択がで作業している言語でのストアの一覧にのみ適用されます。 顧客は、アプリをインストールするときに表示される名前には影響しませんその名前は、インストールされるパッケージのマニフェストから取得されます。 混乱を避けるためには、各言語のパッケージおよびストアの一覧と同じ名前を使用することをお勧めします。
...
初めてアプリを申請する場合、このフィールドは空白のままにしてください。 既存のアプリの更新は、最新のリリースの変更点を顧客をさせることができます。 このフィールドには 1,500 文字の制限があります。 (以前は、このフィールドは **[リリース ノート]** と呼ばれていました)。
...
> [!NOTE]
> これらの機能は、独自の箇条書きを追加しないように、ストアの一覧で箇条書きに表示されます。
