...
言語のパッケージをアップロードしていない場合は、作業中と複数の名前を予約した名前をプルするには、その言語パッケージが関連付けられておらずため、予約済みのアプリ名のいずれかを選択する必要があります。

> [!NOTE]
> のみを選択する**製品名**は、ストア登録情報の言語で作業しているに適用されます。 顧客に、アプリをインストールするときに表示される名前は影響しませんその名前は、インストールされるパッケージのマニフェストから取得されます。 混乱を避けるためには、各言語のパッケージとストア登録情報が同じ名前を使用することをお勧めします。
...
最適な結果を得るには、短い説明 [270 文字してください。 フィールドには 500 文字の制限がありますが、一部のビューで最初に 270 文字のみに表示されます (短い説明の残りの部分は表示可能なリンク) を持つ。
