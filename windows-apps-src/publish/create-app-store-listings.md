...
> [!TIP]
> できます必要に応じて[をインポートおよびエクスポートのストア登録情報](import-and-export-store-listings.md)に情報を提供してパートナー センターで直接ファイルのアップロードではなく、.csv ファイルをオフライン登録情報を入力する場合。 インポートおよびエクスポート オプションでは、一度に複数の更新を行うことができるため、多くの言語の登録情報がある場合に特に便利です。 

以前に公開されたアプリは、Windows をサポートしている場合 8.x や Windows Phone 8.x または以前では、[プラットフォーム固有のストア登録情報の作成](create-platform-specific-store-listings.md)をこれらのユーザーに表示できます。 
...
ストア登録情報を編集するには、申請の概要ページから言語名を選択します。 ストア登録情報をエクスポートし、オフライン、すべての登録情報データを一度にインポートを選んだ場合を除く別に、各言語を編集する必要があります。 機能する方法について詳しくは、次を参照してください。[インポートおよびエクスポートのストア登録情報](import-and-export-store-listings.md)します。

利用可能なフィールドは次のとおりです。

## <a name="product-name"></a>Product name (製品名)

このドロップダウン ボックスでは、(アプリの 1 つ以上の名前を予約している) 場合、Store 登録情報で使用する名前を指定できます。
...
> [!NOTE]
> 作業している登録情報の言語でストアにのみ選択した**製品名**が適用されます。 顧客に、アプリをインストールするときに表示される名前は影響しませんその名前は、インストールされるパッケージのマニフェストから取得されます。 混乱を避けるためには、各言語のパッケージとストア登録情報が、同じ名前を使用することをお勧めします。
...
初めてアプリを申請する場合、このフィールドは空白のままにしてください。 既存のアプリに update では、これは、顧客に最新のリリースで変更された内容をさせることができます。 このフィールドには 1500 文字の制限があります。 (以前は、このフィールドは **[リリース ノート]** と呼ばれていました)。
...
ビデオ トレーラーやプロモーション用の画像など、製品の追加のアセットを提出することができます。 これらはいずれも省略可能ですが、できるだけ多くの資産のアップロードを検討することをお勧めします。 これらの画像は、より魅力的な登録情報を作成し、製品がどのようなものかをユーザーに知ってもらうために役立ちます。
