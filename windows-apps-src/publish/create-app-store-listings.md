...
このドロップダウン ボックスでは、ストア登録情報の名前の使用する必要があります (この場合、アプリの 1 つ以上の名前を予約して) を指定できます。
...
> [!NOTE]
> 作業している登録情報の言語でストアにのみ選択した**製品名**が適用されます。 顧客が、アプリをインストールするときに表示される名前は影響しませんその名前は、インストールされるパッケージのマニフェストから取得されます。 混乱を避けるためには、各言語のパッケージとストア登録情報が同じ名前を使用することをお勧めします。
