...
> [!NOTE]
> 作業している登録情報の言語でストアにのみ選択した**製品名**が適用されます。 顧客は、アプリをインストールするときに表示される名前は影響しませんその名前は、インストールされるパッケージのマニフェストから取得されます。 混乱を避けるためには、各言語のパッケージとストア登録情報が、同じ名前を使用することをお勧めします。
...
最適な結果を得るには、簡単な説明で 270 文字してください。 フィールドには 500 文字の制限が、一部のビューで最初に 270 文字のみを (短い説明の残りの部分は表示可能なリンク) と共に表示されます。
