...
ms.locfileid: "2830128"
---
# <a name="package-flights"></a>パッケージ フライト

担当者の制限付きグループに特定のパッケージを配布するのにパッケージのフライトを使用することができます。 経験が中断されないように、その他のお客様は、既にストアに公開したパッケージが使用されます。

パッケージのフライト パッケージのみが異なっています。ストアの詳細を一覧表示は、すべてのお客様のと同じになります。 飛行機のグループに登録されていないお客様は引き続き、通常の (非 flighted) パッケージが表示されるときに、フライト パッケージに含まれるパッケージ、フライト グループ内のユーザーが届きます。  後でパッケージ フライトからパッケージをすべての顧客に使用できるようにする場合、非 flighted 送信で同じパッケージを簡単に使用できます。 パッケージのフライトが[承認プロセス](the-app-certification-process.md)を送信時と同じ渡す必要があることを確認します。

パッケージのフライトを設定する際に、**既知のユーザー グループ**(飛行機のグループとも呼ばれます) に追加して特定のパッケージを取得する必要があるユーザーを指定できます。 パッケージ フライトをサポートするバージョンの Windows 10 (Windows.Desktop ビルド 10586 以降、Windows.Mobile ビルド 10586.63 以降または Xbox One) を実行するデバイスを使っている、フライト グループに属するすべてのユーザーは、その特定のグループに指定したパッケージ フライトからパッケージを入手します  (パッケージ フライトには、Windows 8.1 や Windows Phone 8.1 以前のバージョンを含む、すべての OS バージョンをターゲットとするパッケージを含めることができます)。いずれかのフライト グループに追加されていないユーザーや、パッケージ フライトをサポートしないデバイスを使っているユーザーはすべて、フライト以外の申請からパッケージを入手します。
...
> [!NOTE]
> 複数のパッケージ フライトがある場合、それぞれにランクを割り当てる必要があります。 さらに詳しい情報[追加して追加 rank のパッケージのフライト](#add-and-rank-additional-package-flights)下を参照してください。
...
公開された以前の申請 (フライト以外の申請、または複数ある場合は他のパッケージ フライトのいずれか) に関連付けられていたパッケージを選ぶオプションがあります。 このパッケージ フライトに使用する新しいパッケージをアップロードする場合は、アップロードする次のとおり ([通常の非 flighted の送信にアプリ パッケージをアップロードする場合と同じ手順](upload-app-packages.md)を使用して)。 このパッケージ フライトに含めるパッケージの指定が終わったら、**[保存]** をクリックします。
...
情報が表示され、ストアの点にも注意し、デバイスの家族の空き時間情報がない flighted 送信物に基づいています。 フライト グループ内のユーザーは、フライト以外の申請でサポートされているデバイス ファミリのみにアプリをダウンロードできます。 詳しくは、「[デバイス ファミリのサポート](#device-family-support)」をご覧ください。 
...
> [!IMPORTANT]
> パッケージの中のパッケージのロールアウト徐々 に、新しいパッケージを取得する割合に含まれていないユーザーが表示されますパッケージ前のパッケージ フライト提出から場合を除き、順位の高いフライトが使用できるようにする)。
...
パッケージを指定し、必要なオプションを構成したら、**[ストアに提出]** をクリックします。 パッケージ フライトに対して、[アプリの認定プロセス](the-app-certification-process.md)が開始されます。 パッケージ フライト パッケージに含まれているノートは、すべての送信と同様、 [Microsoft ストア ポリシー](https://docs.microsoft.com/legal/windows/agreements/store-policies)に準拠する必要があります。
...
既に公開したパッケージの飛行機の新しい送信を作成するには、アプリの概要ページ上のフライト名の横に**更新**をクリックします。 その後、フライト以外の申請と同様に新しいパッケージをアップロード (および不要なパッケージを削除) できます。 その他の必要な変更を加えた後、**[ストアに提出]** をクリックし、更新されたパッケージ フライトを送信して[アプリの認定プロセス](the-app-certification-process.md)を行います。

新しい更新を作成して申請せずに既存のフライトを変更するには、フライト名の横の **[変更]** をクリックします。 これにより、再度パッケージ フライトの認定プロセスを行うことなく、フライト グループ、フライト名、フライトの順位などの詳細を変更できます。 いる場合、進行中の更新プログラムがあるか、パッケージのフライトがまだ公開されたが変わっていない場合は、 **[変更**] オプションが表示されません。 
...
最初のパッケージ フライトを作成したら、上記で説明したプロセスに従って別のパッケージ フライトを作成します。 唯一の違いは、パッケージ フライトを既に 1 つ作成している場合、**[順位]** セクションですべてのパッケージ フライトの優先順位を指定する必要がある点です。 これにより、ストアにフライト グループの 1 つ以上ある場合、個々 の顧客に提供するパッケージを判断します。 順位が低いパッケージ フライトに高いバージョン番号のパッケージが含まれている場合でも、フライト グループのユーザーは、入手可能なパッケージ フライトのうち、常に順位が最も高いものを入手します。

既定では、新しいパッケージ フライトが最高の順位になります。 その順位を変更する場合、他のパッケージ フライトとの関係で適切な位置まで順位を下げる (またはもう一度上げる) ことができます。

非 flighted 送信物が常に、最低 (1) をランク付けされたことに注意してください。 つまり、どのフライト グループにも属さないユーザーは、フライト以外の申請のパッケージのみストアから入手できます。 飛行機のグループのユーザーが常にパッケージを取得する利用可能なパッケージの順位の高いフライト (がない非 flighted 送信、ランクがあるため)。 これにより、複数のフライト グループのメンバーとなっているユーザーにパッケージを配布する方法を柔軟に決めることができます。
