...
各プロモーション コードでは、コードを引き換えてアプリまたはアドオンを Microsoft Store からインストールするために、ユーザーがクリックしてできる、対応する一意の引き換え用 URL があります。  ユーザーがプロモーション コードを利用してアプリをインストールするには、アプリが[アプリの認定プロセス](the-app-certification-process.md)の最終的な公開フェーズに合格する必要があります。

単一用途のコードを生成する (して配布できます顧客ごとに 1 つ)、または指定したユーザー数を複数回使用できるコードを生成することもできます。
