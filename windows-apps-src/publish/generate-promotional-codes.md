...
各プロモーション コードでは、顧客がコードを実行し、Microsoft ストアからのアプリやアドオンをインストールするためにクリックして、対応する固有の出産 URL を持ちます。  ユーザーがプロモーション コードを利用してアプリをインストールするには、アプリが[アプリの認定プロセス](the-app-certification-process.md)の最終的な公開フェーズに合格する必要があります。

1 回だけのコードを生成する (して顧客ごとに 1 つを配布する)、か顧客の指定した数で複数回使用できるコードを生成することもできます。
...
> [!NOTE]
> プロモーション コードを使用するには、アプリが利用できる場合でも (つまり、選択した場合**ことこの製品を使用できるが、ストアのない見つけやすい****停止取得と: の直接リンクをユーザーが製品のストアを確認できます表示されている、のみ無償でダウンロードできます前に、製品を所有しているプロモーション コードまたは、Windows 10 デバイスを使用している**送信物の[クラス ノートブック検索機能](choose-visibility-options.md#discoverability)のセクション内のオプション)。 このオプションは、顧客必要があります (Xbox など)、Windows 10 のプロモーション コードを使用して、製品を取得するためにします。
...
3.  **[新しいプロモーション コードの注文]** ページで、次を入力します。
    ...
    -   プロモーション コードがアクティブになる時期を指定します。 特定の開始日時を選ぶには、**[コードは直ちにアクティブになります]** チェック ボックスをオフにします。 それ以外の場合、コード、アクティブになりますすぐ (ただし、製品コードを使用する顧客の注文で発行プロセスが完了する必要があります)。
...
3.  .tsv ファイルを好みのエディターで開きます。 最適な操作性のためには、Microsoft Excel など、表形式でデータを表示できるアプリケーションで .tsv ファイルを開くことをお勧めしますが、 任意のテキスト エディターでファイルを開くこともできます。
    ...
    -   **購入 URL**: ユーザーがコードを引き換えてアプリまたはアドオンをインストールするために使う URL。 URL は、次の書式: http://go.microsoft.com/fwlink/?LinkId=532540&mstoken=&lt; promotional_code >
...
> [!NOTE]
> 場合によっては、顧客が表示**を購入する**] ボタンを**インストールする**のではなくアプリがプロモーション コードを使って引き換えたが正常にも関わらずされます。 ユーザーは **[購入]** をクリックして、無料でアプリをインストールできます。
