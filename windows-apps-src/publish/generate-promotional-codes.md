...
ms.locfileid: "2918903"
---
# <a name="generate-promotional-codes"></a>プロモーション コードを生成する


Microsoft Store に公開したアプリまたはアドオンのプロモーション コードを生成できます。 プロモーション コードは、影響力を持つユーザーにアプリまたはアドオンへの無料アクセスを提供する簡単な方法です。 また、アプリやアドオンへの無料アクセスをユーザーに提供する顧客サービスのシナリオに対応するため、または Windows 10 の[ベータ テスト](beta-testing-and-targeted-distribution.md)のために、プロモーション コードを使うこともできます。 

各プロモーション コードには、コードを引き換えるし、マイクロソフトのストアから、アプリケーションやアドオンをインストールするために顧客がクリックしてできる、対応する一意の出産 URL があります。  ユーザーがプロモーション コードを利用してアプリをインストールするには、アプリが[アプリの認定プロセス](the-app-certification-process.md)の最終的な公開フェーズに合格する必要があります。

使い捨てのコードを生成します (して顧客ごとに 1 つの配布)、または複数回指定した顧客のために使用できるコードを生成することもできます。
...
> [!NOTE]
> アプリがお客様に利用可能でない場合でも、プロモーション コードを使用することができます (つまりを選択した場合**使用可能ですが、ストアにない検出可能なこの製品を作成****の位置の取得に: 直接のリンクを使用して、顧客が製品のストアを参照してください一覧については、それらが、ことができますのみダウンロードする前に、製品を所有している、またはプロモーション コードがあるし、10 の Windows デバイスを使用している場合**、提出書類の[検索](choose-visibility-options.md#discoverability)] セクションのオプション)。 このオプションを使用するお客様はプロモーション コードを使用して、製品を取得するために Windows の 10 (Xbox を含む) にする必要があります。


## <a name="order-promotional-codes"></a>プロモーション コードの注文

アドオンまたはアプリケーションのプロモーション コードを注文します。
...
3.  **[新しいプロモーション コードの注文]** ページで、次を入力します。
    ...
    -   プロモーション コードがアクティブになる時期を指定します。 特定の開始日時を選ぶには、**[コードは直ちにアクティブになります]** チェック ボックスをオフにします。 それ以外の場合、コードがアクティブになりますでしょうか (ただし、お使いの製品コードを使用する顧客の注文の発行プロセスが完了する必要があります)。
...
3.  .tsv ファイルを好みのエディターで開きます。 最適な操作性のためには、Microsoft Excel など、表形式でデータを表示できるアプリケーションで .tsv ファイルを開くことをお勧めしますが、 任意のテキスト エディターでファイルを開くこともできます。
    ...
    -   **購入 URL**: ユーザーがコードを引き換えてアプリまたはアドオンをインストールするために使う URL。 URL には次の形式: http://go.microsoft.com/fwlink/?LinkId=532540&mstoken=&lt; promotional_code >
...
> [!NOTE]
> 場合によっては、顧客を参照してください**購入**] ボタン**をインストール**するのではなく場合でも、アプリケーションはプロモーション コードを使用して正常に引き換えられました。 ユーザーは **[購入]** をクリックして、無料でアプリをインストールできます。
