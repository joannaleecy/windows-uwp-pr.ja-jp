...
各プロモーション コードでは、コードを引き換えてアプリまたはアドオンを Microsoft Store からインストールするためにユーザーがクリックできますが、対応する一意の引き換え用 URL があります。  ユーザーがプロモーション コードを利用してアプリをインストールするには、アプリが[アプリの認定プロセス](the-app-certification-process.md)の最終的な公開フェーズに合格する必要があります。
...
3.  .tsv ファイルを好みのエディターで開きます。 最適な操作性のためには、Microsoft Excel など、表形式でデータを表示できるアプリケーションで .tsv ファイルを開くことをお勧めしますが、 任意のテキスト エディターでファイルを開くこともできます。
    ...
    -   **購入 URL**: ユーザーがコードを引き換えてアプリまたはアドオンをインストールするために使う URL。 URL は次の形式: http://go.microsoft.com/fwlink/?LinkId=532540&mstoken=&lt; promotional_code >
