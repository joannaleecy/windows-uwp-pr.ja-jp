...
**個人的な対象ユーザー** ] オプションことができますのみ使えましていない発行済みアプリ一般向けの参加者にとき。 すべての OS のバージョンを対象とするアプリを使用してこのオプションを使用できますが、テスト担当者が Windows 10] (Xbox 1 つ) を含む 1607 以降のバージョンが実行されている必要がありますと、指定したメール アドレスに関連付けられている Microsoft アカウントでサインインする必要があります。
...
このオプションは、他のユーザーがストアにアプリを検索することを防ぐ一方のみを特定のグループにテスター、アプリの配布を制限する別の方法 (またはプロモーション コードを含まないことを取得する)。 ただし、プライベート対象ユーザー オプションとは異なり、他のユーザーが直接リンクを知っている場合はアプリの登録情報を参照できる可能性があります。 申請の機密性が重要な場合は、代わりにプライベート対象ユーザーに公開することをお勧めします。
