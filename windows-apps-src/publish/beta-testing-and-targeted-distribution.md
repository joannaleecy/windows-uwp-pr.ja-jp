...
**プライベート ユーザー**オプションことができますのみ使えましていない発行済みアプリ公開するとき。 任意の OS のバージョンを対象とするアプリケーションでこのオプションを使用することができますが、テスト担当者が Windows 10、1607 (を含む Xbox 1 つ)、以降のバージョンを実行する必要があり、提供した電子メール アドレスに関連付けられている Microsoft アカウントを使用して署名する必要があります。
...
このオプションには、ストア内のアプリを検出するを防ぐ他のユーザー中にテスト担当者の特定のグループのみにアプリケーションの配布を制限する別の方法が用意されています (またはプロモーション コードを使用せずに取得する)。 ただし、プライベート対象ユーザー オプションとは異なり、他のユーザーが直接リンクを知っている場合はアプリの登録情報を参照できる可能性があります。 申請の機密性が重要な場合は、代わりにプライベート対象ユーザーに公開することをお勧めします。
