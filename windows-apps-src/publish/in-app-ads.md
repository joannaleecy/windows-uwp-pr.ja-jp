...
4.  **[広告ユニットの種類]** ドロップダウンで、広告の種類を選択します。
    ...
    * アプリのバナー広告を表示する場合は、**見出し**を選択します。
    * アプリでビデオ スポットの広告やスポット バナー広告を表示する場合は、**ビデオにスポット**または**バナー スポット**(に表示するスポット広告の種類の適切なオプションを選択してください) を選択します。
    * アプリのネイティブ ad を表示する場合は、**ネイティブ**を選択します。
...
* アプリでは、[ネイティブ広告](../monetize/native-ads.md)が表示される場合は、これらの値を**NativeAdsManagerV2**コンスを渡します。
  > [!IMPORTANT]
...
> [!NOTE]
> Ad 単位過去 6 か月のアクティビティを持たない、ラベルを**非アクティブ**] としておされ、最終的に、ダッシュ ボードから削除されます。 フィルターを使用して "**アクティブ**" または "**非アクティブ**" の広告ユニットのみを表示することもできます。 誤って "**非アクティブ**" がマークされていると思われる広告ユニットを見つけた場合は、[サポートにお問い合わせください](http://aka.ms/storesupport)。

<span id="mediation" />

## <a name="mediation-settings"></a>仲介設定

[新しい UWP ad 単位を作成](#create-ad-unit)する、または[既存の UWP ad 単位の編集](#available-ad-units)ad 単位の[ad 仲介](../monetize/ad-mediation-service.md)を構成するのにはこのセクションのオプションを使用します。 広告仲介を使うと、複数の広告ネットワークから広告を表示して、広告収益とアプリ プロモーションの機能を最大限に引き出すことができます。表示される広告には、他の有料広告ネットワークからの広告や、Microsoft のアプリ プロモーション キャンペーン用の収益が生じない広告などが含まれます。 選択した広告ネットワークからのバナー広告要求の仲介は自動的に行われます。 アプリ内のバナー広告、スポット広告、またはネイティブ広告に既に関連付けられている UWP 広告ユニットがある場合は、広告仲介を有効にするためにアプリのコードを変更する必要はありません。
...
2. [**アプリの広告**] ページで、設定を**仲介 settings** ] セクションと構成に移動します。

    * 既定では、**[アプリに対して最適な広告仲介設定を Microsoft が選択する]** チェック ボックスがオンになります。 このオプションを使うことをお勧めします。 このオプションでは、アプリがサポートする各市場での広告収益を最大化できるように、機械学習アルゴリズムを使ってアプリの広告仲介設定を自動的に選択します。 このオプションを使用するときに、構成で使用する ad ネットワークができます。 Ad ネットワークの構成の一部であるしないし、アルゴリズムのようになりますアプリは、選択した ad ネットワークから広告をのみ受信をオフにします。
    * 独自の ad 仲介設定を選択する場合は、[**既定の設定の変更**] を選びます。

    > [!NOTE]
    > このセクションの残りの手順では、該当するは、**既定の設定を変更**する場合のみです。
...
|--------------|---------------|---------------------|
| こうと AppNexus |  パートナーを通じて広告こうと AppNexus ネットワークでは、サービスを提供するマイクロソフトが管理されている ad ネットワークです。<p/>**注**: こうと AppNexus が常にランク最初**支払い ad ネットワーク**の一覧のバナー ad 単位とこれらの種類の広告の下のランク付けは変更できません。 | バナー、ビデオ スポット広告 |
