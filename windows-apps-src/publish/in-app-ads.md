...
> [!NOTE]
> 広告ユニットには、過去 6 か月間アクティビティがなければ、私たちはラベルが**非アクティブ**としてと最終的に、ダッシュ ボードから削除します。 フィルターを使用して "**アクティブ**" または "**非アクティブ**" の広告ユニットのみを表示することもできます。 誤って "**非アクティブ**" がマークされていると思われる広告ユニットを見つけた場合は、[サポートにお問い合わせください](http://aka.ms/storesupport)。
...
2. **アプリ内広告**] ページの設定を構成、**仲介設定**] セクションに移動します。
    ...
    * 独自の広告仲介設定を選択する場合は、**既定の設定の変更**を選択します。
    ...
    > [!NOTE]
    > このセクションの残りの手順は、**既定の設定の変更**を選択した場合は、適用のみできます。
...
|--------------|---------------|---------------------|
| [Oath と AppNexus |  これは、Microsoft が管理する広告ネットワーク パートナーを通じて広告ネットワーク、[Oath や AppNexus です。<p/>**注**: [Oath and AppNexus は常に順位の高いまず**有料広告ネットワーク**の一覧で、バナー広告ユニットのとランクを下げるこれらの種類の広告は変更できません。 | バナー、ビデオ スポット広告 |
