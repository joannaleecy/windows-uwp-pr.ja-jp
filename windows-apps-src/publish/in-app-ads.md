...
2. **アプリ内広告**] ページで、設定を構成、**仲介設定**] セクションに移動します。
    ...
    * 独自の広告仲介設定を選択する場合は、**既定の設定の変更**を選択します。
    ...
    > [!NOTE]
    > このセクションの残りの手順は、**変更の既定の設定**を選択する場合のみです。
...
|--------------|---------------|---------------------|
| [Oath と AppNexus |  これは、Microsoft が管理する広告ネットワーク、パートナー様からの広告ネットワーク、[Oath や AppNexus です。<p/>**注**: [Oath and AppNexus は常に順位まず**有料広告ネットワーク**の一覧で、バナー広告ユニットのとこの種類の広告の下でランクを変更することはできません。 | バナー、ビデオ スポット広告 |
