...
4.  **[広告ユニットの種類]** ドロップダウンで、広告の種類を選択します。
    ...
    * アプリのバナー広告を表示する場合は、**バナー**を選択します。
    * アプリのビデオ広告のスポットやスポットのバナー広告を表示する場合は、**スポットのビデオ**または**バナーのスポット**(スポット広告を表示するの種類に応じた適切なオプションを選択することを確認する) を選択します。
    * **ネイティブ**アプリケーションでは、ネイティブの広告を表示する場合を選択します。
...
* アプリでは、[ネイティブの広告](../monetize/native-ads.md)が表示される場合は、これらの値を**NativeAdsManagerV2**コンス トラクターに渡します。
  > [!IMPORTANT]
...
> [!NOTE]
> 広告ユニットには、過去 6 か月の活動がなければ、私たちはラベルを**非アクティブ**と、最終的に、ダッシュ ボードから削除 フィルターを使用して "**アクティブ**" または "**非アクティブ**" の広告ユニットのみを表示することもできます。 誤って "**非アクティブ**" がマークされていると思われる広告ユニットを見つけた場合は、[サポートにお問い合わせください](http://aka.ms/storesupport)。

<span id="mediation" />

## <a name="mediation-settings"></a>仲介設定

[UWP の新しい広告ユニットを作成](#create-ad-unit)する、または[既存の UWP 広告単位を編集](#available-ad-units)広告ユニットの[広告の仲介](../monetize/ad-mediation-service.md)を構成するのにはこのセクションのオプションを使用します。 広告仲介を使うと、複数の広告ネットワークから広告を表示して、広告収益とアプリ プロモーションの機能を最大限に引き出すことができます。表示される広告には、他の有料広告ネットワークからの広告や、Microsoft のアプリ プロモーション キャンペーン用の収益が生じない広告などが含まれます。 選択した広告ネットワークからのバナー広告要求の仲介は自動的に行われます。 アプリ内のバナー広告、スポット広告、またはネイティブ広告に既に関連付けられている UWP 広告ユニットがある場合は、広告仲介を有効にするためにアプリのコードを変更する必要はありません。
...
2. **仲介の設定**] セクションで、構成には**アプリケーションでの広告**ページで、設定を移動します。

    * 既定では、**[アプリに対して最適な広告仲介設定を Microsoft が選択する]** チェック ボックスがオンになります。 このオプションを使うことをお勧めします。 このオプションでは、アプリがサポートする各市場での広告収益を最大化できるように、機械学習アルゴリズムを使ってアプリの広告仲介設定を自動的に選択します。 このオプションを使用する場合は、広告ネットワークの構成で使用するができます。 構成の一部として使用したくないし、アルゴリズムはアプリは、選択した広告ネットワークから広告をのみ受け取ることを確認する広告ネットワークをオフにします。
    * 仲介の設定に独自の広告を選択する場合は、**デフォルト設定の変更**を選択します。

    > [!NOTE]
    > このセクションの残りの手順は、**デフォルト設定の変更**を選択した場合に該当する場合のみです。
...
|--------------|---------------|---------------------|
| 誓いの言葉と AppNexus |  これは、当社のパートナーからの広告ネットワークでは、誓いの言葉と AppNexus サービスを提供する Microsoft 管理広告ネットワークです。<p/>**注**: 誓いの言葉と AppNexus は常にランクまず**支払い広告ネットワーク**] ボックスの一覧で、バナー広告の単位のと広告の種類はこれらの低いランクを変更できません。 | バナー、ビデオ スポット広告 |
...
| MSN のコンテンツの推奨事項 |  MSN のコンテンツの推奨事項から広告を提供するこのオプションを選択します。 |  バナー、バナー (スポット)  |
...
| Undertone | [Undertone](https://www.undertone.com/)から広告を提供するには、このオプションを選択します。 | スポット バナー |
