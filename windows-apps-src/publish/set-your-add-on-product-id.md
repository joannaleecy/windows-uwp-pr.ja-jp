...
アドオンで作成したアプリを関連付ける必要がある[パートナー センター](https://partner.microsoft.com/dashboard) (これはまだ送信していない) 場合でもです。 アプリの **[概要]** ページまたは **[アドオン]** ページに **[新しいアドオンを作成する]** ボタンがあります。
...
- **開発者が管理の消耗品**:残高とフルフィルメントは、アプリ内で管理する必要があります。 すべての OS バージョンでサポートされます。
- **ストアで管理された使用できます。** 残高は、すべての Windows 10 バージョン 1607 以降を実行しているお客様のデバイスで Microsoft によって追跡されます。以前の OS バージョンではサポートされていません。 このオプションを使うには、Windows 10 SDK バージョン 14393 以降を使用して親製品をコンパイルする必要があります。 (パートナー センターで、送信を作成し、いつでも作業を開始できます) ただし、親製品が公開されているまでストアに、ストア管理の消耗アドオンを送信することはできませんにも注意してください。 申請の**プロパティ**のステップで、ストアで管理されるコンシューマブル アドオンの数量を入力する必要があります。
...
(ただし、パートナー センターで、送信を作成し、いつでも作業を開始できます) にサブスクリプションのアドオンをストアに発行できる前に、親製品を提出する必要があります。

## <a name="product-id"></a>製品 ID

どの製品の種類を選んでも、アドオンに割り当てる一意の製品 ID を入力する必要があります。 この名前はパートナー センターのアドオンを識別するために使用してするには、この識別子を使用する[コードでアドオンを参照してください](../monetize/in-app-purchases-and-trials.md#how-to-use-product-ids-for-add-ons-in-your-code)します。
...
-   製品 ID には、次の文字を含めることはできません:  **&lt; &gt; \* % &: \\ ? +、**
-   顧客が製品 ID に表示されません。 (後で、お客様に表示される[タイトルと説明](create-add-on-descriptions.md)を入力できます)。
-   以前に発行アプリは、Windows Phone 8.1 をサポートしているか以前では、する必要がありますのみで使用する文字の英数字、ピリオド、およびアンダー スコア、製品の id。 その他の種類の文字を使った場合、Windows Phone 8.1 以前を実行しているお客様はそのアドオンを購入できなくなります。
