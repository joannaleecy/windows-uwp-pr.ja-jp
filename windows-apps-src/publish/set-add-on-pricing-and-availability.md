...
(**表示設定**] セクションで、**購入を停止する**オプションをオンにした) しない限り、アドオンの基本価格を選択してください。 既定の選択は**無料**、有料のアドオンの場合は、必ず (.99 USD から開始) 使用可能な価格階層の 1 つ選択します。

価格変更をスケジュールして、アドオンの価格を変更する日時を指定することもできます。 さらに、これらの変更を特定の市場向けにカスタマイズすることもできます。 

> [!TIP]
> サブスクリプションのアドオン] の後で送信の基本価格が高いを選択するか、価格変更の価格を向上させることのスケジュールを設定して、アドオンを発行した後の価格を上げることができません。 低い価格を使って、次のいずれかを選択することができますと価格が低下することはできませんその新しい価格よりも高いさせる。 このためは、特に重要なは、適切な価格の層のサブスクリプションのアドオンを選択することを確認します。 
