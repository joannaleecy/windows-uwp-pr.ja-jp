...
**最初にタッチ**Windows は、マウスの機能をエミュレート以上だけが一意であり、独自のタッチ エクスペリエンスを提供します。
...
-   **トースト通知**、ユーザーがトースト通知によって、期間限定または関連する個人のコンテンツについて知っておくし、アプリが閉じているときでも、アプリをもう一度招待したりできるようにします。 詳しくは、[タイル、バッジ、トースト通知](../design/shell/tiles-and-notifications/index.md)についてのページをご覧ください。
...
-   **ローミング**ユーザー タスク右を中断した場所を選択できるようにが最も重視を使用しているデバイスに関係なく UX を保持するデータのローミングによってデバイス間で継続的なエクスペリエンスを作成します。 設定と状態をローミングによって維持することで、キッチンで家族が使う PC、仕事用 PC、個人用のタブレット、その他のフォーム ファクターなど、どこにいてもアプリを快適に使えるようにします。 詳しくは、「[アプリケーション データの管理](../design/app-settings/store-and-retrieve-app-data.md)」と「[アプリケーション データのローミングのガイドライン](https://msdn.microsoft.com/library/windows/apps/hh465094)」をご覧ください。
...
-   **カメラと外部記憶装置**、内蔵カメラまたは接続されているカメラ チャットや電話会議、ビデオ、写真の撮影プロファイルを示し、世界の文書化がログの記録、ユーザーを接続または活動、アプリがするに最適です。 詳しくは、[リムーバブル ストレージのコンテンツへのアクセス](https://msdn.microsoft.com/library/windows/apps/hh465189)についてのページをご覧ください。
...
**コマンドをどのようにまとめるかどうか**。用に設計する必要がある可能性があるコマンドを識別する大まかなフロー手順を使用します。 次に、アプリのどこでコマンドを使うかを考えます。

-   **常に、コンテンツを使用してみてください**。可能であれば、使用すると、ユーザーは、アプリのキャンバス上のコンテンツを直接操作ではなく、コンテンツで機能するコマンドを追加します。 たとえば、旅行アプリで旅行計画を編集するときに、リスト内のアクティビティを上下に移動するコマンド ボタンを使うのではなく、キャンバスのリスト上でアクティビティをドラッグ アンド ドロップできるようにします。
...
**各ビューでアプリをレイアウトする方法を決定します**。Windows アプリを最小幅を全画面表示から任意の幅にサイズ変更をサポートして横向きと縦向きをサポートしています。 アプリは、任意のサイズの任意の画面で、どの向きでも適切に表示され、機能することが望まれます。 つまり、さまざまなサイズとビューについて、UI 要素のレイアウトを計画する必要があります。 そうすることで、ユーザーのニーズと好みに合わせてアプリ UI が柔軟に変化します。
...
**初めての起動**ユーザーをサービスにサインアップするには、自分のアカウントにログインまたは独自のコンテンツを追加、前に何が表示されるかどうか。 ユーザーに情報を求める前に、アプリの価値をアピールします。 アプリを使ってもらうためには、ユーザーが自由に見ることのできるサンプル コンテンツを表示して、どのようなアプリなのかを知ってもらうのもよいでしょう。
