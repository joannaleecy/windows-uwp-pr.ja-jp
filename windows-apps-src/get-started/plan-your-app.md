...
-   **再生**楽しむことができます、オーディオ、ビデオ、またはアプリからホーム ネットワーク内の他のデバイスにストリーミングされた画像。
...
**さまざまなビュー、フォーム ファクターやハードウェア構成**Windows で構えるので、アプリでユーザーを設定します。 アプリの UI は、ユーザーが使うあらゆるデバイス、入力モード、向き、ハードウェア構成、状況で適切に表示される必要があります。

**最初にタッチ**Windows には、マウスの機能をエミュレート単に複数の一意と特徴的なタッチ エクスペリエンスを提供が用意されています。
...
**魅力と新鮮さ**、アプリが感じられし、これらの標準的なエクスペリエンスを持つユーザーを適用してください。

-   **アニメーション**により、ユーザー、アプリが高速かつ滑らかにアニメーションのライブラリを使用します。 コンテキストの変化がわかりやすく、視覚的な切り替えがエクスペリエンスに結び付きます。 詳しくは、[UI のアニメーション化](../graphics/animations-overview.md)についてのページをご覧ください。
...
-   **地理位置情報**標準的な web データからまたは地理位置情報センサーからの地理位置情報を使用して、を取得する、地図上での位置を検索または近くのユーザー、アクティビティ、および目的地通知を受け取るため、ユーザーを支援します。 詳しくは、[地理位置情報](https://msdn.microsoft.com/library/windows/apps/hh465139)についてのページをご覧ください。
...
**コマンドをどのようにまとめるかどうか**。用に設計する必要がある可能性があるコマンドを識別する大まかなフロー手順に従ってを使用します。 次に、アプリのどこでコマンドを使うかを考えます。

-   **常に、コンテンツを使用してみてください**。可能であれば、使用すると、ユーザーは、アプリのキャンバス上のコンテンツを直接操作ではなくコンテンツに機能するコマンドを追加します。 たとえば、旅行アプリで旅行計画を編集するときに、リスト内のアクティビティを上下に移動するコマンド ボタンを使うのではなく、キャンバスのリスト上でアクティビティをドラッグ アンド ドロップできるようにします。
...
**スプラッシュ画面**スプラッシュ画面のできるだけ短時間で読み込むし、いる限り、アプリの状態を初期化する必要がある画面上に維持する必要があります。 スプラッシュ画面の表示内容で、アプリの特徴を伝えます。
