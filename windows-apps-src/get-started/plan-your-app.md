...
**について、アプリとは?** まず、大まかなコンセプトを決めます。ユーザーがアプリで何をできるようにするか、一覧にまとめてみましょう。
...
**アプリに最適とは** すべてのアイデアを全体的に見て、特に目立つシナリオがないか考えてみます。 数多くのアイデアの中から絞り込んで、これに集中しようと思えるシナリオを 1 つだけ選びます。 優れたアイデアをたくさん捨てることになりますが、その 1 つのシナリオを良いものにしあげるには、いさぎよくアイデアを捨てることが肝心です。

シナリオを 1 つ選んだら、アプリの 1 番の特徴を普通の人に 1 行で伝えるにはどうしたらよいかを考えます。 次に、例を示します。
...
-   フローを示してください。何か早い方を今後でしょうか。
-   ストーリー ボード フロー:必要がありますユーザーの行動、UI を使ってフローを完了するでしょうか。
-   プロトタイプ:クイック プロトタイプを含むフローを試してください。

**ユーザーべき操作を実行できますか。** たとえば、旅行アプリを使うと "友人どうしで協力してグループ旅行の計画を作り上げる" ことができます。 必要なフローをリストにまとめましょう。
...
新しい開発者は、いくら強調しても十分。優れたアプリは反復プロセスです。 初期段階から何度もプロトタイプを作ることをお勧めします。 創造力を養う努力と同様、最適なアプリは徹底的な試行錯誤による成果です。
...
-   プラットフォームの研究:プラットフォームで提供される機能し、その使用方法を紹介します。
-   図の関連付け:機能を使用してフローを接続します。
-   プロトタイプ:必要なものには、機能を実行します。

**アプリ コントラクト**  アプリは、広範なアプリ間、機能間のユーザー フローを有効にするアプリ コントラクトに参加できます。

-   **共有**  ユーザーが他のアプリから他のユーザーと、アプリからのコンテンツを共有し、すぎる、他のユーザーやアプリから共有可能なコンテンツを受信できるようにします。
-   **再生する**  ユーザーは、オーディオ、ビデオ、またはアプリから、ホーム ネットワークの他のデバイスにストリームのイメージを利用できます。
-   **ファイル ピッカーやファイル ピッカー拡張子**  ユーザーを読み込むし、ローカル ファイル システム、接続された記憶域デバイス、ホーム グループ、またはその他のアプリからそのファイルを保存できます。 また、ファイル ピッカーの拡張機能を使って、アプリのコンテンツを他のアプリに読み込むこともできます。
...
**さまざまなビュー、フォーム ファクター、およびハードウェア構成**  Windows では、料金と、forefront でアプリにユーザーを格納します。 アプリの UI は、ユーザーが使うあらゆるデバイス、入力モード、向き、ハードウェア構成、状況で適切に表示される必要があります。

**タッチから開始する**  Windows マウス機能をエミュレートだけ一意かつ特徴的なタッチ エクスペリエンスを提供します。
...
**魅力的で新しい**  するアプリは新しい感覚し、これらの標準的なエクスペリエンスを持つユーザーが関与します。 確認してください。

-   **アニメーション**  アニメーションのライブラリを使用して、ユーザーの高速で滑らかなアプリをします。 コンテキストの変化がわかりやすく、視覚的な切り替えがエクスペリエンスに結び付きます。 詳しくは、[UI のアニメーション化](../graphics/animations-overview.md)についてのページをご覧ください。
-   **トースト通知**  ユーザーがトースト通知、経由で時間を区別する、または個人に関連するコンテンツについて知っておくし、アプリが閉じているときでもアプリケーションに招待できます。 詳しくは、[タイル、バッジ、トースト通知](../design/shell/tiles-and-notifications/index.md)についてのページをご覧ください。
-   **アプリ タイル**  をアプリにユーザーを引き込む新しいと関連する更新プログラムを提供します。 次のセクションで詳しく説明します。 詳しくは、「[アプリ タイル](../design/shell/tiles-and-notifications/creating-tiles.md)」をご覧ください。

**パーソナル化**

-   **設定**  ユーザーがアプリの設定を保存することによって、必要なエクスペリエンスを作成できます。 すべての設定を 1 つの画面にまとめて、ユーザーが使い慣れた一般的なメカニズムでアプリを構成できるようにします。 詳しくは、「[アプリ設定の追加](../design/app-settings/app-settings-and-data.md)」をご覧ください。
-   **ローミング**  ローミング ユーザーのタスク権限には、中断した場所を取得できるようにし、関心のある最もを使用しているデバイスに関係なく UX を保持するデータをデバイスにわたって継続的なエクスペリエンスを作成します。 設定と状態をローミングによって維持することで、キッチンで家族が使う PC、仕事用 PC、個人用のタブレット、その他のフォーム ファクターなど、どこにいてもアプリを快適に使えるようにします。 詳しくは、「[アプリケーション データの管理](../design/app-settings/store-and-retrieve-app-data.md)」と「[アプリケーション データのローミングのガイドライン](https://msdn.microsoft.com/library/windows/apps/hh465094)」をご覧ください。
-   **ユーザー タイル**  によりアプリより自由をユーザーに、ユーザー タイル イメージの読み込みまたはユーザーが Windows 全体で、個人のタイルとして、アプリからのコンテンツを設定できるようにします。

**デバイス機能**  アプリが現在のデバイスの機能を最大限に活用してください。

-   **近接ジェスチャ**  ユーザーが物理的に近接、物理的にまとめて、デバイスをタップして (マルチ プレーヤー ゲーム) の他のユーザーとデバイスを接続できるようにします。 詳しくは、「[近接通信とタップのサポート](https://msdn.microsoft.com/library/windows/apps/hh465229)」をご覧ください。
-   **カメラおよび外部記憶装置**  チャットと会議、vlogs の世界を文書化、プロファイル写真の取得を記録する、組み込みまたは接続されているカメラにユーザーを接続または任意のアクティビティ、アプリ点で優れています。 詳しくは、[リムーバブル ストレージのコンテンツへのアクセス](https://msdn.microsoft.com/library/windows/apps/hh465189)についてのページをご覧ください。
-   **加速度計などのセンサー**    今日ではデバイスがセンサーの数に付属します。 アプリでは、環境光に応じてディスプレイの明るさを調節したり、ユーザーがディスプレイの向きを変えたときに UI を自動的に再配置したり、物理的な動きに応じて処理を行ったりできます。 詳しくは、[センサー](../devices-sensors/sensors.md)についてのページをご覧ください。
-   **地理的位置情報**  を回避、地図上の位置を検索または近くの人、アクティビティ、および変換先の通知を受け取るため、ユーザーを標準的な web データや地理的位置情報センサーからの地理的位置情報の情報を使用しています。 詳しくは、[地理位置情報](https://msdn.microsoft.com/library/windows/apps/hh465139)についてのページをご覧ください。

旅行アプリの例についてもう一度考えてみましょう。 友人どうしで協力してグループ旅行の計画を作り上げるアプリには、次のような機能があるとよいかもしれません。

-   共有：ユーザーは、今後の乗車と、友人や家族と前のトリップ興奮を共有する複数のソーシャル ネットワークにそのスケジュールを共有します。
-   ［検索］:ユーザーは、アクティビティまたは他のユーザーの共有またはパブリック スケジュール、独自のトリップで含めることができますをから変換先を検索しています。
-   通知:ユーザーは、旅行の仲間のスケジュールを更新するときに通知されます。
-   設定:どのトリップは、通知を起動する必要がありますまたはソーシャル グループのユーザーのスケジュールの検索ができるように、ユーザーは自分の好みにアプリを構成します。
-   セマンティック ズームします。ユーザーは、スケジュールのタイムライン間を移動し、拡大を計画した後、アクティビティのリストの詳細を参照してください。
-   ユーザー タイル:ユーザーは、旅行を友人と共有するときに表示される画像を選択します。
...
**UI のコンテンツを整理する方法**   ほとんどのアプリ コンテンツは、何らかの形式のグループまたは階層に整理できます。 コンテンツの最上位グループの内容は、1 番の特徴の説明文に沿ったものにします。
...
**UI のコンテンツを表示するにはどうする必要がありますか。** UI をまとめる方法が決まったら、UI がどのように構築され、ユーザーに表示されるかを指定する、UX の目標を定義できます。 どのシナリオでも、できるだけ早くユーザーがアプリを楽しんで使い続けることができるようにする必要があります。 これを行うには、最初にユーザーに表示する UI の部分を決めて、その部分を完成させてから、その他の重要でない部分の構築に時間を使う必要があります。
...
**必要などのような UI の外観とコマンド。**   以前識別したフローを確認します。 各フローについて、ユーザーが行う手順を大まかに作成します。
...
**どのような必要がありますフローと感じるでしょうか。** ユーザーの手順を定義したら、そのフローをパフォーマンスの目標にします。 詳しくは、「[パフォーマンスの計画](../debug-test-perf/planning-and-measuring-performance.md)」をご覧ください。

**コマンドを整理する方法**  を設計する必要がある潜在的なコマンドを識別するために、フロー手順のアウトラインを使用します。 次に、アプリのどこでコマンドを使うかを考えます。

-   **常にコンテンツを使用してください。**  可能であれば、ユーザーがアプリのキャンバス上のコンテンツを直接操作ではなく、コンテンツに対して作用するコマンドを追加します。 たとえば、旅行アプリで旅行計画を編集するときに、リスト内のアクティビティを上下に移動するコマンド ボタンを使うのではなく、キャンバスのリスト上でアクティビティをドラッグ アンド ドロップできるようにします。
-   **場合は、コンテンツを使用することはできません。** コンテンツを直接操作できない場合は、コマンドを次の UI サーフェスのいずれかに配置します。

    -   [コマンド バー](https://msdn.microsoft.com/library/windows/apps/hh465302):ほとんどのコマンドは、ユーザーがタップして表示されるまで、通常は隠されているコマンド バーに配置する必要があります。
    -   アプリのキャンバス。ページまたはビューを 1 つの目的を持つユーザーにある場合は、そのため、キャンバス上で直接コマンドを行うことができます。 しかし、このようなコマンドはなるべく作らないでください。
    -   [コンテキスト メニュー](https://msdn.microsoft.com/windows/uwp/controls-and-patterns/menus):クリップボード操作 (切り取り、コピー、および貼り付け) などのコンテキスト メニューを使用するまたは適用されるコマンドのコンテンツは選択できません (など、マップ上の場所へのピンの形の追加) します。

**それぞれのビューで、アプリのレイアウト方法を決定します。**   Windows アプリを最小幅を全画面表示から任意の幅にサイズ変更をサポートして横と縦向きをサポートしています。 アプリは、任意のサイズの任意の画面で、どの向きでも適切に表示され、機能することが望まれます。 つまり、さまざまなサイズとビューについて、UI 要素のレイアウトを計画する必要があります。 そうすることで、ユーザーのニーズと好みに合わせてアプリ UI が柔軟に変化します。
...
**スプラッシュ スクリーン**  スプラッシュ スクリーンする必要があります、できるだけ高速に読み込むし、いる、アプリの状態を初期化する必要がある限りのみ、画面上に残ります。 スプラッシュ画面の表示内容で、アプリの特徴を伝えます。

**最初に起動**  ユーザー、サービスにサインアップするには、お子様のアカウントにログインしますまたは、独自のコンテンツを追加、前には何が表示されますか?。 ユーザーに情報を求める前に、アプリの価値をアピールします。 アプリを使ってもらうためには、ユーザーが自由に見ることのできるサンプル コンテンツを表示して、どのようなアプリなのかを知ってもらうのもよいでしょう。

**ホーム ページ**  ホーム ページを起動するユーザー、アプリを起動するたびにします。 ここに表示されるコンテンツは、目的をはっきりさせ、アプリの特徴がすぐにわかるようにする必要があります。 このページでは、1 番の特徴だけを強調します。アプリの他の部分はユーザーが探し出してくれることを期待しましょう。 ランディング ページでは、ユーザーがアプリの特徴に気付きやすくするのではなく、散漫な印象を与えないようにすることを重視してください。

## <a name="validate-your-design"></a>設計を検証する

設計のやり直しを避けるために、アプリの開発があまり進まないうちに、設計内容またはプロトタイプをガイドライン、ユーザーの印象、要件に照らして検証します。 各機能には、一連のアプリでは、ポーランド語に役立つ UX ガイドラインと、一連の Microsoft Store でアプリを発行するために満たす必要がありますストアの要件があります。 [Windows アプリ認定キット](https://developer.microsoft.com/windows/develop/app-certification-kit)を使って、ストア要件を技術的に満たすかどうかのテストを実行できます。 また、Microsoft Visual Studio のパフォーマンス ツールを使って、すべてのシナリオでユーザーに快適なエクスペリエンスを提供しているかどうかを確認できます。
