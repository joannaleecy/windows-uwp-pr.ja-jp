...
XDK デベロッパーである場合やクロスプレイ タイトルの開発を行う場合は、XDP で統計を作成します。  Windows 10 で実行されている、純粋な UWP を行う場合は、パートナー センターで統計を作成します。
...
統計情報を定義するクロス play タイトルではない Windows 10 での UWP を開発している場合[パートナー センター](https://partner.microsoft.com/dashboard)します。 読み取り、[パートナー センターの統計情報の構成に関する記事](../leaderboards-and-stats-2017/player-stats-configure-2017.md)をパートナー センターでの統計情報を構成する方法について説明します。

> [!NOTE]
> に関する情報をそのダムを要請する必要があります stats 2013 開発者[Stats 2013 構成](https://developer.microsoft.com/en-us/games/xbox/docs/xdk/windows-configure-stats-2013)で[パートナー センター](https://partner.microsoft.com/dashboard)します。
...
アプリケーションをリアルタイムのアクティビティ (RTA) Xbox 開発者ポータル (XDP) またはパートナー センターで構成されている統計を変更するときに、更新プログラムを取得するをサブスクライブします。
...
> [!IMPORTANT]
> サービスは、使用の 2 時間後に切断されますリアルタイム アクティビティ、コードはこれを検出し、まだ必要な場合、リアルタイムのアクティビティのサービスへの接続を再確立できる必要があります。 これは主に切れると、認証トークンを更新することです。
> 
> クライアントでは、マルチ プレーヤーのセッションでは、RTA を使用し、30 秒間、切断された、マルチプレイヤー セッション Directory(MPSD) RTA セッションが閉じられるし、ユーザー、セッションからサインアウトを開始を検出します。 RTA クライアントは、接続が閉じられたときに検出し再接続を開始し、MPSD がセッションを終了する前に再登録することです。