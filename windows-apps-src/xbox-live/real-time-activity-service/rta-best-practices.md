...
RTA 接続は、これにより、クライアントが再接続する設計上、2 時間後に閉じられます。 これはメッセージの帯域幅を保存する接続の認証トークンがキャッシュされるためです。 最終的にそのトークンの期限が切れます。 接続を閉じると、強制的に再接続するクライアントによって、クライアントは認証トークンの更新が強制されます。
...
> [!WARNING]
> クライアントがマルチ プレーヤーのセッションでは、RTA を使用しが 30 秒間、切断されている場合、[マルチプレイヤー セッション Directory(MPSD)](../multiplayer/multiplayer-appendix/multiplayer-session-directory.md) RTA セッションが閉じると、ユーザー、セッションからサインアウトを開始であることを検出します。 RTA クライアントは、接続が閉じられたときに検出し再接続を開始し、MPSD がセッションを終了する前に再登録することです。
