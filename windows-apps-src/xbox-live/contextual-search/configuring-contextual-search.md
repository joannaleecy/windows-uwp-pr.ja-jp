...
タグ付けに使用するイベントや統計の設定が済んでいれば、コンテキスト検索のためにタイトルを構成する作業は簡単です。  既によくわからない場合は、コンテキスト検索の設定の他の既存の XDP またはパートナー センターのドキュメントを参照してください。
...
このページで、それぞれの統計は次のように表されます。優先度の表示名 (Stat インスタンス名)
...
1. 統計インスタンスを選択します。
  ...
  統計インスタンスのみがサポートされていることに注意してください。統計テンプレートは使用できません。  (XDP またはパートナー センターの統計情報の部分で構成されている) 状態のインスタンスに設定した可視性を意識する必要があります。  **[開く]** とマークされた統計のみがサード パーティー エクスペリエンスに表示されます。
...
現時点では配信 (揮発性のデータ) のみがサポートされているため、現状ではコンテキスト検索ドキュメント上で統計を容易に変更できます。  (2015 年 9 月に) ゲーム クリップのタグ付けのサポートが開始された後は、統計を追加または削除する場合に、既にインデックス付けされているクリップに影響を及ぼすことになります。  新しい統計を追加する場合、変更の反映後に作成されたゲーム クリップおよびブロードキャストのみがタグ付けされ、新しい統計を使用したクエリで表示されます。以前のゲーム クリップに対しては、この統計からクエリを実行することはできません。コンテキスト検索ドキュメントから統計を削除した場合、ゲーム クリップにアタッチされていた以前の統計は不要になり、返されなくなります。
