...
> [!NOTE]
> この記事では、サンド ボックスの詳細な使用方法を説明し、主に、複数のチームと複雑なアクセス許可の要件が含まれる大規模ゲーム スタジオに適用されます。  Xbox Live クリエーターズ プログラムに参加している場合、または ID@Xbox 開発者である場合は、「[Xbox Live のサンドボックスの概要](xbox-live-sandboxes.md)」をお読みになることをお勧めします。
...
サンドボックスは、運用環境内のデータをパーティション分割するための方法です。 Xbox 360 の ERA サービスでは、PartnerNet と ProductionNet は 2 つの異なる環境です。 Xbox One の時代 (年号) サービスで、1 つの運用環境が含まれます*n*個別の仮想環境を仮想環境ごとにサンド ボックスが呼び出された場合。 すべてのコンテンツを対象とした単一の運用環境が存在するので、サンドボックスは実際には、ある環境で生成されたデータが別の環境に移動できない一意の仮想環境です。
...
Microsoft によってのみ設定できます 2 つのシステムのサンド ボックスがあります。証明書と小売します。 名前が示すように、CERT サンドボックスはリリース前の認定を受けているタイトル用のサンドボックスであり、RETAIL サンドボックスはすべての製品ユーザーおよび製品デバイスがアクセスできる実際の製品を表しているサンドボックスです。
...
タイトルは、Xbox 開発者ポータル (XDP) またはパートナー センターで生まれました。 このドキュメントでは、XDP で生まれたタイトルについて説明します。 タイトルには、タイトル、ID、製品 ID、およびサービスの構成 ID (SCID) が割り当てられます。
...
XDP にタイトルを作成するにはパブリッシャーは製品グループを作成、製品グループのジャンルを指定し、し、その中の個々 の製品を作成します。 (詳細については、ドキュメントを参照して、XDP。)次の図は、製品グループ、製品、製品のインスタンス、およびサンド ボックス間のリレーションシップを示しています。
...
> [!NOTE]
> 実行時にアクセスできるようにするにはサンド ボックスで、product インスタンスを配置する必要があります。 ビルドがサンドボックスに配置されると、そのサンドボックスへのアクセスを許可されている XDP ユーザーや開発キット デバイスは、インスタンスを実行することができます。 実行するために、これらのユーザーやデバイスは、各自の開発者アカウント (実行時アクセスのための仮想ユーザーとして機能する特別なアカウント) の 1 つを使用して Xbox 本体から Xbox One にログオンします。
...
-   XDP アカウントまたは開発アカウントを作成したパートナー センター アカウントに関連付けられます。
...
> [!NOTE]
> サンド ボックスへのアクセスに作成されるユーザー グループは、製品グループと製品の XDP で構成データにアクセスできないように使用される同じユーザー グループです。
...
サンド ボックス ID が (大文字と小文字) では、次の形式で文字列を示します。&lt;PublisherMoniker&gt;.*n*します。 XLDP.5 というサンドボックス ID の例について、次に説明します。
...
-   *パブリッシャー サンドボックス*。 公開元はそれぞれの開発中サンドボックスにアクセスできます。 これら XLDP.0、XLDP.1、XLDP.2、XLDP.3 などのようになります。これは、発行元は、タイトルの製品のインスタンスを記述する場所です。 これらのサンドボックスへのアクセスは、公開元がアクセスを許可するユーザー/デバイスに限定されます。

-   *Microsoft サンドボックス*。 これらは、組み込みのサンド ボックスです。小売り業と証明書のみ Microsoft がこれらの保護されたサンド ボックスに公開できるとしています。
...
認定の中に失敗したすべての製品インスタンスは、XDP またはパートナー センターを使用しているパブリッシャーで固定し、デバッグに開発サンド ボックスに戻ることができます。
...
コンテンツの分離での実行時アクセス管理のみ以下の例です。

### <a name="scenario-1-two-titles-one-sandbox"></a>シナリオ 1:2 つのタイトル、1 つのサンド ボックス
...
### <a name="scenario-2-one-title-different-teams"></a>例 2:1 つのタイトル、さまざまなチーム
...
次の図は、TitleX が 2 つの製品のインスタンスを持つことを示しています。PI 1 および PI 2。 製品インスタンスはサンドボックス内に存在する必要がありますが、同じタイトルの 2 つの製品インスタンスが同じサンドボックス内に存在することはできません。 そのため、TitleX-PI-1 はサンドボックス XLDP.1 内にあり、TitleX-PI-2 はサンドボックス XLDP.2 内にあります。
...
### <a name="scenario-3-two-titles-completely-separate"></a>例 3:完全に独立した 2 つのタイトル
...
### <a name="scenario-4-anyway-you-like-it"></a>シナリオ 4:などのか
...
## <a name="determine-the-sandbox-your-device-is-targeting"></a>デバイスが対象とするサンド ボックスを確認します。

Xbox Live Api には、実行時に、タイトルが対象とするどのようなサンド ボックスを表示できるアプリ構成シングルトンが含まれています。 これを行うへのアクセスを**サンド ボックス**プロパティの`xbox::services::xbox_live_app_config`します。
...
> [!NOTE]
> ユーザーがサインインするまで、サンド ボックスのプロパティは、値を付与しません。
