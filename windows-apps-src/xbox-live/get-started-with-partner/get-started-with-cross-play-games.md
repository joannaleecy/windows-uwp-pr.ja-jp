...
MSA アプリ ID を [アプリケーション ID] フィールドに入力したら、[Submit Application ID Setup] (アプリケーション ID 設定の提出) ボタンをクリックします。 これにより、Xbox Live セキュリティで MSA アプリ ID の情報が保存されます、UWP から、XToken を取得する要求を行うたびに含まれているタイトル クレームにマップされますこの XDP 製品である限り、UWP が作成した AppX マニフェスト Id を使用して正しくd パートナー センターで!)
...
そのためには、担当の DAM に連絡し、タイトル名に対して Xbox アプリのタイトル ID を更新する必要があることを伝えてください。  XDP で作成されたタイトル ID を含めるようにしてください (XDP 製品のセットアップの下に表示される&gt;製品の詳細) と UWP 用 Windows 10 用の URL (アプリの管理下にパートナー センターで表示&gt;アプリ id)。
...
<table>
  <tr>
    <td>
      UWP 専用ゲームの場合に公開する XDK タイトル バイナリがあるない場合でも、パートナー センターでの申請を開始する前に XDP での証明書をカタログとサービス構成を公開する必要です。
    </td>
  </tr>
</table>
...
2.  製品の認定ページで、パートナー センターで「このアプリは手動で公開」、されている場合は、"を今すぐ公開] を選択するかを選択した"\[date\]"である場合に発生するのには公開待ちます。
...
<table>
  <tr>
    <td>
      UWP 専用ゲームの場合は、カタログに公開する必要とサービス構成を RETAIL に公開が完了するとパートナー センターで、それ以外の場合、リリースされた UWP 前に、XDP では、Xbox Live にアクセスできません。
    </td>
  </tr>
</table>
