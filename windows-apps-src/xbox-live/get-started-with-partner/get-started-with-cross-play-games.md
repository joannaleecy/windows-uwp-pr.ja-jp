...
2.  パートナー センター:[ポータル](https://partner.microsoft.com/dashboard)UWP 開発今日とセットアップの Xbox Live 構成 UWP で使うのためのアプリ id を予約することができます。
...
3.  パッケージ ファミリ名 (PFN): これは、パートナー センターで作成された各製品に割り当てられる id です。 このパートナー センターの製品の id を UWP をバインドすると、この pfn が割り当てがかかります。 PFN は一意の製品 ID であり、複数のプラットフォームに対応する場合があります。 PFN は Xbox タイトル ID と 1 対 1 に対応します。

4.  MSA アプリ ID: は、MSA クライアント ID とも呼ばれる、パートナー センターでは、製品の作成時に、MSA によって割り当てられるもう 1 つのアプリ id です。 この ID によって、Microsoft サービスはアプリを識別することができます。 MSA アプリ ID は PFN と 1 対 1 に対応します (したがって Xbox タイトル ID とも 1 対 1 に対応します)。
...
1.  **XDK タイトルは XDP に取り込む必要があります**。 両方、サービス構成および公開メインラインのエクスペリエンスから、パートナー センターは XDK ベースのタイトルをサポートするためを備えていません。

2.  **XDP で作成した 1 つのサービス構成を、ゲームの XDK バージョンと UWP バージョンの両方で使用できます**。 ゲームの XDK バージョンと UWP バージョンで 1 つのサービス構成を共有できる新しい機能が XDP に追加されています。 UWP バージョンは、パートナー センターでパッケージ用に公開する必要があります/カタログ、サービス構成の公開はすべて XDP で行うことができます。

3.  **サービス構成を XDP とパートナー センターで分けることはできません**。 XDP とパートナー センターでは、1 つが他方で既に公開されているから、もう一方で公開の認識しません。 これにより、サービス構成が復元できないほど壊れたり、非常に悪いユーザー エクスペリエンス (実績やセーブ データの消失など) が生じる可能性があります。これを避けるため、XDK+UWP のクロスプレイ ゲーム用のサービス構成は、すべて XDP で行う必要があります。

### <a name="create-your-uwp-product-in-partner-center"></a>パートナー センターで UWP 製品を作成します。

パートナー センターで UWP 製品を作成、」の説明に従って:[新規または既存の UWP プロジェクトへの Xbox Live の追加](get-started-with-visual-studio-and-uwp.md)
...
MSA アプリ ID を [アプリケーション ID] フィールドに入力したら、[Submit Application ID Setup] (アプリケーション ID 設定の提出) ボタンをクリックします。 これにより、Xbox Live セキュリティで MSA アプリ ID の情報が保存されます、UWP から、XToken を取得する要求を作成するたびに含まれているタイトル クレームにマップされますこの XDP 製品である限り、UWP は、作成する AppX マニフェスト Id を使用して正しくd パートナー センターで!)

### <a name="enter-the-partner-center-pfn-into-xdp"></a>パートナー センターの PFN を XDP に入力します。
...
### <a name="flag-your-app-for-xbox-cert-in-partner-center"></a>パートナー センターで Xbox 認定用のアプリをフラグします。

現時点では、Xbox の認定を受けるために、ゲームを Xbox Live 対応として認識されるようにするには、手動で対応する必要があります。 アプリにフラグを設定するリリース マネージャーと協力は、Xbox Live SmartCert 検出のパートナー センターで有効になっています。

### <a name="update-your-uwp-game-in-the-xbox-app"></a>Xbox アプリで UWP ゲームを更新する

通常、Xbox アプリでは、Xbox アプリ内で Xbox Live エクスペリエンスを提供するために、すべての UWP ゲームに対して自動生成された 1 つのタイトル ID を使用します。 UWP ゲームの Xbox アプリで適切な XDP で生成されたタイトル ID を使用するのには、データを更新する必要がある、パートナー センター内**UWP ゲームをリリース用に提出する前に**。

そのためには、担当の DAM に連絡し、タイトル名に対して Xbox アプリのタイトル ID を更新する必要があることを伝えてください。  XDP で作成されたタイトル ID を含めるようにしてください (XDP 製品のセットアップの下に表示される&gt;製品の詳細) と UWP 用 Windows 10 の URL (アプリの管理下にパートナー センターで表示&gt;アプリ id)。

## <a name="configure-xbox-live-in-xdp"></a>XDP で Xbox Live を構成する

### <a name="service-configuration"></a>サービス構成

XDP とパートナー センターの製品を適切に構成してバインドされている、自由に通常どおり、XDK タイトルの XDP 内で共有 Xbox Live 構成を設定しています。

**リマインダー** – にバインドされた XDK+UWP ゲームする必要があります、ない状況では、有効にする、構成、またはパートナー センターを介して Xbox Live サービス構成を公開します。 このガイダンスに従わない場合、ゲームの Xbox Live 構成が破損して修復できなくなる可能性があります。

### <a name="catalog-configuration"></a>カタログの構成

カタログの構成を XDK+UWP ゲームの場合は、2 回セットアップする必要があります: XDK 用に、XDP で 1 回および UWP 用パートナー センターでします。

XDP での構成のセットアップは、通常の XDK 製品の場合とまったく同じです。 パートナー センター構成より詳細な手順を参照して[次に](https://dev.windows.com/en-us/publish)します。
...
バイナリの構成を XDK+UWP ゲームの場合は、2 回セットアップする必要があります: XDK 用に、XDP で 1 回および UWP 用パートナー センターでします。
...
一般に、XDK+UWP ゲームを公開する XDK タイトルを XDP とパートナー センターでの UWP で別々 に公開する方法と同じプロセスに従います。 そのため、以下では、クロスプレイ ゲームに固有のプロセスまたは考慮事項を中心に説明します。
...
### <a name="coordination-between-xdp-publishing-and-partner-center-publishing"></a>XDP での公開とパートナー センターでの公開の間の調整

XDP では、CERT から RETAIL への移行は、2 つの独立した、明示的なアクションです。 ただし、パートナー センターで、申請プロセスに自動的に移動するゲームは認定製品版にします。 そのため、正しい順序で操作する必要があります。

認定に提出する準備ができたら、以下の手順を順番に実行してください。

1.  XDP で XDK 製品を CERT に公開します (カタログ、バイナリ、サービス構成を含みます)

2.  パートナー センターに提出する UWP 製品を開始します。
...
<table>
  <tr>
    <td>
      UWP 専用ゲームの場合は、公開する XDK タイトル バイナリがあるない場合でも、パートナー センターでの申請を開始する前に XDP での証明書をカタログとサービス構成を公開する必要はします。
    </td>
  </tr>
</table>
...
2.  製品の認定ページで、パートナー センターで「このアプリは手動で公開」、されている場合は、"を今すぐ公開] を選択または発行「ないより早く \[date\]」を選択した場合に発生するまで待機します。

以上の手順が完了すると、共有サービス構成が RETAIL の状態になり、XDK タイトルおよび UWP ゲームが全世界に向けて公開されます。 これで、

<table>
  <tr>
    <td>
      UWP 専用ゲームの場合は、カタログを公開する必要とサービス構成を RETAIL に公開が完了するとパートナー センターで、それ以外の場合、リリースされた UWP 前に、XDP では、Xbox Live にアクセスできません。
    </td>
  </tr>
</table>
