...
4.  MSA アプリ ID: は、MSA クライアント ID とも呼ばれる、パートナー センターでは、製品の作成時に、MSA によって割り当てられる別のアプリ id です。 この ID によって、Microsoft サービスはアプリを識別することができます。 MSA アプリ ID は PFN と 1 対 1 に対応します (したがって Xbox タイトル ID とも 1 対 1 に対応します)。
...
1.  **XDK タイトルは XDP に取り込む必要があります**。 両方、サービス構成および公開メインラインのエクスペリエンスから、パートナー センターは XDK ベースのタイトルをサポートするために備えていません。
...
3.  **サービス構成を XDP とパートナー センターの間で分割することはできません**。 XDP とパートナー センターでは、1 つが他方で既に公開されている、他の公開の認識しません。 これにより、サービス構成が復元できないほど壊れたり、非常に悪いユーザー エクスペリエンス (実績やセーブ データの消失など) が生じる可能性があります。これを避けるため、XDK+UWP のクロスプレイ ゲーム用のサービス構成は、すべて XDP で行う必要があります。
...
MSA アプリ ID を [アプリケーション ID] フィールドに入力したら、[Submit Application ID Setup] (アプリケーション ID 設定の提出) ボタンをクリックします。 これにより、Xbox Live セキュリティで MSA アプリ ID の情報が保存されます、UWP から、XToken を取得する要求を行うたびに含まれているタイトル クレームにマップされますこの XDP 製品である限り、UWP は、作成した AppX マニフェスト Id を使用して正しくd パートナー センターで!)
...
通常、Xbox アプリでは、Xbox アプリ内で Xbox Live エクスペリエンスを提供するために、すべての UWP ゲームに対して自動生成された 1 つのタイトル ID を使用します。 UWP ゲームの Xbox アプリで適切な XDP で生成されたタイトル ID を使用するのには、データを更新する必要があるパートナー センター内で**UWP ゲームをリリース用に提出する前に**。
...
XDP では、CERT から RETAIL への移行は、2 つの独立した、明示的なアクションです。 ただし、パートナー センターで、申請プロセスに自動的に移動、ゲームは認定製品版にします。 そのため、正しい順序で操作する必要があります。
...
<table>
  <tr>
    <td>
      UWP 専用ゲームの場合は、カタログに公開するために必要とサービス構成を RETAIL に公開が完了するとパートナー センターで、それ以外の場合、リリースされた UWP 前に、XDP では、Xbox Live にアクセスできません。
    </td>
  </tr>
</table>
