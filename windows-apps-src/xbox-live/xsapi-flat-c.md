...
C++ API には、呼び出しパターンにより、開発者はスレッド モデルを完全に制御する新しい非同期スレッドが導入されています。 詳細については、[呼び出しパターン XSAPI フラット C レイヤーの非同期呼び出し](flatc-async-patterns.md)を参照してください。
