...
**scid、templateName、名** | 組み合わせて使用して、セッションを識別できます。 セッションへの MPSD URI パスで指定されているのと同じ 3 つのパラメーターです。</br> </br>`https://sessiondirectory.xboxlive.com/serviceconfigs/{scid}/sessiontemplates/{templateName}/sessions{name}` の順にクリックします。</br></br>XSAPI では、これらは `multiplayer_session_reference ` コンストラクターを指す 3 つのパラメーターです。
...
### <a name="2--playing-the-match"></a>2. 一致の再生
...
タイトルはまたセッションを使用して、マップやモードなどのマッチの設定を把握します。 これらのタイトル固有の設定は、セッション テンプレートで設定するか、トーナメント定義の中でカスタムの定数として設定できます  (詳細については、次を参照してください[競技場のタイトルを構成する](#configuring-a-title-for-arena)。)。次に例を示します。
...
少なくとも 1 つのプレーヤーを開始時刻までの時間の前にセッション内でアクティブにする必要があります (`multiplayer_session_arbitration_server::arbitration_start_time()`) までのタイムアウトとします。までのタイムアウトはミリ秒単位の数として、セッションでは`/constants/system/arbitration/forfeitTimeout`(`multiplayer_session_constants::forfeit_timeout()`)。 失効時刻までに参加済みのアクティブなプレイヤーがいない場合、マッチはキャンセルされます。
...
タイトルは、開始時刻から判定時刻までの間、いつでも結果を報告できます。 判定は、失効時刻から判定時刻までの間に、セッションのすべてのアクティブなメンバーの結果が送信された後で随時実行されます。 たとえば、失効時にアクティブなメンバーが 1 人だけのセッションでも、メンバーは結果を投稿でき (または投稿しなければならず)、判定が行われます。 判定は、判定時刻までに報告された結果の数に関係なく、実行済みでない限り実行されます。 判定時刻に結果が 1 つも提出されていない場合、マッチのすべての参加者が敗者と判定されます。
...
* "completed"。すべてのアクティブなプレイヤーでは、結果が報告されました。
* "partialresults":一部のアクティブ プレーヤーには、調停タイムアウトするまでの結果が報告されました。
* 「結果」:アクティブなプレーヤーの [なし] には、調停タイムアウトするまでの結果が報告されました。
* 「キャンセル」。までタイムアウトするまでのアクティブ プレーヤーが到着しません。
...
### <a name="4--returning-to-the-xbox-arena-ui"></a>4。Xbox Arena UI に戻る
...
アリーナではタイトルが個別のマッチとしてのみ提示されるため、タイトルを更新せずに、徐々に新しい形式を導入できます。 そのため、Arena に統合されるタイトルの基本的なユーザー エクスペリエンスは、複数の対決形式と連携できるシンプルかつ柔軟なものでなければなりません。
...
競技場のタイトルを有効にする、いくつか追加手順が必要です、Xbox 開発者ポータル (XDP) でを構成するときに、または[パートナー センター](https://partner.microsoft.com/dashboard)します。
...
* **トーナメント資産**– (以前の 'マルチ プレーヤーとマッチメイ キング' セクションです)。これらは、タイトルのトーナメントのイメージです。

パートナー センターで競技場も有効にする、**トーナメント**Xbox Live サービスの下のメニュー。
...
最後に、**maxMembersCount** システム設定が必要です。 これには、トーナメント マッチでプレイできるプレイヤーの合計数を設定します。 プレイヤーの数は、ゲーム モードの設定によってさらに制限できるため、セッションで設定する値には、必ずタイトルでサポートされる最大プレイヤー合計数を設定してください。 たとえば、プレーヤーの一致をサポートしているゲームの最大数は 5 つの vs 場合です。5 設定**maxMembersCount**を 10 にします。 この MPSD テンプレートを使用した場合、**maxMembersCount** を上限として、任意の数のプレイヤーのマッチがサポートされます。
