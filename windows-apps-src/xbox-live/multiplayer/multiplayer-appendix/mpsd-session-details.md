...
マルチ プレーヤーのセッション ディレクトリ (MPSD) セッションは、セッション名を備え、セッションの既定の設定を提供する JSON ドキュメントには、セッションのテンプレートのインスタンスとして識別されます。 テンプレートは GUID であるサービス構成識別子 (SCID) を使用したサービス構成の一部です。 このテンプレートで確認できます[Xbox 開発者ポータル (XDP)](https://xdp.xboxlive.com)と[パートナー センター](https://partner.microsoft.com/dashboard)します。 サービスの構成は、インジェスト、管理、およびセキュリティ ポリシーに使用する開発者向けリソースです。 セッションが MPSD を介してアクセスされると、プリンシパルの承認が XDP またはパートナー センターを通じて、開発者によって設定されたアクセス ポリシーに従って、サービス構成に対して実行されます。 サービス構成へのアクセスが承認された後にセッションが読み込まれると、セッションのメンバーシップ検証などのセカンダリー アクセス チェックがセッション レベルで実行されます。
...
セッションのスキーマは、セッションのバージョン (メジャー バージョン) とプロトコルのバージョン (マイナー バージョン) によってバージョン管理されます。 バージョンは、X Xbl コントラクト バージョン ヘッダーが組み合わされて、"100\*メジャー + マイナー"。 たとえば、v1.7 タイトルには、107 の最新のテンプレートのコントラクト バージョンと仮定すると、REST 要求ごとに次のヘッダーが含まれています。X Xbl のコントラクト バージョン:107.
...
機能は、MPSD がそのセッションに適用する動作を構成する、MPSD セッション内の定数です。 頻繁に使用します XDP とパートナー センター セッション テンプレートの機能を設定します。 /constants/system/capabilities オブジェクトに設定されます。 機能が必要ない場合は、空の capabilities オブジェクトを使用します。
...
大きな MPSD セッションは最大 1000 のメンバーを持つことができますが、すべてのメンバー一覧の取得など、いくつかのセッション機能が無効になります。 セッションの大きさは、**MultiplayerSessionCapabilities.Large プロパティ**によって表されます。 大きなセッションを指定する場合はこのプロパティが true に設定され、"大きな" 機能は /constants/system/capabilities オブジェクトで示されます。 
...
タイトルは、MPSD セッション内にゲーム メタデータを格納します。 ゲーム メタデータは、セッション データを表示し、タイトルがゲーム セッションを見つけて参加できるようにするために必要な情報です。 タイトルは、セッション、メンバーなど、player の色、セッションなどの武器を選択したプレーヤーのカスタム プロパティ セクションでは player 固有のメタデータを格納します。セッション全体のメタデータ、たとえば、現在のマップは、MPSD セッションのグローバルのカスタム プロパティ セクションに格納されます。
...
セッションの作成時に、クライアントはアービターとして 1 つの本体を指定します。 参照してください[方法。MPSD セッションの監視を設定](multiplayer-how-tos.md)します。
...
アービターは、これらの操作のいずれかで空のゲーム セッション スロットを入力するプレーヤーを検索します。 タイトルは、遅延の結合を許可するロビー セッションまたは別のメカニズムを使用する場合は、そのメカニズムを使用して新しいセッション メンバーを検索します。
-   別のマッチ チケット セッションを作成します。

参照してください[方法。開いているセッションのスロットをマッチメイ キング中に入力](multiplayer-how-tos.md)します。


#### <a name="handling-invited-session-members"></a>招待されたセッション メンバーの処理

アービターは、招待されたセッション メンバーを監視し、1 人のユーザーに対する招待間の最小間隔を適用する必要があります。 参照してください[方法。ゲームの招待を送信](multiplayer-how-tos.md)します。
