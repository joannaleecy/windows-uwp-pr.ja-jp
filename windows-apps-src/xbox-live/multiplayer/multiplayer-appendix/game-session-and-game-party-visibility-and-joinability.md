...
> **注:** 再試行を要求できますプライベートに可視性の設定、 **join()** 招待された player で呼び出します。 招待がプラットフォーム UI を介して送信される場合は、そのプレイヤーがセッションで予約される前に、他のプレイヤーがそれを受信する可能性があります。 このような競合状態では、参加しているプレイヤーに対する予約がプライベート セッションまたは可視セッションで必要になるため、招待されたプレイヤーに対する **join()** の呼び出しが失敗する場合があります。
>
...
2 番目のアクセス制御方法は参加可能性です。 参加可能性は、セッションの有効期間の間に動的に設定でき、セッションに参加できるユーザーの種類を決定します。 セッション joinability の値は次のとおりです。
...
**注:** ゲーム セッション joinability はパーティのアプリの UI に反映されません。 参加可能性が制限されている場合であっても、パーティー アプリの招待 UI および **ShowSendInvitesAsync** はゲーム セッションの招待を許可します。
...
**注:** パーティ joinability ではセッション joinability には影響しませんが、パーティ joinability はパーティのアプリの UI に反映されます。 プレイヤーは、タイトルの外部でパーティー アプリのパーティーの参加可能性を手動で変更できます。
...
**注:** タイトルを起動し、最終的な決定が行わ前に、ホストまたは監視に接続するには、招待されたプレーヤーが必要になります。 セッションに空きがない場合、または招待が拒否された場合は、ユーザーにエラー メッセージを表示してもかまいません。

> **注:** ホストまたは監視への接続を確立するには、セキュリティで保護されたデバイスのアドレスが必要です。 どのセッション メンバーがセッションの現在のホストまたはアービターか、および招待されたプレイヤーがどのセキュア デバイス アドレスに接続する必要があるかということは、セッションの **HostDeviceToken** を使用して示す必要があります。
...
-   ゲームを開く-、開くゲームをゲーム パーティ joinability を既定値でまま。友人が結合可能です。 これにより、フレンドは招待がなくてもゲーム パーティーに (および拡張によってゲーム セッションに) 参加できます。
...
**注意**:プレーヤーは、パーティのアプリをゲーム セッションの joinability を手動で変更できます。
...
この問題に対処するには、セッション メンバーの最大サイズを常に、タイトルが実際にサポートしているプレイヤーの最大数よりも大きい値に設定する必要があります。 たとえば、プレイヤー数が 8 の場合、最大セッション サイズを 12 などと設定する必要があります。 このようにすると、古いプレイヤーがまだタイムアウトしていない場合でも、新しいプレイヤーが参加できます。アービターまたはホストは、セッションに空きがないかどうかを判別し、新しいプレイヤーがまだ参加できるかどうかを決定するカスタム セッション プロパティを設定します (**IsFull** : “true”)。 これにより、セッションが参加可能かどうかを参加しているプレイヤーがすばやくチェックできます。

どのメンバーでは、インデックスでタイムアウトになったかを示すカスタム プロパティは、監視またはホストで管理も必要があります (たとえば、 **TimedOutPlayers** :“3, 5, 9”). これにより、新しいプレイヤーは現在のセッション メンバーを正しく識別できます。
...
**注:** タイトルが中断または終了され、それ以上通知を受信する場合、プレーヤーは自動的にゲームのパーティから削除されます。
