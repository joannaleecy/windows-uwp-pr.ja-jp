...
このトピックでは、新しい Xbox One マルチプレイヤー セッション ディレクトリ (MPSD) サービスを使用するマルチプレイヤー セッション作成の概要を示します。 主な対象読者は、セッション テンプレートを Xbox デベロッパー ポータル (XDP) に直接提出する Xbox One タイトル デベロッパーです。 MPSD サービスでは、同様に、パートナー センターで構成できますが、この記事では重視されていません。 MPSD の構成に関連する用語と概念、使用方法、およびマルチプレイヤー セッションのトラブルシューティングについて理解を促進することを目的としています。
...
| ゲーム セッション         | これは、実際のゲーム セッションをユーザーがまとめて再生、MPSD で公開されています。 マルチプレイヤーのすべてのシナリオは、最終的にはゲーム セッションになります。                                                                                                                               |
...
|--------------------------------------|-----------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------|
| **ゲーム セッション情報を取得します。**     | **XSessionGetDetails**、**XSessionSearchByID**、または独自に追跡。                                              | サービスからセッション情報を要求する。                                                          |
| **ホストを移行します。**                     | 必要なときにタイトルが **XSessionMigrateHost** を呼び出す。                                                           | 新しいセッションの作成は不要、SessionID に新しいホストを割り当てるだけ。                                  |
| **複数のプレイヤーのセッションを管理します。**  | 複数のセッションを一度に扱うことが難しい。 たとえば、**XNetReplaceKey** と **XNetUnregisterKey** のどちらが適切か。 | サービス ベースのセッションにより、1 つのセッションの管理がより整理され、複数のセッションの扱いが容易になる。    |
| **記号の完全な詳細を処理し、切断** | それぞれ **XCloseHandle** または **XSessionDelete** によって切断とサインアウトを別々に処理する必要がある。 | 集中管理されたサービスがサインアウトと切断の処理を簡素化し、タイムアウトはゲーム構成内で設定される。 |
...
```
優先度順に、この属性の値は *timeout、latency、bandwidthdown、bandwidthup、network、group*、または *episode* に設定できます。 ネットワーク設定値の場合、ネットワークの構成や状態 (競合しているネットワーク アドレス変換など\[NAT\]) QoS メトリックが計測されることを禁止します。 joining の終了時の値として可能性があるのは *group* だけです  (joining からのタイムアウト時に予約は削除されます)。
...
-   エピソードを初期化中の場合\#1 が発生すると、ステージに設定されている*失敗*およびセッションを初期化することはできません。 それ以外の場合、初期化エピソードが完了すると、"initializing" オブジェクトは削除されます。 **externalEvaluation** が **false** に設定されている場合、evaluating ステージはスキップされます。 **metrics** と **measurementServerAddresses** のどちらも設定されていない場合、measuring ステージはスキップされます。
...
-   ホスト候補はデバイス トークンを優先度順に列挙したものです。 後に設定されている、*測定*初期化エピソードのステージ\#場合は、1 **peerToHostRequirements**設定されていると **/properties/system/host**が設定されていません。 **/properties/system/host** オブジェクトが設定された後にクリアされます。
...
-   メンバーがステージを正常に通過しなかった場合、joining または measuring ステージからの遷移時に、メンバー オブジェクトの *initializationFailure* 属性が設定されます。 優先順位に従って、*timeout*、*latency*、*bandwidthdown*、*bandwidthup*、*network*、*group*、または *episode* に設定できます。
    *ネットワーク*値。 つまり、ネットワークの構成や状態 (競合しているネットワーク アドレス変換など\[Nat\]) QoS メトリックが計測されることを禁止します。 joining の終了時の値として可能性があるのは *group* だけです  (joining からのタイムアウト時に予約は削除されます)。*エピソード*への参加または測定中に失敗する初期化エピソードのすべてのメンバーで障害が発生した「評価」段階の後に値が設定されます。
...
-   *要求されたセッションにアクセスできません。プライベート セッションは、セッションのメンバーのみが読み取ることができます。*

    -   可視性が "private" のセッションにアクセスしようとしています。 セッション内のメンバーのみがセッション ドキュメントを読むことができます。

-   *要求本文は、プリンシパルの認証には、サーバーが含まれていない限り、既存のメンバーの参照を含めることはできません。*
...
> PUT serviceconfigs/00000000-0000-0000-0000-000000000000/sessiontemplates/クイック/セッション/foo http/1.1 Content-type: application/json なし-If-match: \*

次の例は、セッションの etag が If-Match ヘッダーと一致しない場合に 412 Precondition Failed を返します。

> PUT serviceconfigs/00000000-0000-0000-0000-000000000000/sessiontemplates/クイック/セッション/foo http/1.1 コンテンツの種類: アプリケーション/json 場合 - 一致するもの。9555A7DE-8B91-40E4-8CFB-0629312C9C7D
