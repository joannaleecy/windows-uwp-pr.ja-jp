...
Config XDP またはデベロッパー センターの UI では、各セッションをマップする 1 つまたは複数のホッパー、各セッションがまとめてそのホッパーでマッチングされたは方法を決定するルールが含まれます。 詳細については、「マッチメイキングの基本的なホッパー構成」を参照してください。
