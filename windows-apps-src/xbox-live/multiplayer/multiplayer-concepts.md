...
* ゲーム セッション - これは、ラウンド、一致する、レベルなど、ゲームの特定のセッションのインスタンスで再生するユーザーを表すセッション。このセッションでは、複数のロビー セッションに参加しているセッションのインスタンス、通常のマッチメイ キング サービスを通じてからメンバーを含めることができます。
