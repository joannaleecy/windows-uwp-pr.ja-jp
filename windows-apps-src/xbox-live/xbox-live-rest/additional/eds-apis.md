...
| 絞り込み条件値をクエリします。| /{ロケール}/{queryRefiner} のメタデータ/metadata/mediaitemtypes/{mediaItemType}/queryrefiners/| 項目の種類を指定したメディアの指定されたクエリの絞り込み条件の値|
| 調整のサブ値をクエリします。| /{ロケール}/{queryRefiner} のメタデータ/metadata/mediaitemtypes/{mediaItemType}/queryrefiners//subQueryRefinerValues| 指定されたクエリの絞り込み条件値 ("subgenres には、指定されたジャンル"など) のサブ値の一覧。 クエリの絞り込み条件値でという名前の"queryRefinerValue"は、渡される URI 語幹で禁止されている文字の絞り込み条件値クエリを許可するのには、クエリ文字列パラメーターとして渡されます。|
...
EDS Api では、既定では、各項目のフィールドの非常に小さい最小セットを返します。
...
このパラメーターを受け取る API, 指定された値が指定したメディア項目のすべての種類のすべてのフィールドのスーパー セットである必要があります。 別のメディア項目の種類のフィールドの別のセットを指定することはできません。 ただし、フィールドは、1 つのメディア項目の種類がない別、それに適用する場合はでのみ表示、メディア項目の種類のデータが存在する (例: AvatarItems のみが、フィールドを含む"AvatarBodyType"がフィールド名の結合 API への呼び出しに含まれている場合)。
