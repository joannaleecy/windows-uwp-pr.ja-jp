...
かどうか。 含める = valuemetadata クエリ パラメーターには、ランキングの値に関連付けられたメタデータを含めるへの応答がことができます。 値のメタデータには、指定されたクライアントが含まれています。 データ モデルやレース トラックに最適な時間を実現するために使用された車の色などの値に関連付けられています。
...
| columnDefinition.statName| string| 必須。 ランキングがに基づいてユーザーの統計の名前。| 
...
| value| string| 必須。 ランキングの基になる統計のユーザーの値。| 
