...
| Accept| string| コンテンツの種類の受け入れられる。 1 つだけのプレゼンスがサポートは<b>アプリケーション/json</b>がも指定ヘッダーで。| 
...
| キャッシュ コントロール| string| キャッシュ動作を指定する正し要求します。 値の例:「は」。| 
| X XblCorrelationId| string| サーバーが返すし、クライアントが受信を関連付けるためにサービスで生成された値です。 値の例:"4106d0bc-1cb3-47bd-83fd-57d041c6febe"。| 
| X コンテンツの種類オプション| string| SDL に準拠した値を返します。 値の例:"nosniff"。| 
| Date| string| メッセージが送信された日付/時刻。 値の例:"火曜日 2012 年 1 17 年 11 月 10時 33分: 31 GMT"。| 
