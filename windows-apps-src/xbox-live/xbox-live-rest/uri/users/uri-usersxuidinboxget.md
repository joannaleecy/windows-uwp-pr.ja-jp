...
ユーザーのメッセージ要約にはには、メッセージの件名のみが含まれています。 ユーザー生成のメッセージは、現在のメッセージ テキストは、最初の 20 文字です。 システムのメッセージは、"LIVE System"などの別のサブジェクトを提供することができます。
...
ユーザーのメッセージの概要を取得する要求、独自のユーザーが必要です。
...
| 送信| DateTime|  | UTC 日付と時刻は、メッセージが送信されます。 (サービスによって提供されます)。|
| 有効期限| DateTime|  | UTC 日付と時刻は、有効期限が切れるメッセージ。 (すべてのメッセージによって、将来的に決定する最長有効期間がある)。|
| メッセージの種類| string| 50| メッセージの種類: ユーザー、システム、FriendRequest、ビデオ、QuickChat、VideoChat、できるようにする PartyChat、タイトル、GameInvite します。|
...
| プロパティ| 型| 最大長| 注釈|
| --- | --- | --- | --- |
| continuationToken| string| 100| 必要に応じてサーバーによって返されます。 により、以降の呼び出しで列挙を継続します。|
