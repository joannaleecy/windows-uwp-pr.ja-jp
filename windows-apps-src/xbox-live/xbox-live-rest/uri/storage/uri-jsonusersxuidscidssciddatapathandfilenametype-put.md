...
ファイルをアップロードします。 データ型の json は、複数のブロックのアップロードはサポートされていません。 これらの Uri のドメインが`titlestorage.xboxlive.com`します。
...
要求本文には、アップロードするファイルのすべての内容が含まれています。 
...
アップロードが成功した場合は、{} の応答で、201 が返されます。
