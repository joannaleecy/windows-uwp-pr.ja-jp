...
| customSelector| string| 設定の種類のファイル。 どのようなカスタムの仮想ノードを含めることを示します。 構成の種類のファイルについて詳しくは、タイトル ストレージを参照してください。| 
...
| コンテンツ範囲| これが、部分ダウンロードである場合、このヘッダーは、ダウンロードされたバイト数の範囲を指定します。| 
