...
| デバイスの種類| 必須| とき、ユーザーの deviceType が存在しないか、コンソール以外に、タイトルに一致する設定には、コンソール専用のタイトルする必要がありますできません。 | 403|
...
| preserveSession| 列挙値| かどうかには、セッションに一致するセッションとして再利用を示す値。 値は、「ことはありません」と"always"します。 |
