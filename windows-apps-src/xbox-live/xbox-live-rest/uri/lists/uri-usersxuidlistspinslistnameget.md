...
リストが存在しない場合し、結果がリスト項目はありません、**数える**0 になりますされ**listVersion** 0 になります。
...
| maxItems| 32 ビット符号付き整数| 省略可能。 返される項目の最大数。 要求の最大値が指定されていない場合、既定値は 25 項目はします。 サービスがこの値に最大を配置しません。値がリスト内の項目の数よりも大きい場合は、エラーなしですべての項目が返されます。| 
| filterItemId| string| 省略可能。 一覧で検索する項目を指定します。 リストの項目のすべてのインスタンスを返します。 により、クライアントはリストの項目には、そのをすばやく確認します。 大規模なリスト全体のリストを反復処理することがなく、項目のすべてのインスタンスを判断するのに便利です。 既定値: null です。| 
| filterContentType| string| 省略可能。 返されるコンテンツの種類のコンマ区切りの一覧を指定します (戻りません種類の一覧ではなく)。 のみがリストから特定のコンテンツの種類を取得するために使用します。 コンテンツの種類のコンマ区切りの一覧は、このフィルターに使用されます。 (複数のコンテンツの種類を 1 つの呼び出しで照会できます)。サポートされているコンテンツの種類には、エンターテインメント探索サービス (EDS) で定義されているすべてのメディアの種類が含まれます。 既定値: null (すべてのコンテンツの種類)。| 
| filterDeviceType| string| 省略可能。 返されるデバイスの種類のコンマ区切りの一覧を指定します (戻りません種類の一覧ではなく)。 戻り値のセットを特定のデバイスの種類のセットから挿入されている項目を返すだけをフィルター処理します。 デバイスの種類のコンマ区切りの一覧は、このフィルター (1 つの呼び出しで複数のデバイスの種類を問い合わせることができます) に使用されます。 設定可能な値: XboxOne、MCapensis、WindowsPhone、WindowsPhone7、Web、PC、MoLive します。 既定値: null (すべてのコンテンツの種類)。| 
