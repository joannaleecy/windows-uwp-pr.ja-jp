...
リストが存在しない場合し、結果にリスト項目はありません、**数える**0 になりますされ**listVersion** 0 になります。
...
| maxItems| 32 ビット符号付き整数| 省略可能。 返される項目の最大数。 要求で最大値が指定されていない場合、既定値は 25 項目はします。 サービスがこの値は最大を配置しません。値がリスト内の項目の数よりも大きい場合は、エラーなしですべての項目が返されます。| 
