...
項目を削除するは、リスト内のインデックスを示され、クエリ文字列パラメーター**インデックス**で識別されます。 コンマで区切られたインデックスのリストがある必要があり、1 つの呼び出しでは 100 のインデックスを渡すことができます。 ただし、インデックスが (空のクエリ文字列パラメーター) を渡されていない場合、一覧の全体の内容が削除されます (空のリストが残ります、バージョン番号がインクリメント引き続き)。 項目が削除されると一覧が「キーを押してコンパクト」インデックスの順序で穴が残っていないようにします。 したがって、この呼び出しは、等ではありません。
 
この呼び出しに必要な**場合-マッチ: versionNumber** **versionNumber**がファイルの現在のバージョン番号は、要求に含まれるヘッダー。 含まれていないまたはし、現在の一覧のバージョン番号、http/412 (precondition failed) が一致しない場合は、ステータス コードが返されます、応答の本文には、現在のバージョン番号を含む一覧の最新のメタデータにが含まれます。 これは、互いに踏み潰すさまざまなクライアントからの更新プログラムを防ぐため。
