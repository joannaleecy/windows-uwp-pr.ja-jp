...
---
# <a name="post-serviceconfigsscidbatch"></a>POST (/serviceconfigs/バッチ処理)
...
この HTTP/REST メソッドでは、サービス構成 ID (SCID) レベルでの複数の Xbox ユーザー Id にバッチ クエリを作成します。 このメソッドは、 **Microsoft.Xbox.Services.Multiplayer.MultiplayerService.GetSessionsForUsersFilterAsync**でラップすることができます。

2015 マルチプレイヤーを組み合わせることができます多くのクエリを 1 つの呼び出しにすべてのクエリが同じ場合、 *xuid*クエリ文字列パラメーターで表される、さまざまな Xbox ユーザー Id (Xuid) を処理します。 クエリ文字列パラメーターと、応答は、通常のクエリやバッチ クエリの同じであるに注意してください。

バッチ クエリでは、各 XUID に属するセッションは*xuid*のパラメーターが要求で表示されるように同じ順番で応答に書き込まれます。 一致する各*xuid*に 1 回、応答で複数回表示されるように、同じセッションのことができます。
