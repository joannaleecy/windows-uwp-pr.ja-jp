...
2015 マルチプレイヤーを組み合わせます多くのクエリを 1 つの呼び出しにすべてのクエリが同じがある場合、 *xuid*クエリ文字列パラメーターで表される、さまざまな Xbox ユーザー Id (Xuid) を処理します。 クエリ文字列パラメーターで、および、応答は、定期的なクエリおよびバッチ クエリの同じである注意してください。

バッチ クエリでは、各 XUID に属するセッションは、 *xuid*パラメーターが要求されると同じ順序で応答に書き込まれます。 複数回と一致する各*xuid*に 1 回、応答で、同じセッションのことができます。
