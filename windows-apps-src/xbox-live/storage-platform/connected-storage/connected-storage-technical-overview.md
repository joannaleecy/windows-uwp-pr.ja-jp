...
-   2 GB、t: パスでアクセス可能な専用のハード ドライブ ストレージの\\します。
...
**図 1。  接続された記憶域スペース (タイトル/コンピューターごとまたはタイトル/ユーザー)**

![](../../images/connected_storage/connected_storage_space_containers.png) データは 1 つとしてコンテナーに格納またはバッファーと呼ばれる*blob*します。 次の図は、ディスク上のコンテナーの内部システム表現を示しています。 コンテナーごとに、コンテナー内の各 BLOB に対応するデータ ファイルへの参照を含むコンテナー ファイルがあります。
...
**SubmitUpdatesAsync** データを受け取るための共有パーティション内のメモリーには 16 MB の上限があります。 専用の 16 MB バッファー内に空きメモリーが十分にないために、システムが **SubmitUpdatesAsync** の呼び出しを即座に処理できない場合、呼び出しは処理のキューに入れられます。 システムによって 16 MB のバッファーからハード ドライブにデータが継続的に転送され、16 MB のバッファー内の領域が使用できるようになると、キューに入れられた更新が要求された順序で処理されます。

**図 3。  SubmitUpdatesAsync 動作**

![](../../images/connected_storage/submitupdatesasync_behavior.png) クラウドへのアップロードは、同様の方法で行われます。個々 の blob は、サービスにアップロードされ、その他のすべてのアップロードされた blob を参照するコンテナー ファイルに、更新操作は最終的な更新プログラムでコミットされます。 クラウドへのアップロードでは、このように 1 回の最終的な更新に統合することで、**SubmitUpdatesAsync** 呼び出しで参照されているすべてのデータが完全にコミットされるか、コンテナーが変更されないままになるかのどちらかになります。 このようにすることで、アップロード処理中にシステムがオフラインになったり、電源が切断されたりした場合でも、ユーザーは別の Xbox One 本体にアクセスして、クラウドからデータをダウンロードし、すべてのコンテナーが一貫した状態でプレイを続行できます。
...
**図 5。  システムでは、更新された情報をキャプチャし、値をハード ドライブにコピーします。**
...
**図 6。  HDD、コンテナー ファイルが更新され、参照されていない blob を削除します。**
...
Bob が食料を購入するシナリオを考えます。 わかりやすくするために、1 単位のゴールドで 1 単位の食料を買うことができ、Bob が 25 単位の食料を買うことにします。 アプリが実行する 1 つ**SubmitUpdatesAsync**金、Bob からの 25 の単位を減算するには、食品の 25 のユニットとし、もう 1 つを追加する呼び出し\_インベントリ コンテナー。 ただし、両方の **SubmitUpdatesAsync** 呼び出しの完了ハンドラーが呼び出された場合でも、ハード ドライブへのデータの書き込みを停止させる可能性がある停電や、クラウドへの不完全な同期などが原因で、誤った結果となる場合があります。 以下の図は、システムによって実行される手順と、いずれかの手順で停電が発生した場合の結果について説明しています。
...
**図 7。  システムは、食品の blob の値をディスクに書き込みます。**

![](../../images/connected_storage/update_method_wrong_way_1.png) 次に、システムは、新しく書き込まれた値を参照するコンテナーを更新します。 次の図に示されているように、この手順の実行後、次の手順の前に電源が失われた場合、Bob は、対応するゴールドをインベントリーから差し引かれずに 25 単位の食料を得るという得な取引をすることになります。

**図 8。  システムでは、新しく書き込まれた値を参照するコンテナーを更新します。**

![](../../images/connected_storage/update_method_wrong_way_2.png)

次に、システムはゴールドの BLOB の新しい値のデータをディスクに書き込みます。 Gold、ボブが参照されている値\_インベントリ コンテナーも更新されていない、し、Bob が彼にする必要がありますよりも 25 以上の gold-が、私たちは、目的の結果に一歩近づきました。

**図 9 です。  システムは、ゴールドの blob の新しい値のデータをディスクに書き込みます。**
...
**図 10。  システムでは、新規に書き込まれるゴールド blob を参照するコンテナーのファイルを更新します。**
...
**図 11。  システムでは、食品の blob の新しい値のデータを書き込みます。**

![](../../images/connected_storage/update_method_right_way_1.png) システムは、ゴールドの blob の新しい値のデータをディスクに書き込みます。

**図 12。  システムでは、ゴールドの blob の新しい値のデータを書き込みます。**

![](../../images/connected_storage/update_method_right_way_2.png) 最後に、システムは、新しい blob の両方を参照するコンテナーのファイルを更新します。

**図 13。  システムでは、どちらも新しい blob を参照するコンテナーのファイルを更新します。**

![](../../images/connected_storage/update_method_right_way_3.png) この例は非常に単純な 1 つを発行して、アトミックに適用する必要があるコンテナー内でデータへのすべての変更を行う重要性を示しています。 **SubmitUpdatesAsync**必要なすべての更新プログラムを呼び出します。 ゴールドで食料を購入するケースをこのように処理することで、アプリでは、誤って一方の値のみが更新され、本来よりも多いゴールドがキャラクターに残る可能性がある競合条件を回避します。
...
**図 14 です。  Blob 更新の 2 つの 512 k には 2 秒ごとに、もう 1 つの SubmitUpdatesAsync 操作の処理時間 1024 k blob の更新、およびその他のハード ドライブ アクティビティがありません。**

![](../../images/connected_storage/submitupdatesasync_proc_time_mixed_size_fixed_interval.png) 図 14 は、3 つの処理時間を示しています。 1024 k は、さまざまな時間間隔で blob。
...
**図 15 です。  3 つの処理時間 1024 k は、さまざまな時間間隔で blob。**

![](../../images/connected_storage/submitupdatesasync_proc_time_fixed_size_various_intervals.png) これらは、実例のみです。 通常、アプリはこのような頻度でデータを保存するべきではありませんが、ディスク I/O のない環境で動作することも通常はありません。
...
**図 16。  アプリが接続されている記憶域スペースを要求したときに、システムによって後にシーケンスします。**

![](../../images/connected_storage/app_requests_connected_storage_space.png) システムは、サービスと、次の段階をたどります、 **GetForUserAsync**要求。
 ...
 **同じコンテナーがローカルとクラウドで変更された場合** ユーザーが、別の本体でプレイしてクラウド内のコンテナーを変更し、現在の本体をオフラインで使用して同じコンテナーを変更した場合、データは自動的に結合されません。 ユーザーは保持するデータを選択するよう求められます。 ユーザーは、競合が発生した場合、置換ポリシーを選択できます。ローカル データまたはクラウドのデータが常に保持されているか、ユーザーが選択できる**キャンセル**を選択を行うを延期します。 ユーザーが置き換えポリシーとしてクラウド データまたはローカル データのどちらかを選択した場合、同じ名前であるが内容が異なるコンテナーは、そのポリシーに従って解決されます。
...
2 つのツールでは、接続された記憶域のアプリの使用の開発に役立ちます。XbStorage および Fiddler します。
