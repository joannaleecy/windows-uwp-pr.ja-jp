...
- *Gamesaveutil* は、PC のローカルにキャッシュされた UWP の接続ストレージを管理するためのコマンド ライン ツールです。 *gamesaveutil* ツールは、Fall Creators Update 以降 (ビルド 10.0.16299.15 以降) の [Windows 10 SDK](https://developer.microsoft.com/en-US/windows/downloads/windows-10-sdk) に付属しています。 Windows 10 SDK の適切なバージョンをインストールしたら*gamesaveutil*フォルダーの下にあります。**ProgramFiles(x86)/Windows キット/10/bin/[SDK Version]/x64/gamesaveutil.exe**します。
...
これらのコマンドの 1 つの例外は**リセット** **/force** SCIDs とユーザーのすべての保存データのローカル ストレージをクリアしますが、クラウドに格納されたデータは変更されません。 これは、タイトルのプレイ時にユーザーが本体にローミングしてクラウドからデータをダウンロードする場合の状態に本体を移行させるのに便利です。
...
|構成方法  |説明  |
|---------|---------|
|/force   |接続ストレージをリセットする必要があることを確認します。 指定せずにリセット コマンドを実行している **/force**次のメッセージが表示されます。 記憶域の接続された工場出荷時リセットはこのコマンドが、リセットを実行していない場合を除き、可能性のある破壊的な操作、 **/force**フラグが存在します。 |
...
- 場合、 **/context**パラメーターを使用するとでファイル名が指定された\<infile > が、 **ContextDescription**セクションが正しく設定される、そのファイルを使用して指定して、接続されているストレージ領域。
...
|構成方法  |説明  |
|---------|---------|
|*出力ファイルします。*             |データのエクスポート先となる XML ファイル。         |
...
ダミー データが生成され、によって指定されたファイルを保存します\<ファイル名 >。 ファイルの XML 形式の詳細については、後の「[インポートおよびエクスポートのファイル形式](#xbstorage_fileformat)」を参照してください。    サービス構成 ID (SCID) は 00000000-0000-0000-0000-000000000000 に設定され、コンピューターごとの接続ストレージ領域用にアカウントが設定されます。 これらの値を変更する場合は、生成された後のファイルを直接編集できます。
...
> [!NOTE]
> バイトのデータは、単純な昇順のシーケンス。たとえば、5 バイトの blob は、次のバイトがあります。00 01 02 03 04. >>  ユーザーごとの接続ストレージ領域を指定する場合は、XML ファイルの **Account** ノードを次のように変更します。
>  ```
...
| /X *:address* [*+accesskey*]  | ターゲットの本体のホスト名またはアドレス (本体に **Tools IP** として表示されます) を指定しますが、既定の本体は変更しません。 このオプションを使用しない場合、既定の本体が使用されます。*Accesskey* は、アクセス キーを知っている人だけに本体へのアクセスを制限するために使用できる文字列です。 コマンドを使用して、アクセス キーを設定**xbconfig** **accesskey = * * * キー your*; アクセス キーを有効にする、コンソールを再起動します。 アクセス キーが設定されている本体にアクセスするには、本体の IP アドレスまたはホスト名の後にプラス記号 (+) とアクセス キーを付け加える必要があります。
> [!NOTE]
...
指定されたファイルにデータをエクスポート\<outfile >。

このファイルはデータが含まれる XML ファイルです。 「gamesaveutil」と入力すると、例の生成方法を参照できます。

データをエクスポートする場所を指定する方法は 2 つあります。

かどうか/context パラメーターを使用するとでファイル名が指定された\<infile > そのファイルを使用して、ソース データの場所を指定して、正しく設定される ContextDescription セクションが含まれています。
...
ダミー データが生成され、によって指定されたファイルを保存します\<ファイル名 >。
...
この xml の書式を設定するために必要な変更のみ**インポート**、**エクスポート**、および**生成**で*gamesaveutil* を置換する\<アカウント > のメンバー ノード、 \<ContextDescription > を持つノードを\<PackageFamilyName > ノード。
これは、変更は、 \<ContextDescription > これからノード。
