...
1.  状態情報用に、ゲーム内文字列の列挙を定義します。 たとえば、ゲームで 5 つのマップがあり、それらのマップをリッチ プレゼンス文字列に含める場合、5 つのマップをローカライズされた文字列としても列挙する必要があります。 XDP で列挙の構成がサポートされるまでは、担当の DAM に返送するスプレッドシートで情報を指定することにより、これらの定義を手動で行う必要があります。
