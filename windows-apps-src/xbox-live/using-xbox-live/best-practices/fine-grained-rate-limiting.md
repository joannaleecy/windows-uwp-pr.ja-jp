...
タイトルとユーザーの組み合わせは、Xbox Live リソースの公平な使用を保証するための鍵として選ばれました。 ユーザーだけの追跡では、ユーザー エクスペリエンスが各タイトルの統合に翻弄されるシナリオが生まれます。 たとえば、ほとんどのタイトルでは People サービスを既に使用していますが、公平な使用という目的のために、5 分間で 100 回までは要求を許容する、きめ細かなレート制限を People サービスに対して設定するとします。 ユーザーが 1 分で 100 個の要求を行ったゲームをプレイする場合、制限を超えるし、ユーザーは、ユーザー、サービスに要求を作成できません。同じ期間にユーザーがホーム画面に戻るし、彼の友人のリストをクリックするを想像してください。以降、ユーザーは既に制限を超えましたが友人リストの呼び出しがこの期間が経過すると、5 分まで失敗する場合でも、ホーム画面が制限された状態で、ユーザーを配置する責任を負います。
...
| 期間 (秒)  | バースト期間内の要求数  | サステイン期間内の要求数  | \# 15 秒間隔内でのスロットルされた要求の | どちらの制限? (バースト、サステイン、または両方)  |
|-------------|--------------|----------------|-----------------------------------------------------|---------------------------|
| 0-15        | 35           | 35             | 5                                                   | バースト                     |
| 15-30       | 28           | 63             | 0                                                   | なし                       |
| 30-45       | 21           | 84             | 0                                                   | なし                       |
| 45-60       | 36           | 120            | 20                                                  | どちらもオン                      |
...
|-----------------|------------|------------------------|----------------------------------|
| バージョン         | 整数型    | `"version":1`          |                                  |
| currentRequests | 整数型    | `"currentRequests":13` | 送信された要求の総数    |
| maxRequests     | 整数型    | `"maxRequests":10`     | 許可された要求の総数 |
| periodInSeconds | 整数型    | `“periodInSeconds”:15` | 期間の長さ                      |
| 種類            | String     | `“type”:”burst”`       | スロットル制限タイプ              |

## <a name="implemented-limits"></a>実装された制限

以下のサービスは FGRL 制限を実装しており、これらの制限は **2016 年 5 月**以降適用されています。 繰り返しますが、これらの制限はすべてのサンドボックスおよびタイトルに共通です。 **Xbox 開発者プラットフォームまたはパートナー センターを介して公開され、2016 年 5 月の前に出荷されたすべてのタイトルは従来と見なされ、したがって例外します。**
...
| **名前** | **サービス エンドポイント** | **FGRL の Anticipiated ゲームへの影響**
| --- | :---: | --- 
| 統計情報の読み取り | userstats.xboxlive.com | 実績やランキングのエントリが更新または取得されません。
| Profile | profile.xboxlive.com | プレイヤーのデータが正しく更新または表示されません。
...
| スコアボード | leaderboards.xboxlive.com | ランキングのゲーム内 UX が設定/更新されません。
| 成績 | achievements.xboxlive.com | ロック解除されている実績のゲーム内 UX が更新されません。
...
**注:** 最新の API マッピングは定期的に更新し、下にあります[トレース アナライザー API マッピングの Live](https://github.com/Microsoft/xbox-live-trace-analyzer/blob/master/Source/XboxLiveTraceAnalyzer.APIMap.csv)します。
...
**注:タイトルと同じか上位 10 倍の指定したサステイン制限は 2018 年 4 月の後に証明書を渡すには許可されません**します。  たとえば、上の表で指定されているように、FGRL が有効になるサステイン制限が、300 秒間に 300 の呼び出しに設定されている場合、300 秒間に呼び出しが 3000 以上であるタイトルは認定されません。
