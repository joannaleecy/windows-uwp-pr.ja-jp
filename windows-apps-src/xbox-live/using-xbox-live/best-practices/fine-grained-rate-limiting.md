...
以下のサービスは、**2016 年 5 月に施行開始される** FGRL 制限を実装します。 繰り返しますが、これらの制限はすべてのサンドボックスおよびタイトルに共通です。 **現在は XDP またはデベロッパー センターにあり、2016 年 5 月頃に出荷されるタイトルはすべてレガシーとみなされ、制限の適用が免除されます。** 以下の制限はまだ最終的に確定していません。

| **サービス名**          | **バースト制限 (15 秒間、1 ユーザーおよび 1 タイトルあたり)** | **サステイン制限 (300 秒間、1 ユーザーおよび 1 タイトルあたり)** |
|---------------------------|-------------------------------------------------|----------------------------------------------------|
| ゲーム クリップ転送       | 50                                              | 300                                                |
| ゲーム クリップ メタデータ       | 50                                              | 300                                                |
| スマート マッチ (MPSD ではない)    | 10                                              | 100                                                |
| タイトル ストレージ             | 30                                              | 300                                                |
| 統計情報の読み取り (作成ではない) | 100                                             | 300                                                |
| 実績の読み取り         | 100                                             | 100                                                |
| ランキングの読み取り         | 30                                              | 100                                                |
| プロフィール                   | 10                                              | 30                                                 |
| People                    | 10                                              | 30                                                 |
| プライバシー                   | 10                                              | 30                                                 |
| プレゼンス                  | 読み取り 10、書き込み 3                                | 読み取り 100、書き込み 30                                 |
...
「[Xbox Live Best Practices](best-practices-for-calling-xbox-live.md)」ドキュメントを参照してください。呼び出しパターンを改善するための手順に加えて、スロットリングの問題を通知および軽減するために XSAPI アサーション、XSAPI Social Manager、XSAPI Multiplayer Manager、および Live トレース ツールをどのように使用できるかを説明しています。 ベスト プラクティス ドキュメントは GDNP から入手でき、1602 以降の SDK および XDK ドキュメントに含まれています。
...
dev/cert サンドボックスでは 4 月から制限が導入されます。 5 月には RETAIL で制限が導入される可能性があります。
...
RETAIL での制限導入前に XDP またはデベロッパー センターにあったタイトルはレガシーとみなされ、制限が免除されます。
