...
-   **GameRenderer**オブジェクト。派生した Direct3D 11 のレンダラーを表す、 **DirectXBase**デバイスに固有のすべてのオブジェクトと、レンダリングを処理するクラスです。 詳細については、[レンダリングのフレームワークは](tutorial--assembling-the-rendering-pipeline.md)を参照してください。
-   **オーディオ**オブジェクト。ゲームのオーディオ再生を制御します。 詳細については、[サウンドを追加する](tutorial--adding-sound.md)を参照してください。
