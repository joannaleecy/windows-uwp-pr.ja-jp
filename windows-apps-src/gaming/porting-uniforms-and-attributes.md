...
| バッファー オブジェクト             | バッファー。参照してください[ **D3D11\_SUBRESOURCE\_データ**](https://msdn.microsoft.com/library/windows/desktop/ff476220)と[ **D3D11\_バッファー\_DESC** ](https://msdn.microsoft.com/library/windows/desktop/ff476092)と一般的な使用バッファー定義します。 |
...
-   設定、 [ **D3D11\_SUBRESOURCE\_データ**](https://msdn.microsoft.com/library/windows/desktop/ff476220)バッファーのデータ要素の構成を含む構造体。
-   設定、 [ **D3D11\_バッファー\_DESC** ](https://msdn.microsoft.com/library/windows/desktop/ff476092)バッファーの種類と同様に、バッファー内の個々 の要素のサイズを含む構造体。
...
Direct3D では、uniform データを "定数バッファー" として指定します。定数バッファーには、uniform と同じように、個々のシェーダーに渡す定数データを含めます。 uniform バッファーと同じように、シェーダーが解釈できる方法でメモリ内の定数バッファー データをパックすることが重要です。 DirectXMath 型を使用して (よう[ **XMFLOAT4**](https://msdn.microsoft.com/library/windows/desktop/ee419608)) プラットフォームの種類ではなく (など**float\*** または**float\[4\]**) で適切なデータ要素のアラインメントが保証されます。
