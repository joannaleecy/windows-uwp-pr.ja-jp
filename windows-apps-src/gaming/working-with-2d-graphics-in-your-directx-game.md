...
-   ピクセル座標とラスター座標 ピクセルとは、ラスター ディスプレイ上の 1 個の点のことです。ピクセルには固有の座標ペア (x, y) が与えられ、これによりディスプレイ上の位置を表します  (「ピクセル」という用語は多くの場合、同義表示を構成する物理ピクセルと表示に送信される前に、ピクセルの色とアルファ値を保持するために使用されるメモリをアドレス指定可能な要素の間です。)ラスターは、Api で多くの場合、ディスプレイの物理的なピクセル グリッドと 1 対 1 対応のある四角形のピクセルの要素として扱われます。 ラスター座標系は、グリッドの左上端のピクセル (0, 0) から開始します。
