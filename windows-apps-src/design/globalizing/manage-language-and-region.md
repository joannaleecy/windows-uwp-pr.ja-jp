...
言語リソース修飾子でリソース ファイルまたはそのフォルダーに名前を付けます。 リソース修飾子の詳細については、「[言語、スケール、ハイ コントラスト、その他の修飾子用にリソースを調整する](../../app-resources/tailor-resources-lang-scale-contrast.md)」を参照してください)。 リソース ファイルは、イメージ (またはその他のアセット) をすることができますか、テキスト文字列が含まれている *.resw*などのリソースのコンテナー ファイルであることができます。
...
ローカライズは、すべてのリソースに必要なできない可能性があります。

- 少なくとも、既定の言語ですべてのリソースが存在しないことを確認します。
- 密接に関連する言語 (部分的なローカライズ) の一部のリソースのサブセットだけで十分です。 たとえば、アプリの完全なリソースのセットがスペイン語で用意されている場合、アプリの UI でカタルニア語にローカライズされていないものがある可能性があります。 カタルニア語を話し、し、スペイン語も話すユーザー、カタルニア語では利用できないリソースはスペイン語で表示されます。
- 一部のリソースは、共通のリソースには、その他のリソース マップの大部分の中に、言語固有の例外を必要があります。 この例では、すべての言語が未定義の言語タグ 'und' で使用するもので、リソースをマークします。 Windows では 'und' 言語タグがワイルドカード ('\*' と同様) として解釈され、最上位のアプリ言語と対応付けされるのはその他の固有の対応付けの後です。 たとえば、いくつかのリソース (要素の幅など) がフィンランド語では異なっていて、残りのリソースはすべての言語で同じであれば、フィンランド語のリソースはフィンランド語の言語タグでマークし、残りは 'und' でマークする必要があります。
- フォントや、テキストの高さなど、言語のスクリプトに基づくリソースは、指定されたスクリプトと共に非決定言語タグを使用してください: 'und -&lt;スクリプト&gt;' です。 たとえば、ラテン語フォントの場合は `und-Latn\\fonts.css` を使い、キリル語フォントの場合は `und-Cryl\\fonts.css` を使います。
