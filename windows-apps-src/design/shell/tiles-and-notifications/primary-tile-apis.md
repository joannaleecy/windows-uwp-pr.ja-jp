...
> [!NOTE]
> ながら、アプリがフォア グラウンドにし、ユーザーが意図的にプライマリ タイル bepinned を要求した (たとえばの後、ユーザーは、タイルのピン留めについてのヒントに [はい] をクリックして) この APIafterthe をのみ呼び出す必要があります、UI スレッドからこの API を呼び出す必要があります。
