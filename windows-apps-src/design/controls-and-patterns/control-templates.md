...
> [!NOTE]
> Windows 10、バージョンは 1809 以降 ([SDK 17763](https://developer.microsoft.com/windows/downloads/windows-10-sdk)) を使用することができます[ **X:bind** ](https://msdn.microsoft.com/library/windows/apps/Mt204783)マークアップ拡張機能を使用する場所で[TemplateBinding](../../xaml-platform/templatebinding-markup-extension.md). 詳しくは、「[TemplateBinding マークアップ拡張](../../xaml-platform/templatebinding-markup-extension.md)」をご覧ください。
...
次の XAML は、`Checked`、`Unchecked`、`Indeterminate` の各状態に対する [**VisualState**](https://msdn.microsoft.com/library/windows/apps/br209007) オブジェクトを示しています。 この例では、[**ControlTemplate**](https://msdn.microsoft.com/library/windows/apps/br209391) のルート要素である [**Border**](https://msdn.microsoft.com/library/windows/apps/br209250) に対して [**VisualStateManager.VisualStateGroups**](https://msdn.microsoft.com/library/windows/apps/hh738505) 添付プロパティを設定します。 `Checked` **VisualState**ことを指定します、 [**不透明度**](/uwp/api/Windows.UI.Xaml.UIElement.Opacity)の[**パス**](/uwp/api/Windows.UI.Xaml.Shapes.Path)という名前`CheckGlyph` (これは、前の例で説明) は 1 です。 `Indeterminate` **VisualState**ことを指定します、**不透明度**の[**楕円**](/uwp/api/Windows.UI.Xaml.Shapes.Ellipse)という`IndeterminateGlyph`は 1 です。 `Unchecked` **VisualState**にない[ **Setter** ](https://msdn.microsoft.com/library/windows/apps/br208817)または[**ストーリー ボード**](https://msdn.microsoft.com/library/windows/apps/br210490)のため、 [**チェック ボックスをオン**](https://msdn.microsoft.com/library/windows/apps/br209316)既定の外観を返します。
...
| `Unchecked` から `Checked`。       | [ **Setter** ](https://msdn.microsoft.com/library/windows/apps/br208817)の値、 `Checked` [ **VisualState** ](https://msdn.microsoft.com/library/windows/apps/br209007)が適用されるため、 [**不透明度** ](/uwp/api/Windows.UI.Xaml.UIElement.Opacity)の`CheckGlyph`は 1 です。                                                                                                                                                         | X が表示される。                                |
| `Checked` から `Indeterminate`。   | [ **Setter** ](https://msdn.microsoft.com/library/windows/apps/br208817)の値、 `Indeterminate` [ **VisualState** ](https://msdn.microsoft.com/library/windows/apps/br209007)が適用されるため、 [**不透明度** ](/uwp/api/Windows.UI.Xaml.UIElement.Opacity)の`IndeterminateGlyph`は 1 です。 **Setter**の値、 `Checked` **VisualState**が削除されるため、 [**不透明度**](https://msdn.microsoft.com/library/windows/apps/br228078)の`CheckGlyph`は 0 です。 | 円が表示される。                            |
| `Indeterminate` から `Unchecked`。 | [ **Setter** ](https://msdn.microsoft.com/library/windows/apps/br208817)の値、 `Indeterminate` [ **VisualState** ](https://msdn.microsoft.com/library/windows/apps/br209007)が削除されるため、 [**不透明度** ](/uwp/api/Windows.UI.Xaml.UIElement.Opacity)の`IndeterminateGlyph`は 0 です。                                                                                                                                           | 何も表示されない。                             |
